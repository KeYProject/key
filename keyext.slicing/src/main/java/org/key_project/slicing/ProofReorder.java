...
public final class ProofReorder {
    public static void reorderProof(Proof proof, DependencyGraph depGraph) {
        ...
        for (int i = 1; i <= root.sequent().size(); i++) {
            var node = depGraph.getGraphNode(proof, BranchLocation.ROOT, PosInOccurrence.findInSequent(root.sequent(), i, PosInTerm.getTopLevel()));
            q.addLast(node);
        }
        ...
        // final children
        var finalChildren = newOrder.stream().max(Comparator.comparing(n -> n.serialNr())).get().childrenIterator();
