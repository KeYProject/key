 ...
 * @author Arne Keller
 */
public record GraphvizResult(BufferedImage image,String error){public GraphvizResult{if((image!=null)==(error!=null)){throw new IllegalArgumentException("result must either be an image or an error");}}

/**
 * @param image rendered image
 * @return new result object with that image attached
 */
public static GraphvizResult makeImage(BufferedImage image){return new GraphvizResult(image,null);}

/**
 * @param error error text
 * @return new result object with that error attached
 */
public static GraphvizResult makeError(String error){return new GraphvizResult(null,error);}
 ...
 */
@Override public BufferedImage image(){return image;}
