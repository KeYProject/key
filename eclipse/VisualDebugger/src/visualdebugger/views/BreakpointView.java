...
public class BreakpointView extends ViewPart {
	...
	class BpContentProvider implements IStructuredContentProvider {
		...
		public Object[] getElements(Object parent) {
            ...
            else return new String[] { "One", "Two", "Three" };
		}
	}
	...
	public void createPartControl(Composite parent) {
		viewer = new TableViewer(parent, SWT.MULTI | SWT.H_SCROLL | SWT.V_SCROLL |SWT.SEPARATOR);
                ...
                if(event.getSelection() instanceof IStructuredSelection) {
                    IStructuredSelection selection = (IStructuredSelection)event.getSelection();
                       ...
                       //     ICompilationUnit unit = bp.getCompilationUnit();
                            ...
                            try {
                                IMarker[] markers = bp.getCompilationUnit().getResource().findMarkers("VisualDebugger.bpmarker", true, 1);
                                for (int i=0;i<markers.length;i++){
                                   
                                    if (((Integer)markers[i].getAttribute("StatementId")).intValue()==bp.getId().getId()){
                                        marker = markers[i];
                                    }
                                }
                            ...
                            try{
                                IEditorPart ed = org.eclipse.ui.ide.IDE.openEditor(page, marker, true);
                            ...
                            /*//bp.getAnnotation().getSourceViewer().setSelectedRange(bp.getSelection().getOffset(), bp.getSelection().getLength());
		...
		addAction = new Action() {
			public void run() {
                ...
                if (editor instanceof ITextEditor){
                    ...
                    IFile file = (IFile)
                    tedit.getEditorInput().getAdapter(IFile.class);
...
//                        System.out.println(method.getClass());
                    ...
                    FindStatementVisitor visitor = new FindStatementVisitor(tsel.getOffset());
               ...
               //     ISourceViewer sviewer = getSourceViewer(file);
                  ...
                  //  BreakpointAnnotation annotation = new BreakpointAnnotation("VisualDebugger.BpAnnotationType",true,"Statement Breakpoint",sviewer);
                                                        //new BreakpointAnnotation("VisualDebugger.BpAnnotationType",marker);
                    //System.out.println("P " +annotation.isPersistent());
                   BreakpointEclipse bp = new BreakpointEclipse(new SourceElementId("",visitor.getStatementId()),visitor.getTextSelection(),visitor.getStatement(),unit,method);
                   if(!bpManager.addBreakpoint(bp))
                       return;    
                   try {
                       Map map = new HashMap();
                       map.put("StatementId", new Integer(visitor.getStatementId()));
                       //MarkerUtilities.setLineNumber(map, 10);
                       MarkerUtilities.setCharStart(map, visitor.getTextSelection().getOffset());
                       MarkerUtilities.setCharEnd(map, visitor.getTextSelection().getOffset()+visitor.getTextSelection().getLength());
                      
                       MarkerUtilities.createMarker(unit.getResource(),map,"VisualDebugger.bpmarker");
                   ...
                   // TODO start/end line
                     //IDocument doc = tedit.getDocumentProvider().getDocument(null);
                     //IRegion i = doc.getLineInformationOfOffset(bp.getSelection().getOffset());
                     // Line Tracker...
                    
         //           sviewer.getAnnotationModel().
           //                             addAnnotation(annotation,         
             //                      new Position(visitor.getTextSelection().getOffset(), visitor.getTextSelection().getLength()));
                 //sviewer.revealRange(visitor.getTextSelection().getOffset(), visitor.getTextSelection().getOffset());
                 
                 
                     viewer.setInput(bpManager);
                     DebuggerEvent event = new DebuggerEvent(DebuggerEvent.TREE_CHANGED,ExecutionTree.getITNode());
        ...
        deleteAction = new Action() {
            public void run() {
                ...
                if (domain instanceof BreakpointEclipse){
                     ...
                     //ISourceViewer sviewer =  bp.getAnnotation().getSourceViewer();
