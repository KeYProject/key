...
public class VisualDebuggerView extends ViewPart implements DebuggerListener {
	...
	public void createPartControl(Composite parent) {
            ...
            parent.setLayout(layout);
             ...
             l.setToolTipText("Shows the path condition for the current selected node in the execution tree");
...
//                    SubStatusLineManager statusLineManager=(SubStatusLineManager)(getViewSite().getActionBars().getStatusLineManager());
...
//                cmpTextEditor.setLayoutData(gid);
             ...
             statusMgr.getControl().setLayoutData(new GridData(GridData.FILL_HORIZONTAL));
...
//		// Other plug-ins can contribute there actions here
	...
	private void makeActions() {
		...
		runAction.setToolTipText("Run "+vd.getRunLimit()+" statements");
                runAction
                .setImageDescriptor(getImageDescriptor("run_exc.gif"));
		...
		stepIntoAction.setToolTipText("Step Into");
             ...
             showMainWindowAction = new Action("KeY-Prover Window",Action.AS_CHECK_BOX) {
                 public void run() {                     
                    if (this.isChecked()){
                        Main.getInstance().setVisible(true);
                       // Main.getInstance().toFront();
                    }
                    else Main.getInstance().setVisible(false);
            ...
            showMainWindowAction.setToolTipText("Shows the KeY-Prover which runs in the background");
            
            

            quanSplitAction = new Action("Quantifier Instantiation with Splitting ",Action.AS_CHECK_BOX) {
                public void run() {                     
           ...
           //showMainWindowAction.setText("KeY-Prover");
            //quanSplitAction.setToolTipText("Shows the KeY-Prover which runs in the background");
            
            
            
            
            
            this.allInvariants = new Action("Use all Invariants",Action.AS_CHECK_BOX) {
                public void run() {                     
                    StartVisualDebuggerAction.allInvariants=allInvariants.isChecked();
                    
                }
            };
            allInvariants.setChecked(false);            
            //allInvariants.setText("KeY-Prover");
            allInvariants.setToolTipText("Adds all class invariants to the precondition");



            
            this.showImpliciteAttributes = new Action("Show Implicite Attributes",Action.AS_CHECK_BOX) {
                public void run() {                     
                    VisualDebugger.showImpliciteAttr=showImpliciteAttributes.isChecked();
                    
                }
            };
            showImpliciteAttributes.setChecked(false);            
            //allInvariants.setText("KeY-Prover");
            //showImpliciteAttributes.setToolTipText("Adds all class invariants to the precondition");
        ...
        doubleClickAction = new Action() {
			public void run() {
				...
				Object obj = ((IStructuredSelection)selection).getFirstElement();
				    ...
				    vd.getMediator().getSelectionModel().setSelectedNode(((ITNode)obj).getNode());
                                }
	...
	private void showMessage(String message) {
        ...
        public synchronized void update(DebuggerEvent event){
            if (event.getType()==DebuggerEvent.PROJECT_LOADED_SUCCESSFUL){
                ...
                if (event.getType()==DebuggerEvent.EXEC_FINISHED){                    
                    setStatusMessage(projectName);
                }else
                    if (event.getType()==DebuggerEvent.EXEC_STARTED){                        
