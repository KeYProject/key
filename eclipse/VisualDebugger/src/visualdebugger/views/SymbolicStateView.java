...
public class SymbolicStateView extends ViewPart implements DebuggerListener {
    ...
    private void hookShell() {
        ...
        prestateForTracesSlider.addSelectionListener(new SelectionAdapter() {
            public void widgetSelected(SelectionEvent e) {
                ...
                possibleIndexTerms = stateVis
                        .getPossibleIndexTermsForPcState(sel);
                if (possibleIndexTerms.length > 0)
                    currentState = stateVis.getSymbolicState(sel,
                            possibleIndexTerms[0], true);
                else
        ...
        arrayIndexSlider.addSelectionListener(new SelectionAdapter() {
            public void widgetSelected(SelectionEvent e) {
                if (preState)
                    currentState = stateVis.getSymbolicState(
                            prestateForTracesSlider.getSelection(),
                            (SetOfTerm) possibleIndexTerms[arrayIndexSlider
                                    .getSelection()], true);
                else
                    currentState = stateVis.getSymbolicState(
                            prestateForTracesSlider.getSelection(),
        ...
        preButton.addSelectionListener(new SelectionAdapter() {
            public void widgetSelected(SelectionEvent e) {
                ...
                currentState = stateVis.getSymbolicState(
                        prestateForTracesSlider.getSelection(),
                        possibleIndexTerms[arrayIndexSlider.getSelection()],
                        true);
                refreshVisualizedState();
            }
        });
    ...
    private void startRefreshThread() {
        ...
        final Thread barThread = new Thread(
                "Refresh Symbolic State View Thread") {
            public void run() {
                refreshPCStates();
            }
        };
    ...
    private void refreshPCStates() {
        ...
        if (stateVis != null) {
            ...
            if (possibleIndexTerms.length > 0)
                currentState = stateVis.getSymbolicState(0,
                        possibleIndexTerms[0], true);
            else
    ...
    private synchronized void refreshVisualizedState() {
            ...
            for (Iterator soEndsIt = ofStart.getSymbolicObject()
                ...
                if (!soEnd.isNull()) {
                    final ObjectFigure ofEnd = getOFbySO(figure2node.keySet(),
                            soEnd);
                    if (ofEnd != ofStart) {
                        final Edge edge = new Edge((Node) figure2node
                                .get(ofStart), (Node) figure2node.get(ofEnd));
        ...
        for (Iterator ofIt = figure2node.keySet().iterator(); ofIt.hasNext();) {
            ...
            if (ofStart instanceof ArrayObjectFigure
                    && !((SymbolicArrayObject) ((ArrayObjectFigure) ofStart)
                            .getSymbolicObject()).isPrimitiveType()) {
                ArrayObjectFigure aofStart = (ArrayObjectFigure) ofStart;
                SymbolicArrayObject saoStart = ((SymbolicArrayObject) aofStart
                        .getSymbolicObject());

                for (IteratorOfTerm indexIt = saoStart.getAllIndexTerms()
                        .iterator(); indexIt.hasNext();) {
                    Term index = indexIt.next();
                    SymbolicObject soEnd = ((SymbolicArrayObject) aofStart
                            .getSymbolicObject())
                            .getAssociationEndForIndex(index);
                    if (!soEnd.isStatic()) {
                        final HashMap index2y = aofStart.getIndexY();
                        final ObjectFigure ofEnd = getOFbySO(figure2node
                                .keySet(), soEnd);
                        connections.add((createArrayConnection(ofStart,
                                ((Integer) index2y.get(index)).intValue(),
                                ofEnd, "")));
                    }
    ...
    private void setSelected(Object o) {
        if (o instanceof ObjectFigure.AttributeLabel) {
            ...
            if (al.getSo().getAttributes().contains(al.getPv()))
                this.setConstraints(al.getSo().getAttrConstraints(al.getPv()),
                        symbolicObjects, al.getSo());
            else {
                this.setConstraints(currentState.getConstraints(al
                        .getValueterm()), symbolicObjects, al.getSo());
                System.out.println(currentState.getConstraints(al
                        .getValueterm()));
            ...
            if (al.getSo().getConstraintsForIndex(al.getIndex()) != SLListOfTerm.EMPTY_LIST)
                setConstraints(
                        al.getSo().getConstraintsForIndex(al.getIndex()),
                        symbolicObjects, al.getSo());
            else
            ...
            for (Iterator it = this.symbolicObjects.iterator(); it.hasNext();) {
                ...
                if (next instanceof SymbolicArrayObject) {
                    ...
                    if (sao != al.getSo()) {
                        indexConstr = indexConstr.union(sao
            ...
            if (result > -1) {
                if (!preState)
                    currentState = stateVis.getSymbolicState(
                            prestateForTracesSlider.getSelection(),
                            possibleIndexTerms[result], false);
                else
                    currentState = stateVis.getSymbolicState(
                            prestateForTracesSlider.getSelection(),
                            possibleIndexTerms[result], true);
    ...
    private void setConstraints(ListOfTerm cons, LinkedList sos,
            SymbolicObject so) {
        if (cons != null) {
