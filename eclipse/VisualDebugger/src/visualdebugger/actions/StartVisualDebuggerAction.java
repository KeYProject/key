...
public class StartVisualDebuggerAction implements IObjectActionDelegate {
    ...
    public void run(IAction action) {                    
        ...
        if(selection != null && selection instanceof StructuredSelection) {
            ...
            if(srcFile == null) {
                ...
                visualdebugger.Activator.getDefault().setProject(srcFile.getJavaProject());
                ...
                if(status==PROJECT_ALREADY_OPEN ||
                    ...
                    try {
                        ...
                        final JMLPOAndSpecProvider provider = keyProver.getJMLPOAndSpecProvider();
                        ...
                        if (spec!=null){
                            startProver("Debugging "+selectedMethod.getElementName(), provider, spec, 
                                    allInvariants, false, false);                          
                        } else {
                            ...
                            if(state == Window.OK) {
                                ...
                                if(selectedPO instanceof AssignableCheckProofOblInput) {
                                    ...
                                    startProver("Debugging "+selectedMethod.getElementName(),
                                            provider, assignableCheckPO.getSpec(), 
                                            allInvariants, addInvariantsToPostCondition, true);                          
                                } else if(selectedPO instanceof JMLSpec){                                    
                                    startProver("Debugging "+selectedMethod.getElementName(),
                                            provider, (JMLSpec)selectedPO, 
                                            allInvariants, addInvariantsToPostCondition, false);   
                                } else {
    ...
    private void startProver(
            String debuggerEventMsg,
            final JMLPOAndSpecProvider provider, 
            final JMLSpec spec,
            final boolean allInvariants,
            final boolean invPost, 
            final boolean assignable) {
        VisualDebugger.getVisualDebugger().
        fireDebuggerEvent(new DebuggerEvent(DebuggerEvent.PROJECT_LOADED_SUCCESSFUL, 
                debuggerEventMsg));
        //Runnable keyThread = new Runnable() {
        //    public void run() {
                provider.createPOandStartProver(spec, allInvariants, invPost, assignable);                                    
        //    }
            
        //};
        //SwingUtilities.invokeLater(keyThread);
    }
    ...
    protected synchronized int assertProjectParsed(IProject project,
        ...
        String inputName = "visualDebugger-project_"
            + project.getName();
        //File location = project.getLocation().toFile();
        File location = new File(VisualDebugger.tempDir);
        VisualDebugger.print("tempDir "+VisualDebugger.tempDir);
    ...
    private MethodDeclaration getSepMethodDeclarationDEPRECATED(AST ast,ITypeBinding type) {
        ...
        SimpleType loggerType =  ast.newSimpleType( ast.newName( type.getQualifiedName()));
        ...
        variableDeclaration2.setType((Type)ASTNode.copySubtree(ast,loggerType));
    ...
    public void insertSeps(IJavaProject projekt){       
        ICompilationUnit[] units= getTypes(projekt);
        types=new HashSet();
        debugCU = createDebuggerClass(AST.newAST(AST.JLS3));
        // System.out.println("porject"+projekt.getPath());
        ...
        for(Iterator it = types.iterator(); it.hasNext();){
            ...
            td.bodyDeclarations().add(getSepMethodDeclaration(debugCU.getAST(), this.getType(debugCU.getAST(),next)));

        }

        td.bodyDeclarations().add(getSepMethodDeclaration(debugCU.getAST(), debugCU.getAST().newPrimitiveType(PrimitiveType.INT)));
        td.bodyDeclarations().add(getSepMethodDeclaration(debugCU.getAST(), debugCU.getAST().newPrimitiveType(PrimitiveType.BYTE)));
        td.bodyDeclarations().add(getSepMethodDeclaration(debugCU.getAST(), debugCU.getAST().newPrimitiveType(PrimitiveType.BOOLEAN)));
        td.bodyDeclarations().add(getSepMethodDeclaration(debugCU.getAST()));



        String s= debugCU.toString();       

        String projectPath = projekt.getPath().toOSString().substring(1);
        ...
        new File(VisualDebugger.tempDir+       projectPath+File.separator+VisualDebugger.debugPackage
    ...
    public void insertSeps(ICompilationUnit unit){
        VisualDebugger.print("InsertSeps to: "+unit.getPath());
        boolean junit = false;
        if (unit.getImport("junit.framework.*").exists()){
            System.out.println("TEST CASE UNIT");
            junit = true;
        }
        String source="";
        try {
...
//      computation of the new source code
        ...
        String d = VisualDebugger.tempDir+fn.substring(1, fn.lastIndexOf(
                File.separator));
        File fil= new File(d);
        if (!fil.exists())
            fil.mkdirs();
    ...
    private CompilationUnit createDebuggerClass(AST ast) {
        ...
        packageDeclaration.setName(ast.newSimpleName(VisualDebugger.debugPackage));
