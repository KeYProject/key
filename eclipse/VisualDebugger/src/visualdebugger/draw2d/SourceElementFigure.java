    ...
    public SourceElementFigure(ETStatementNode etNode){
       ...
       // if (etNode.getStatementId()!=null){
        if (etNode.getStatementId().isStatement()){
            ...
            if (VisualDebugger.getVisualDebugger().getBpManager().suspendByBreakpoint(etNode.getStatementId())){
                breakpoint = true;
                setBorder(BREAKMODEBORDER);
            }
            else setBorder(BORDER);
         statement=(Statement)visualdebugger.Activator.getDefault().getASTNodeForStatementId(etNode.getStatementId());

        unit= visualdebugger.Activator.getDefault().getCompilationUnit(etNode.getStatementId());
        st = statement.toString();
        }else{
            expression= (Expression)visualdebugger.Activator.getDefault().getASTNodeForStatementId(etNode.getStatementId());
