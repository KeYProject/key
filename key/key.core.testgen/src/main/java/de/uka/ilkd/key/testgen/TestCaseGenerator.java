...
public class TestCaseGenerator {
    ...
    protected String buildDummyClassForAbstractSort(Sort sort) {
        ...
        // Put the string buffer as soon as possible, due to possible recursive calls of this method.
        ...
        sb.append(" public ").append(className).append("(){ };").append(NEW_LINE); // default constructor
    ...
    protected String getOracleAssertion(List<OracleMethod> oracleMethods) {
        ...
        LOGGER.debug("Modifier Set: {}", oracleGenerator.getOracleLocationSet(info.getAssignable()));
    ...
    public StringBuilder createTestCaseCotent(Collection<SMTSolver> problemSolvers) {
        ...
        for (final SMTSolver solver : problemSolvers) {
            try {
                ...
                final String originalNodeName = solver.getProblem().getGoal()
                        /*TODO:Warning this is wrong if we generate a test from an inner node (e.g. closed proof tree),
                ...
                if (solver.getSocket().getQuery() != null) {
                    ...
                    if (TestCaseGenerator.modelIsOK(m)) {
                        ...
                        testMethod.append("   //Test preamble: creating objects and intializing test data")
                        ...
                        testMethod.append(TAB + "//Other variables").append(NEW_LINE)
                                .append(getRemainingConstants(m.getConstants().keySet(), vars)).append(NEW_LINE);
                        testMethod.append("   //Calling the method under test   ").append(NEW_LINE)
                                .append(info.getCode()).append(NEW_LINE);


                        if (junitFormat) {
                            testMethod.append("   //calling the test oracle").append(NEW_LINE).append(TAB)
        ...
        if (i == 0) {
            logger.writeln("Warning: no test case was generated. Adjust the SMT solver settings (e.g. timeout) " +
            ...
            if (!existingConstants.contains(c.toString())) {
                ...
                if (junitFormat) {
                    result.append(NEW_LINE).append(TAB).append(NULLABLE).append(" ")
                            ...
                            .append(getPreName(c.toString())).append(" = ").append(init).append(";");
                }
            }
        }
        ...
        if (heap != null) {
            for (final ObjectVal o : heap.getObjects()) {
                ...
                for (final String f : o.getFieldvalues().keySet()) {
                    ...
                    //possible bug if vType represents an abstract type or an interface. See: getSafeType.
                    ...
                    assignments.add(new Assignment(
                            new RefEx(rcObjType, receiverObject, vType, fieldName), "(" + vType + ")" + val));
    ...
    private String createOldMap(Set<String> objNames) {
        StringBuilder result =
                new StringBuilder(NEW_LINE + TAB + "Map<Object,Object> " + OLDMap + " = new HashMap<Object,Object>();");
    ...
    private StringBuilder getMainMethod(String className, int i) {
        ...
        res.append(" public static void  main (java.lang.String[]  arg) {").append(NEW_LINE).append("   ").append(className).append(" testSuiteObject;").append(NEW_LINE).append("   testSuiteObject=new ").append(className).append(" ();").append(NEW_LINE).append(NEW_LINE);
        for (int j = 0; j < i; j++) {
    ...
    private String createIntSet() {
        ...
        res.append(TAB + "Set<Integer> ").append(allint).append("= new HashSet<Integer>();").append(NEW_LINE);
    ...
    private String createObjSet(Heap h) {
        ...
        res.append(TAB + "Set<Object> " + ALL_OBJECTS + "= new HashSet<Object>();").append(NEW_LINE);
