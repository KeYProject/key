...
/**
 * THIS FILE IS A CLONE FROM THE CORRESPONDING OPENJDK CLASS; IT IS ONLY CHANGED
 * FUNCTIONALLY IN THE {@link #startEdit()} METHOD S.TH. OVERRIDING GETITEMS()
 * HAS AN EFFECT ON THE DISPLAYED ITEMS. CHANGES HAVE BEEN DONE BY DOMINIC
 * SCHEURER (DS).
 * 
 ...
 * 
 * <p>
 * By default, the ComboBoxTableCell is rendered as a {@link Label} when not
 * being edited, and as a ComboBox when in editing mode. The ComboBox will, by
 * default, stretch to fill the entire table cell.
 * 
 * <p>
 * To create a ComboBoxTableCell, it is necessary to provide zero or more items
 * that will be shown to the user when the {@link ComboBox} menu is showing.
...
public class ComboBoxTableCell<S,T> extends TableCell<S,T> {
    ...
    /***************************************************************************
     *                                                                         *
     * Methods from CellUtils (moved here by DS)                               *
     *                                                                         *
     **************************************************************************/  

    // Added here by DS from CellUtils which is not visible
    private ComboBox<T> createComboBox(
            ComboBoxTableCell<S, T> comboBoxTableCell,
            ObservableList<T> items2,
            ObjectProperty<StringConverter<T>> converterProperty) {
        ComboBox<T> comboBox = new ComboBox<T>(getItems());
    ...
    private static <T> void updateItem(final Cell<T> cell,
            final StringConverter<T> converter, final HBox hbox,
...
//        setConverter(converter != null ? converter : CellUtils.<T>defaultStringConverter());
    ...
    @Override public void startEdit() {
        ...
        if (comboBox == null) {
            // Using getItems() instead of items to enable overriding --
            // changed by DS
            comboBox = createComboBox(this, getItems(), converterProperty());
