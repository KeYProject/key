...
public class IconFactory {
    ...
    private static Image keyHoleAlmostClosed = getImage(
            "images/ekey-brackets.gif");
    public static Image keyHoleClosed = getImage("images/keyproved.gif");
    private static Image keyHoleInteractive = getImage(
            "images/keyinteractive.gif");
    ...
    private static Image keyLogoSmall = getImage(
            "images/key-color-icon-square.png");
    private static Icon provedFolderIcon = KeYFolderIcon
            .getKeYFolderIconClosed();
    private static Icon linkedFolderIcon = KeYFolderIcon
            .getKeYFolderIconLinked();
    private static Icon closableFolderIcon = KeYFolderIcon
            .getKeYFolderIconClosable();

    private static Image oneStepSimplifier = getImage(
            "images/toolbar/oneStepSimplifier.png");
    ...
    private static Image autoModeStart = getImage(
            "images/toolbar/autoModeStart.png");
    private static Image autoModeStop = getImage(
            "images/toolbar/autoModeStop.png");
    private static Image decisionProcedureConfigArrow = getImage(
            "images/toolbar/decProcArrow.png");
    ...
    private static Image openMostRecentKeYFile = getImage(
            "images/toolbar/openMostRecent.png");
    ...
    private static Image expandGoals = getImage(
            "images/toolbar/expandGoals.png");
    ...
    private static Image interactiveAppLogo = getImage(
            "images/interactiveAppLogo.png");

    private static Image counterexampleImage = getImage(
            "images/toolbar/ce.png");
    private static Image testgenerationImage = getImage(
            "images/toolbar/tg.png");
    ...
    /**
     * Creates an icon from an image contained in a resource. The resource is
     * fist search using the package name of the calling Object and if it is not
     * found there the package name of its superclass is used recursively.
     * 
     * @param o
     *            the Object reference to the calling object
     * @param filename
     *            String the name of the file to search (only relative pathname
     *            to the path of the calling class)
    ...
    /**
     * Creates an icon from an image contained in a resource. The resource is
     * fist search using the package name of the given class and if the resource
     * is not found the packagename of its superclass is used recursively.
     * 
     * @param cl
     *            the Class the resource is looked for
     * @param filename
     *            String the name of the file to search (only relative pathname
     *            to the path of the calling class)
     * @return the newly created image
    ...
    public static class KeYFolderIcon extends FolderIcon16 {
        ...
        private static final Icon closedIcon = new KeYFolderIcon(
                Color.green.darker());
        private static final Icon closableIcon = new KeYFolderIcon(
                Color.blue.darker());
        private static final Icon linkedIcon = new KeYFolderIcon(
                new Color(255, 0, 240));
    ...
    private static class KeYControlIcon extends TreeControlIcon {
        ...
        private void paintMe(Component c, Graphics g) {
            ...
            int min = getIconWidth() < getIconHeight() ? getIconWidth()
                    : getIconHeight();

            g.setColor(c.getGraphics().getColor());
            g.drawRect(midx - (min / 4), midy - (min / 4), (min / 2) - 1,
                    (min / 2) - 1);

            g.drawLine(midx - (min / 4 - 2), midy, midx + (min / 4 - 2), midy);
            if (collapsed) {
                g.drawLine(midx, midy - (min / 4 - 2), midx,
                        midy + (min / 4 - 2));
            }
        }
    }
