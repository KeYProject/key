...
public class ProofTreeView extends JPanel {
    ...
    private WeakHashMap<Proof, GUIProofTreeModel> models = new WeakHashMap<Proof, GUIProofTreeModel>(
            20);
    ...
    public final static KeyStroke searchKeyStroke = KeyStroke.getKeyStroke(
            KeyEvent.VK_F,
    ...
    /**
     * Roots of subtrees containing all nodes to which rules have been applied;
     * this is used when auto mode is active
     */
    ...
    public ProofTreeView(KeYMediator mediator) {
        ...
        delegateView.getInputMap(JComponent.WHEN_FOCUSED).getParent().remove(
        ...
        MouseListener ml = new MouseAdapter() {
            public void mousePressed(MouseEvent e) {
                if (e.isPopupTrigger()) {
                    TreePath selPath = delegateView.getPathForLocation(e.getX(),
                            e.getY());
                    if (selPath != null && (selPath
                            .getLastPathComponent() instanceof GUIProofTreeNode
                            || selPath
                                    .getLastPathComponent() instanceof GUIBranchNode)) {
    ...
    private void unregister() {
        ...
        mediator.getUI().getProofControl()
                .removeAutoModeListener(proofListener);
    ...
    public void removeNotify() {
        ...
        try {
            ...
            Debug.out(
                    "Exception thrown by class ProofTreeView at unregister()");
        }
        super.removeNotify();
    }
    ...
    private void setProof(Proof p) {
        ...
        if (proof != null) {
            ...
            expansionState = new ExpansionState(delegateView,
            ...
            delegateModel = null;
            delegateView.setModel(new DefaultTreeModel(
                    new DefaultMutableTreeNode("No proof loaded.")));
            expansionState = null;
        }
    ...
    private void collapseClosedNodesHelp(TreePath path) {
        ...
        for (int count = delegateModel
                .getChildCount(node), i = 0; i < count; i++) {
    ...
    /**
     * Selects the given Branchnode in the ProofTreeView and displays the first
     * child in the main view.
     */
    ...
    /**
     * In auto mode, add a node which has been modified in a way leading to
     * structural changes of the proof tree
     */
    ...
    class GUITreeSelectionListener
            implements TreeSelectionListener, java.io.Serializable {
        /**
         *
         */
        ...
        public void valueChanged(TreeSelectionEvent e) {
            ...
            GUIAbstractTreeNode treeNode = ((GUIAbstractTreeNode) e
        ...
        public Component getTreeCellRendererComponent(JTree tree, Object value,
            ...
            if (value instanceof GUIBranchNode) {
                ...
                if (((GUIBranchNode) value).isClosed()) {
                    ...
                    // Find leaf goal for node and check whether this is a
                    // linked goal.

                    // TODO (DS): This marks all "folder" nodes as linked that
                    // have
                    ...
                    proof.breadthFirstSearch(((GUIBranchNode) value).getNode(),
                            v);
                    if (v.isLinked()) {
            ...
            {
                final Node child = ((GUIAbstractTreeNode) value)
                        .findChild(node);
                if (child != null
            ...
            DefaultTreeCellRenderer tree_cell = (DefaultTreeCellRenderer) super.getTreeCellRendererComponent(
            ...
            if (node.leaf()) {
                ...
                if (goal == null || node.isClosed()) {
                    ...
                    if (goal.isLinked()) {
                        ...
                        tree_cell.setToolTipText(
                                "Linked goal - no automatic rule application");
                    }
                    else if (!goal.isAutomatic()) {
                        tree_cell.setForeground(ORANGE_COLOR);
                        tree_cell.setIcon(
                                IconFactory.keyHoleInteractive(20, 20));
                        ProofTreeView.this.setToolTipText("Disabled Goal");
                        tree_cell.setToolTipText(
                                "Interactive goal - no automatic rule application");
                    }
                    else {
                ...
                /*
                 * if ( node.getBranchSink ().getResetConstraint
                 * ().isSatisfiable () ) tree_cell.setForeground(Color.blue);
                 * else
                 */
    ...
    class ProofTreePopupMenu extends JPopupMenu
        ...
        private JMenuItem expandGoals = new JMenuItem("Expand Goals Only",
                IconFactory.expandGoals(ICON_SIZE));
        private JMenuItem expandGoalsBelow = new JMenuItem(
                "Expand Goals Only Below");
        private JMenuItem collapseAll = new JMenuItem("Collapse All",
                IconFactory.minus(ICON_SIZE));
        private JMenuItem collapseOtherBranches = new JMenuItem(
        ...
        private JMenuItem prevSibling = new JMenuItem("Previous Sibling",
                IconFactory.previous(ICON_SIZE));
        private JMenuItem nextSibling = new JMenuItem("Next Sibling",
                IconFactory.next(ICON_SIZE));
        private Map<JCheckBoxMenuItem, ProofTreeViewFilter> filters = new LinkedHashMap<JCheckBoxMenuItem, ProofTreeViewFilter>(); // TODO:
                                                                                                                                   // change
                                                                                                                                   // to
                                                                                                                                   // radio
                                                                                                                                   // button
        ...
        private void create() {
            ...
            for (ProofTreeViewFilter filter : ProofTreeViewFilter.ALL) {
                final JCheckBoxMenuItem m = new JCheckBoxMenuItem(
                        filter.name());
        ...
        public void actionPerformed(ActionEvent e) {
            if (e.getSource() == delayedCut) {
                ...
                Object sibling = delegateModel.getChild(parent,
                        delegateModel.getIndexOfChild(parent, node) - 1);
                if (!(sibling != null && sibling instanceof GUIBranchNode)) {
                    ...
                    for (int i = parent.getChildCount(); i > index; i--) {
                        ...
                        if (sibling != null
                                && sibling instanceof GUIBranchNode) {
                            break;
                        }
                    }
                }
        ...
        private void openNotes() {
            ...
            final String newNotes = (String) JOptionPane.showInputDialog(this,
                    null, "Annotate this proof node", JOptionPane.PLAIN_MESSAGE,
                    editIcon, null, origNotes);
            if (newNotes != null) {
        ...
        private void expandGoalsBelow() {
            ...
            if (branch == path) {
                ...
                for (int count = delegateModel
                        .getChildCount(tmpNode), i = 0; i < count; i++) {
        ...
        /**
         * run automatic on the currently selected node. All enabled goals below
         * the current node are taken into consideration.
         *
         * CAUTION: If the node itself is a goal then allow applying rules to it
         * even if it were disabled. Desired behaviour?
         *
         */
        private void runStrategyOnNode() {
            ...
            // is the node a goal?
            if (invokedGoal == null) {
                ImmutableList<Goal> enabledGoals = proof
                        .getSubtreeEnabledGoals(invokedNode);
        ...
        private void showSubtreeStatistics() {
            ...
            if (proof == null) {
                ...
                for (Pair<String, String> x : invokedNode.statistics()
                        .getSummary()) {
                    if ("".equals(x.second))
                        stats += "\n";
        ...
        private final class SetGoalsBelowEnableStatus extends DisableGoal {
            ...
            public SetGoalsBelowEnableStatus(boolean enableGoals) {
                ...
                if (enableGoals) {
                    putValue(SHORT_DESCRIPTION,
                            "Include this node and all goals in the subtree in automatic rule application");
                    putValue(SMALL_ICON, KEY_HOLE_PULL_DOWN_MENU);
                }
                else {
                    putValue(SHORT_DESCRIPTION,
                            "Exclude this node and all goals in the subtree from automatic rule application");
            ...
            /*
             * In addition to marking setting goals, update the tree model so
             * that the label sizes are recalculated
             */
            @Override
            public void actionPerformed(ActionEvent e) {
                ...
                // trigger repainting the tree after the completion of this
                // event.
                ...
                if (branch == path) {
                    if (e.getStateChange() != ItemEvent.SELECTED) {
                        ...
                        if (invokedNode.parent() == null || delegateModel
                            ...
                            // it's still a branch
                            if (delegateModel
                                    .getRoot() instanceof GUIBranchNode) {
                                TreeNode node = ((GUIAbstractTreeNode) delegateModel
                                        .getRoot()).findBranch(invokedNode);
                                if (node instanceof GUIBranchNode) {
                                    selectBranchNode((GUIBranchNode) node);
