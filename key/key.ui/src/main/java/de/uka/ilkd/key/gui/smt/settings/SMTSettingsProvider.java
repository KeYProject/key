...
public class SMTSettingsProvider extends TablePanel implements SettingsProvider {
    private final LinkedList<SolverOptions> solverOptions = new LinkedList<>();
    private final GeneralOptions generalOptions;
    private SMTSettings temporarySettings;
    ...
    public JComponent getPanel(MainWindow window) {
        ProofDependentSMTSettings pd = window.getMediator().getSelectedProof().getSettings().getSMTSettings();
        //TODO get proof independent settings without a proof
        ProofIndependentSMTSettings pi = window.getMediator().getSelectedProof().getProofIndependentSettings().getSMTSettings();
        temporarySettings = new SMTSettings(pd.clone(), pi.clone(), window.getMediator().getSelectedProof());
        generalOptions.setSmtSettings(temporarySettings);
    ...
    class GeneralOptions extends TablePanel {
        ...
        private final static String infoTimeoutField = "Timeout for the external solvers in seconds. Fractions of a second are allowed.\n"
                + "Example: 6.5";
        private final FileChooserPanel saveToFilePanel;
        ...
        public JTextField createMaxProcesses() {
            return addTextField("Concurrent Processes:",
                    Long.toString(settings.maxConcurrentProcesses),
                    infoMaxProcesses,
                    e -> {
                        int value;
                        try {
        ...
        public JComboBox<String> getProgressModeBox() {
            return addComboBox(infoProgressModeBox, settings.modeOfProgressDialog,
        ...
        public FileChooserPanel getSaveToFilePanel() {
            return addFileChooserPanel("Store translation to file:",
                    ...
                    true, settings.storeSMTTranslationToFile, e -> {
        ...
        public void setSmtSettings(SMTSettings settings) {
            this.settings = settings.getPiSettings();
            saveToFilePanel.getFolderField().setText(this.settings.pathForSMTTranslation);
