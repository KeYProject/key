        ...
        for (int i = 0; i < count; i += 1) {
            ...
            if ("Exceptions".equals(name)) {
                ...
                if (readJava5Signatures) {
                    List<TypeArgumentInfo>[] typeArgInfos = readMethodSignature(prereadParams, typeParams);
    ...
    MethodInfo readMethodInfo() throws IOException, ByteCodeFormatException {
        ...
        if (isConstructor)
            ...
            if ((accessFlags & 0x2000) != 0)
                res = new AnnotationPropertyInfo(methAccessFlags, rtype, name, cf, currentDefaultValue);
            else
