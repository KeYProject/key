...
public class LIGNestedNoHghrBnd extends AbstractLoopInvariantGenerator {
//	private final DependenciesLDT depLDT;
//	private final HeapLDT heapLDT;

	public LIGNestedNoHghrBnd(Sequent sequent, Services services) {
...
//		heapLDT = services.getTypeConverter().getHeapLDT();
	...
	public LoopInvariantGenerationResult generate() {
		...
		outerCompPreds.add(tb.leq(indexOuter, tb.dotLength(arrays[0])));
		outerCompPreds.add(tb.geq(indexInner, lowInner));
		outerCompPreds.add(tb.leq(indexInner, tb.dotLength(arrays[0])));

		outerDepPreds.add(tb.noR(tb.arrayRange(arrays[0], lowOuter, tb.sub(tb.dotLength(arrays[0]),tb.one()))));
		outerDepPreds.add(tb.noW(tb.arrayRange(arrays[0], lowOuter, tb.sub(tb.dotLength(arrays[0]),tb.one()))));

		innerCompPreds.add(tb.geq(indexInner, lowInner));
		innerCompPreds.add(tb.leq(indexInner, tb.dotLength(arrays[0])));

		innerDepPreds.add(tb.noR(tb.arrayRange(arrays[0], lowInner, tb.sub(tb.dotLength(arrays[0]),tb.one()))));
		innerDepPreds.add(tb.noW(tb.arrayRange(arrays[0], lowInner, tb.sub(tb.dotLength(arrays[0]),tb.one()))));
...
//		outerDepPreds = compressor.compress();
	...
	private Sequent modifySeqAnte(Goal g) {
		...
		initialFormulas.add(tb.leq(indexInner, tb.dotLength(arrays[0])));
		for (Term arr : arrays) {
			initialFormulas.add(tb.noR(tb.arrayRange(arr, lowInner, tb.sub(tb.dotLength(arrays[0]),tb.one()))));
			initialFormulas.add(tb.noW(tb.arrayRange(arr, lowInner, tb.sub(tb.dotLength(arrays[0]),tb.one()))));
		}
