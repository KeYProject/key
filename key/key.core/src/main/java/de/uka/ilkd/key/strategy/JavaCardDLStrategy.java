...
public class JavaCardDLStrategy extends AbstractFeatureStrategy {
    ...
    private RuleSetDispatchFeature setupCostComputationF() {
        ...
        bindRuleSet(d, "javaIntegerSemantics",
                ifZero(sequentContainsNoPrograms(), longConst(-5000),
                        ifZero(leq(CountBranchFeature.INSTANCE, longConst(1)), longConst(-5000),
                                inftyConst())));
        ...
        bindRuleSet(d, "order_terms", add(ifZero(applyTF("commEqLeft", tf.intF),
        ...
        bindRuleSet(d, "simplify_prog", ifZero(
                ThrownExceptionFeature.create(exceptionsWithPenalty, getServices()), longConst(500),
        ...
        // delete cast
        bindRuleSet(d, "cast_deletion", ifZero(implicitCastNecessary(instOf("castedTerm")),
        ...
        if (programsToRight) {
            bindRuleSet(d, "boxDiamondConv",
                    SumFeature.createSum(new FindPrefixRestrictionFeature(
        ...
        if (strategyProperties.contains(StrategyProperties.AUTO_INDUCTION_ON)
            ...
            bindRuleSet(d, "induction_var",
                    ifZero(applyTF(instOf("uSub"), IsInductionVariable.INSTANCE), longConst(0),
                            inftyConst()));
        }
    ...
    private void setUpStringNormalisation(RuleSetDispatchFeature d) {
        ...
        final TermFeature keepChar = or(op(seqLDT.getSeqSingleton()), or(
                op(charListLDT.getClIndexOfChar()),
        ...
        bindRuleSet(d, "defOpsConcat", add(NonDuplicateAppModPositionFeature.INSTANCE, ifZero(
        ...
        bindRuleSet(
                d, "defOpsReplaceInline", ifZero(
                        add(applyTF("str", seqLiteral), applyTF("searchChar", charOrIntLiteral),
                                applyTF("replChar", charOrIntLiteral)),
                        longConst(-2500), inftyConst()));
        ...
        bindRuleSet(d, "stringsConcatNotBothLiterals",
                ifZero(MatchedIfFeature.INSTANCE,
                        ifZero(add(applyTF(instOf("leftStr"), seqLiteral),
                                applyTF(instOf("rightStr"), seqLiteral)), inftyConst()),
                        inftyConst()));
        ...
        bindRuleSet(d, "stringsContainsDefInline", SumFeature
                .createSum(new Feature[] { EqNonDuplicateAppFeature.INSTANCE, longConst(1000) }));
    }

    private void setupReplaceKnown(RuleSetDispatchFeature d) {
        final Feature commonF =
                add(ifZero(MatchedIfFeature.INSTANCE, DiffFindAndIfFeature.INSTANCE),
                        longConst(-5000), add(DiffFindAndReplacewithFeature.INSTANCE,
        ...
        bindRuleSet(d, "replace_known_right",
                add(commonF, ifZero(DirectlyBelowSymbolFeature.create(Junctor.IMP, 1),
                        longConst(100),
    ...
    protected boolean arithDefOps() {
        return StrategyProperties.NON_LIN_ARITH_DEF_OPS.equals(
                strategyProperties.getProperty(StrategyProperties.NON_LIN_ARITH_OPTIONS_KEY))
                || StrategyProperties.NON_LIN_ARITH_COMPLETION.equals(strategyProperties
    ...
    private void setupSplitting(RuleSetDispatchFeature d) {
        ...
        final Feature noCutsAllowed = sum(subFor, AllowedCutPositionsGenerator.INSTANCE,
        ...
        bindRuleSet(d, "split_cond", add(// do not split over formulas containing auxiliary
                ...
                ifZero(applyTF(FocusProjection.INSTANCE,
                        ContainsExecutableCodeTermFeature.PROGRAMS), longConst(-100),
                        longConst(25))));
        ...
        bindRuleSet(d, "cut_direct", SumFeature.createSum(new Feature[] {
                not(TopLevelFindFeature.ANTEC_OR_SUCC_WITH_UPDATE),
                AllowedCutPositionFeature.INSTANCE, ifZero(
                        NotBelowQuantifierFeature.INSTANCE, add(
                                ...
                                ifZero(applyTF(FocusProjection.INSTANCE,
                                        opSub(tf.eq, any(), vf.nullTerm)), longConst(-5),
                                        longConst(0)),
    ...
    private void setupApplyEq(RuleSetDispatchFeature d, IntegerLDT numbers) {
        ...
        // summands)
        bindRuleSet(d, "apply_equations", SumFeature.createSum(new Feature[] {
                ifZero(applyTF(FocusProjection.create(0), tf.intF),
                        add(applyTF(FocusProjection.create(0), tf.monomial),
                                ScaleFeature.createScaled(FindRightishFeature.create(numbers),
                                        5.0))),
                ifZero(MatchedIfFeature.INSTANCE, add(CheckApplyEqFeature.INSTANCE, let(equation,
                        AssumptionProjection.create(0), add(not(applyTF(equation, ff.update)),
                                ...
                                let(left, sub(equation, 0),
                                        let(right, sub(equation, 1), ifZero(applyTF(left, tf.intF),
                                                add(applyTF(left, tf.nonNegOrNonCoeffMonomial),
                                                        ...
                                                        MonomialsSmallerThanFeature.create(right,
                                                                left, numbers)),
    ...
    private void setupFormulaNormalisation(RuleSetDispatchFeature d, IntegerLDT numbers,
        ...
        bindRuleSet(d, "conjNormalForm", ifZero(
        ...
        bindRuleSet(d, "cnf_setComm", add(
                SetsSmallerThanFeature.create(instOf("commRight"), instOf("commLeft"), locSetLDT),
        ...
        final TermBuffer right = new TermBuffer();
        bindRuleSet(
                d, "apply_equations_andOr", add(
                        let(left, instOf("applyEqLeft"),
        ...
        bindRuleSet(d, "distrQuantifier", add(
                or(applyTF(FocusProjection.create(0),
        ...
        bindRuleSet(
                d, "swapQuantifiers", add(
                        applyTF(FocusProjection.create(0),
                                add(ff.quantifiedClauseSet, EliminableQuantifierTF.INSTANCE,
        ...
        bindRuleSet(d, "cnf_orComm",
                SumFeature.createSum(new Feature[] { applyTF("commRight", ff.clause),
        ...
        bindRuleSet(d, "cnf_orAssoc",
                SumFeature.createSum(
                        new Feature[] { applyTF("assoc0", ff.clause), applyTF("assoc1", ff.clause),
                                applyTF("assoc2", ff.literal), longConst(-80) }));

        bindRuleSet(d, "cnf_andComm", SumFeature.createSum(new Feature[] {
                applyTF("commLeft", ff.clause), applyTF("commRight", ff.clauseSet),
                applyTFNonStrict("commResidue", ff.clauseSet),
                ...
                ifZero(add(applyTF("commLeft", not(ff.literal)),
                        applyTF("commRight", rec(ff.andF, not(ff.literal)))), longConst(100),
                        longConst(-60)),
                clausesSmallerThan("commRight", "commLeft", numbers) }));

        bindRuleSet(d, "cnf_andAssoc",
                SumFeature.createSum(new Feature[] { applyTF("assoc0", ff.clauseSet),
                        applyTF("assoc1", ff.clauseSet), applyTF("assoc2", ff.clause),
                        longConst(-10) }));

        bindRuleSet(d, "cnf_dist",
                SumFeature.createSum(new Feature[] { applyTF("distRight0", ff.clauseSet),
        ...
        final Feature pullOutQuantifierAllowed = add(not(NotBelowQuantifierFeature.INSTANCE),
                onlyBelowQuanAndOr, applyTF(FocusProjection.create(0),
                        sub(ff.quantifiedClauseSet, ff.quantifiedClauseSet)));
    ...
    private void setupQuantifierInstantiation(RuleSetDispatchFeature d) {
        if (quantifierInstantiatedEnabled()) {
            ...
            bindRuleSet(d, "gamma", SumFeature.createSum(new Feature[] {
            ...
            bindRuleSet(d, "gamma",
                    add(isInstantiated("t"),
                            ...
                            InstantiationCostScalerFeature
                                    .create(InstantiationCost.create(instOf("t")), longConst(0))));
    ...
    private void setupPolySimp(RuleSetDispatchFeature d, IntegerLDT numbers) {
        ...
        bindRuleSet(d, "polySimp_balance",
                SumFeature.createSum(new Feature[] { applyTF("sepResidue", tf.polynomial),
        ...
        bindRuleSet(d, "polySimp_critPair", ifZero(MatchedIfFeature.INSTANCE, add(
                monSmallerThan("cpLeft1", "cpLeft2", numbers),
    ...
    private void setupPolySimpInstantiationWithoutRetry(RuleSetDispatchFeature d) {
        ...
        bindRuleSet(d, "polySimp_newSym", ifZero(not(isInstantiated("newSymDef")),
                SumFeature.createSum(applyTF("newSymLeft", tf.atom),
                        ...
                        instantiate("newSymDef", MonomialColumnOp.create(instOf("newSymLeftCoeff"),
                                instOf("newSymRight"))))));

        final TermBuffer divisor = new TermBuffer();
        final TermBuffer dividend = new TermBuffer();

        bindRuleSet(d, "polySimp_applyEqPseudo", add(applyTF("aePseudoTargetLeft", tf.monomial),
            ...
            instantiateDivs = add(
                    instantiate("elimGcdLeftDiv",
                            DividePolynomialsProjection.createRoundingUp(gcd,
                                    instOf("elimGcdLeft"))),
                    instantiate("elimGcdRightDiv", DividePolynomialsProjection.createRoundingUp(gcd,
                            instOf("elimGcdRight"))));
        } else {
            instantiateDivs = add(
                    instantiate("elimGcdLeftDiv",
                            DividePolynomialsProjection.createRoundingDown(gcd,
                                    instOf("elimGcdLeft"))),
                    instantiate("elimGcdRightDiv", DividePolynomialsProjection
                            .createRoundingDown(gcd, instOf("elimGcdRight"))));
        }
        bindRuleSet(d, ruleSet, add(applyTF("elimGcdLeft", tf.nonNegMonomial),
    ...
    private void setupInEqSimp(RuleSetDispatchFeature d, IntegerLDT numbers) {
        ...
        bindRuleSet(d, "inEqSimp_balance", add(applyTF("sepResidue", tf.polynomial),
                ifZero(isInstantiated("sepPosMono"),
                        add(applyTF("sepPosMono", tf.nonNegMonomial),
                                monSmallerThan("sepResidue", "sepPosMono", numbers))),
        ...
        bindRuleSet(d, "inEqSimp_exactShadow",
                SumFeature.createSum(new Feature[] { applyTF("esLeft", tf.nonCoeffMonomial),
                        ...
                        ifZero(MatchedIfFeature.INSTANCE,
                                SumFeature.createSum(applyTFNonStrict("esCoeff1", tf.nonNegLiteral),
                                        ...
                                        not(PolynomialValuesCmpFeature.leq(instOf("esRight2"),
                                                instOf("esRight1"), instOfNonStrict("esCoeff1"),
                                                instOfNonStrict("esCoeff2"))))) }));

        // category "propagation"

        bindRuleSet(d, "inEqSimp_contradInEqs", add(applyTF("contradLeft", tf.monomial), ifZero(
        ...
        bindRuleSet(d, "inEqSimp_contradEqs", add(applyTF("contradLeft", tf.monomial), ifZero(
                MatchedIfFeature.INSTANCE,
        ...
        bindRuleSet(d, "inEqSimp_subsumption", add(applyTF("subsumLeft", tf.monomial),
                ifZero(MatchedIfFeature.INSTANCE, SumFeature.createSum(
                        DiffFindAndIfFeature.INSTANCE, applyTF("subsumRightSmaller", tf.polynomial),
        ...
        bindRuleSet(d, "inEqSimp_and_contradInEqs",
                SumFeature.createSum(new Feature[] { applyTF("contradLeft", tf.monomial),
                        applyTF("contradRightSmaller", tf.polynomial),
                        applyTF("contradRightBigger", tf.polynomial), PolynomialValuesCmpFeature.lt(
                                instOf("contradRightSmaller"), instOf("contradRightBigger")) }));

        bindRuleSet(d, "inEqSimp_andOr_subsumption",
                SumFeature.createSum(new Feature[] { applyTF("subsumLeft", tf.monomial),
        ...
        bindRuleSet(d, "inEqSimp_or_tautInEqs",
                SumFeature.createSum(new Feature[] { applyTF("tautLeft", tf.monomial),
        ...
        bindRuleSet(d, "inEqSimp_or_weaken",
                SumFeature.createSum(new Feature[] { applyTF("weakenLeft", tf.monomial),
        ...
        bindRuleSet(d, "inEqSimp_or_antiSymm",
                SumFeature.createSum(new Feature[] { applyTF("antiSymmLeft", tf.monomial),
    ...
    private void setupMultiplyInequations(RuleSetDispatchFeature d, Feature notAllowedF) {
        ...
        final Feature totallyBounded =
                not(sum(intRel, SequentFormulasGenerator.sequent(),
                        not(add(applyTF(intRel, tf.intRelation),
                                InEquationMultFeature.totallyBounded(instOf("multLeft"),
                                        instOf("multFacLeft"), sub(intRel, 0))))));

        final Feature exactlyBounded =
                not(sum(intRel, SequentFormulasGenerator.sequent(),
                        not(add(applyTF(intRel, tf.intRelation),
                                InEquationMultFeature.exactlyBounded(instOf("multLeft"),
                                        instOf("multFacLeft"), sub(intRel, 0))))));
        ...
        bindRuleSet(d, "inEqSimp_nonLin_multiply",
                add(applyTF("multLeft", tf.nonNegMonomial), applyTF("multRight", tf.polynomial),
                        ifZero(MatchedIfFeature.INSTANCE,
                                SumFeature.createSum(applyTF("multFacLeft", tf.nonNegMonomial),
                                        ...
                                        /*
                                         * ifZero ( applyTF ( "multRight", tf.literal ), longConst (
                                         * -100 ), applyTF ( "multRight", tf.polynomial ) ), ifZero
                                         * ( applyTF ( "multFacRight", tf.literal ), longConst (
                                         * -100 ), applyTF ( "multFacRight", tf.polynomial ) ),
                                         */
    ...
    private void setupInEqSimpInstantiationWithoutRetry(RuleSetDispatchFeature d) {
        ...
        bindRuleSet(d, "inEqSimp_nonLin_divide",
                SumFeature.createSum(new Feature[] { applyTF("divProd", tf.nonCoeffMonomial),
                        ...
                        ifZero(MatchedIfFeature.INSTANCE,
                                let(divisor, instOf("divX"), let(dividend, instOf("divProd"),
                                        SumFeature.createSum(applyTF(divisor, tf.nonCoeffMonomial),
                                                ...
                                                ReducibleMonomialsFeature.createReducible(dividend,
                                                        divisor),
    ...
    private void setupNonLinTermIsPosNeg(RuleSetDispatchFeature d, String ruleSet, boolean pos) {
        ...
        bindRuleSet(d, ruleSet, SumFeature.createSum(new Feature[] {
                ...
                ifZero(MatchedIfFeature.INSTANCE, let(divisor, instOf("divX"), let(dividend,
                        instOf("divProd"),
                        SumFeature.createSum(applyTF(divisor, tf.nonCoeffMonomial),
                                ...
                                let(quotient, ReduceMonomialsProjection.create(dividend, divisor),
                                        add(sum(antecFor, SequentFormulasGenerator.antecedent(),
                                                not(applyTF(antecFor, pos
                                                        ? opSub(tf.geq, eq(quotient), tf.posLiteral)
    ...
    private void setupSquaresAreNonNegative(RuleSetDispatchFeature d) {
        ...
        bindRuleSet(d, "inEqSimp_nonNegSquares", forEach(intRel, SequentFormulasGenerator.sequent(),
                ifZero(applyTF(intRel, tf.intRelation),
                        forEach(product, SubtermGenerator.leftTraverse(sub(intRel, 0), tf.mulF),
                                ifZero(productIsProduct,
                                        let(factor, sub(product, 1), ifZero(productContainsSquare,
    ...
    private Feature allowInEqStrengthening(TermBuffer atom, TermBuffer literal) {
        ...
        return add(not(succIntEquationExists()), not(sum(antecFor,
    ...
    private void setupDefOpsPrimaryCategories(RuleSetDispatchFeature d) {
        ...
        if (arithDefOps()) {
            ...
            bindRuleSet(d, "defOps_div",
                    SumFeature.createSum(new Feature[] { NonDuplicateAppModPositionFeature.INSTANCE,
    ...
    private void setupDefOpsExpandMod(RuleSetDispatchFeature d) {
        ...
        final Feature subsumedModulus = add(
                applyTF(superTerm, sub(opSub(tf.mod, any(), tf.literal), tf.zeroLiteral)),
        ...
        bindRuleSet(d, "defOps_mod",
                ifZero(add(applyTF("divNum", tf.literal), applyTF("divDenom", tf.literal)),
                        longConst(-4000),
                        SumFeature
                                .createSum(applyTF("divNum", tf.polynomial),
                                        applyTF("divDenom", tf.polynomial), ifZero(
                                                isBelow(ff.modalOperator), exSubsumedModulus, or(
                                                        add(applyTF("divNum", tf.notContainsDivMod),
                                                                applyTF("divDenom",
    ...
    private void setupDivModDivision(RuleSetDispatchFeature d) {
        ...
        final Feature isReduciblePoly = sum(numTerm,
                SubtermGenerator.rightTraverse(instOf("divNum"), tf.addF), checkNumTerm);
        ...
        final Feature isReduciblePolyE =
                sum(numTerm, SubtermGenerator.rightTraverse(instOf("divNum"), tf.addF),
                        ifZero(applyTF(numTerm, tf.addF), longConst(0),
                                sum(divCoeff,
                                        MultiplesModEquationsGenerator.create(numTerm, denomLC),
                                        checkCoeffE)));

        bindRuleSet(d, "defOps_divModPullOut", SumFeature.createSum(new Feature[] {
    ...
    private RuleSetDispatchFeature setupApprovalDispatcher() {
        ...
        bindRuleSet(d, "apply_select_eq", add(isInstantiated("s"), isInstantiated("t1"),
    ...
    private void setupClassAxiomApproval(final RuleSetDispatchFeature d) {
        ...
        if (classAxiomApplicationEnabled()) {
            ...
            final Feature approveInst = ifZero(isInstantiated("sv_heap"),
                    /*
                     * the sum expression is 0 if the heap does not occur and infinite if it does
                     * occur, the outer 'not' then ensures that the costs are infinite in the first
                     * and 0 in the latter case
                     */
