...
public final class ApplyEventUpdateOnRigid implements VariableCondition {
	...
	public ApplyEventUpdateOnRigid(UpdateSV u, SchemaVariable x, SchemaVariable x2) {
		this.u = u;
		this.x = x;
		this.x2 = x2;
	}
	...
	private static Term applyUpdateOnRigid(Term update, Term target, TermServices services) {
		...
		Term result = services.getTermFactory().createTerm(target.op(), updatedSubs, target.boundVars(),
				target.javaBlock());
		return result;
	}
	...
	public MatchConditions check(SchemaVariable var, SVSubstitute instCandidate, MatchConditions mc,
		...
		Term uInst = null;
		if(svInst.getInstantiation(u) instanceof EventUpdate) {
	...
	public String toString() {
		return "\\applyEventUpdateOnRigid(" + u + ", " + x + ", " + x2 + ")";
	}
}