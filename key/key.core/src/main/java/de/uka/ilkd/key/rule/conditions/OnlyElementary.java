...
public class OnlyElementary extends VariableConditionAdapter {

	private final SchemaVariable updateSV;

	public OnlyElementary(SchemaVariable var) {
		this.updateSV = var;
	}
	...
	private boolean checkForEvent(Term update) {
		...
		if(op == EventUpdate.SINGLETON || op == UpdateJunctor.SKIP || op == InverseEventUpdate.SINGLETON|| op == AnonEventUpdate.SINGLETON|| op == InverseAnonEventUpdate.SINGLETON) {
			return false;
	...
	public String toString() {
		return "\\onlyElementary("+updateSV.name()+")";
	}
