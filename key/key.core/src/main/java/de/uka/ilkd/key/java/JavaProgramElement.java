...
public abstract class JavaProgramElement extends JavaSourceElement implements ProgramElement {
    ...
    public JavaProgramElement() {
        this((List<Comment>) null);
    }

    public JavaProgramElement(@Nullable List<Comment> comments) {
        this(null, comments);
    }
     ...
     */
    private static List<Comment> extractComments(ExtList list) {
        var c = list.collectList(Comment.class);
    ...
    public void prettyPrint(PrettyPrinter w) throws IOException {
        int s = (comments != null) ? comments.size() : 0;
        int t = 0;
        for (int i = 0; i < s; i += 1) {
