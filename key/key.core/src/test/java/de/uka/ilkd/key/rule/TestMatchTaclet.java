...
public class TestMatchTaclet {
    ...
    public void testVarOccursInIfAndAddRule() {
        ...
        assertEquals(0, NoPosTacletApp.createNoPosTacletApp(if_addrule_conflict)
                .findIfFormulaInstantiations(seq, services).size(), "An area conflict should happen because there is a free" +
    ...
    public void testRWVarOccursInAddAndIf() {
        ...
        assertNotNull(if_add_no_clash.getMatcher().matchFind(match.sub(0),
                MatchConditions.EMPTY_MATCHCONDITIONS, services), "Match not found but should exist" +
    ...
    public void testXNotFreeInYConflict() {
        ...
        assertNull(NoPosTacletApp.createNoPosTacletApp
                (not_free_conflict,
                        not_free_conflict.getMatcher().matchFind
                                (free_in,
                                        MatchConditions.EMPTY_MATCHCONDITIONS, services), services), "Match should not be found because of conflict with " +
                "..not free in..");

        Term not_free_in = TacletForTests.parseTerm("\\forall testSort z; (p(z) & p(c))");
        assertNotNull(NoPosTacletApp.createNoPosTacletApp
                (not_free_conflict,
                        not_free_conflict.getMatcher().matchFind
                                (not_free_in,
                                        MatchConditions.EMPTY_MATCHCONDITIONS, services), services), "Match should be found because .. not free in.. " +
                "is not relevant");
    }
    ...
    public void testCloseWithBoundRenaming() {
        ...
        assertTrue(tacletApp.findIfFormulaInstantiations(seq, services).size() > 0, "Match should be possible(modulo renaming)");
    }
    ...
    public void testConflict() {
        ...
        for (int i = 0; i < conflict.length; i++) {
            assertNull(conflict[i].getMatcher().matchFind
                    (match,
                            MatchConditions.EMPTY_MATCHCONDITIONS, services), "Match should not be found because of area conflict:" + i);
        }
    }
    ...
    public void testUpdateMatch() {
        ...
        match = match.sub(0);
        assertNotNull(all_left.getMatcher().matchFind(match, MatchConditions.EMPTY_MATCHCONDITIONS, services), "Instantiations should be found as updates can be ignored if " +
        ...
        match2 = match2.sub(0);
        assertNotNull(assign_n.getMatcher().matchFind(match2, MatchConditions.EMPTY_MATCHCONDITIONS, services), "Instantiations should be found.");
    }
    ...
    public void testInsequentStateRestriction() {
        ...
        mc = (restrictedTaclet.getMatcher().matchFind(match,
                MatchConditions.EMPTY_MATCHCONDITIONS,
                services));
        assertNotNull(mc, "Test inSequentState failed: did not match on term with without update prefix");
