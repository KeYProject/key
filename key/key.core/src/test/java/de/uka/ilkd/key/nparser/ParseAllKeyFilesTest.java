...
public class ParseAllKeyFilesTest {
    private static final Logger LOGGER = LoggerFactory.getLogger(ParseAllKeyFilesTest.class);
    ...
    public static void debugLexer(KeYLexer tokens) {
        Token t;
        do {
            t = tokens.nextToken();
            LOGGER.debug("{}",
                    String.format("%02d %20s %d:%-50s\n",
                            tokens.getLine(),
                            tokens.getVocabulary().getSymbolicName(t.getType()),
                            tokens._mode,
                            t.getText().replace("\n", "\\n")));
