...
public class JoinRuleUtils {
   ...
   public static Term translateToFormula(final Services services,
      ...
      try {
         final KeYParserF parser =
               new KeYParserF(ParserMode.TERM, new KeYLexerF(new StringReader(
                     toTranslate), ""), services, services.getNamespaces());
   ...
   public static LocationVariable getFreshLocVariableForPrefix(String prefix,
      ...
      do {
         newName = services.getTermBuilder().newName(prefix);
         result = new LocationVariable(new ProgramElementName(newName), sort);
         services.getNamespaces().variables().add(result);
      }
      while (newName.equals(prefix));
    ...
    * @return A term equal to the input, but with constants substituted by fresh
    *         variables.
    */
      ...
      if (term.op() instanceof Function
         ...
         if (!replMap.containsKey(constant)) {
            LogicVariable freshVariable =
                  getFreshVariableForPrefix(stripIndex(constant.toString()),
    ...
    * @return The first Java block in the given term or the empty block if there
    *         is no non-empty Java block.
    */
   ...
   /**
    * Tries to prove the given formula without splitting and returns whether the
   ...
   /**
    * Tries to simplifies the given {@link Term} in a side proof with splits. If
    * this attempt is successful, i.e. the number of atoms in the simplified
    * formula is lower (and, if requested, also the number of disjunctions), the
    * simplified formula is returned; otherwise, the original formula is
    * returned.
    * <p>
    * 
    ...
    *           If set to true, the method also takes the number of disjunctions
    *           (in addition to the number of atoms) into account when judging
    *           about the complexity of the "simplified" formula.
    * @param timeout
    *           Time in milliseconds after which the side proof is aborted.
   ...
   public static Term trySimplify(final Proof parentProof, final Term term,
      ...
      try {
         ...
         if (countAtoms(simplified) < countAtoms(term)
               && (!countDisjunctions || countDisjunctions(simplified, false) < countDisjunctions(
                     term, false))) {
    ...
    * @param services
    *           The Services object.
    * @return true iff source elements can be matched, considering branch-unique
    *         location names.
    */
    ...
    * conjunction of the first element of the pair and the second element of the
    * pair is equivalent to pathCondition1.
    * 
   ...
   public static Option<Pair<Term, Term>> getDistinguishingFormula(
      ...
      return new Option.Some<Pair<Term, Term>>(
            new Pair<Term, Term>(joinConjuctiveElements(TermWrapper.toTermList(
                  new ArrayList<Term>(), distinguishingElements), services),
   ...
   /**
    * Convenience method for converting a whole list of goal-pio combinations to
    * symbolic execution states; relies on
    * {@link #sequentToSETriple(Node, PosInOccurrence, Services)}.
    *
    * @param sequentInfos
    *           Goals and PosInOccurrences specifying join partners and the
    *           positions of the program counter-post condition formulae in the
    *           goals.
   ...
   public static ImmutableList<SymbolicExecutionState> sequentsToSEPairs(
      ...
      for (Triple<Goal, PosInOccurrence, HashMap<ProgramVariable, ProgramVariable>> sequentInfo : sequentInfos) {
         ...
         Triple<Term, Term, Term> partnerSEState =
               sequentToSETriple(sequentInfo.first.node(), sequentInfo.second,
                     services);

         result =
               result.prepend(new SymbolicExecutionState(partnerSEState.first,
   ...
   /**
    * Anonymizes all program variables occurring in the given term. If x is a PV
    * in the term, the result will be of the form
   ...
   private static Pair<Term, ImmutableSet<QuantifiableVariable>> anonymizeProgramVariables(
      ...
      for (LocationVariable loc : getLocationVariables(term, services)) {
         ...
         elementaries =
               elementaries.prepend(tb.elementary(tb.var(loc), tb.var(newVar)));
      }
   ...
   private static ImmutableSet<LocationVariable> getProgramLocations(
      ...
      CollectLocationVariablesVisitor visitor =
            new CollectLocationVariablesVisitor(programCounterTerm.javaBlock()
   ...
   private static HashSet<LocationVariable> getProgramLocationsHashSet(
      ...
      CollectLocationVariablesVisitorHashSet visitor =
            new CollectLocationVariablesVisitorHashSet(program, true, services);
   ...
   private static ApplyStrategyInfo tryToProve(Term toProve, Services services,
      ...
      // Sequent to prove
            Semisequent.EMPTY_SEMISEQUENT, new Semisequent(new SequentFormula(
                  toProve))), services, doSplit, sideProofName, timeout);
   }
   ...
   public static abstract class Option<T> {
      static class Some<T> extends Option<T> {
         ...
         public Some(T value) {
            assert value != null : "Wrappign null values in a Some is not allowed. Consider a None instead.";
      ...
      /**
       * Returns the value of this object if is a Some; otherwise, an exception
       * is thrown.
       *
