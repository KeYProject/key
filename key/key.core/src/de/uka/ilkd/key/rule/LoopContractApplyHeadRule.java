...
/**
 * <p>
 * This rule transforms a block that starts with a for loop into one that starts
 * with a while loop.
 * </p>
 *
 * <p>
 * This is necessary because the rules for loop contracts (subclasses of
 * {@link AbstractLoopContractRule}) can only be applied on blocks that start
 * with a while loop.
 * </p>
 *
 * <p>
 * The transformation is equivalent to the {@link ForToWhileTransformation}, the
 * only difference being that we transform the whole block containing the loop
 * instead of just the loop itself. This is implemented as a built-in rule
 * because the opening brace of the block on which it is applied belongs to the
 * non-active prefix and thus cannot be matched by the taclet language.
 * </p>
 *
 * <p>
...
public class LoopContractApplyHeadRule implements BuiltInRule {
    ...
    public ImmutableList<Goal> apply(Goal goal, Services services,
        ...
        StatementBlock block = new StatementBlock(someContract.getLoop(),
                someContract.getTail());
        StatementBlock headAndBlock = new StatementBlock(someContract.getHead(),
                block);
        ...
        JavaBlock newJavaBlock = JavaBlock
                .createJavaBlock((StatementBlock) new ProgramElementReplacer(
        ...
        for (LoopContract c : contracts) {
            ...
            services.getSpecificationRepository()
                    .addLoopContract(c.setBlock(block), false);
        }

        Goal result = goal.split(1).head();
        result.changeFormula(
                new SequentFormula(tb.apply(update,
                        tb.prog(modality, newJavaBlock, target.sub(0)))),
                ruleApp.pio);
    ...
    public boolean isApplicable(Goal goal, PosInOccurrence pio) {
        ...
        final ImmutableSet<LoopContract> contracts = AbstractLoopContractRule
                .getApplicableContracts(instantiation, goal,
