...
public final class LoopContractInternalRule extends AbstractLoopContractRule {
     ...
     */
    private static Term[] createPreconditions(final Term selfTerm,
            final LoopContract contract, final List<LocationVariable> heaps,
        ...
        final Term selfConditions = conditionsAndClausesBuilder
                .buildSelfConditions(heaps, contract.getMethod(),
                        contract.getKJT(), selfTerm, services);
    ...
    private static Term[] createUsageAssumptions(final Term[] postconditions,
        ...
        return new Term[] { postconditions[0],
                wellFormedAnonymisationHeapsCondition, reachableOutCondition,
    ...
    private static Term[] createUpdates(final Instantiation instantiation,
        ...
        final Term anonymisationUpdate = updatesBuilder
                .buildAnonOutUpdate(anonOutHeaps, modifiesClauses);
        return new Term[] { contextUpdate, remembranceUpdate,
                anonymisationUpdate };
    }
    ...
    private static LoopContract.Variables[] createVars(final Goal goal,
            final Term selfTerm, final LoopContract contract,
            final Services services) {
     ...
     *            the heaps used in the anonOut update.
     * @param vars
     *            the variables for both the current and the next loop
     *            iteration.
     * @param modifiesClauses
     *            the modifies clauses.
     * @param assumptions
    ...
    private static void setUpGoals(final Goal goal,
            final ImmutableList<Goal> result, final LoopContract contract,
            final Instantiation instantiation,
            final Map<LocationVariable, Function> anonOutHeaps,
            final LoopContract.Variables[] vars,
            final Map<LocationVariable, Term> modifiesClauses,
            final Term[] assumptions, final Term[] usageAssumptions,
            final Term decreasesCheck, final Term[] postconditions,
            final Term[] postconditionsNext, final Term[] updates,
            final Term nextRemembranceUpdate, final Term context,
            final GoalsConfigurator configurator, final Services services) {
        configurator.setUpPreconditionGoal(result.tail().head(), updates[0],
                assumptions);
        configurator.setUpUsageGoal(result.head(), updates, usageAssumptions);
        final ProgramVariable exceptionParameter = createLocalVariable("e",
                vars[0].exception.getKeYJavaType(), services);
        configurator.setUpLoopValidityGoal(goal, contract, context, updates[1],
                ...
                exceptionParameter, vars[0].termify(instantiation.self),
                vars[1]);
    }
    ...
    @Override
    public LoopContractInternalBuiltInRuleApp createApp(
    ...
    public ImmutableList<Goal> apply(final Goal goal, final Services services,
        ...
        LoopContractInternalBuiltInRuleApp application = (LoopContractInternalBuiltInRuleApp) ruleApp;
        ...
        final Map<LocationVariable, Function> anonOutHeaps = createAndRegisterAnonymisationVariables(
                heaps, contract, services);
        final LoopContract.Variables[] vars = createVars(goal,
                instantiation.self, contract, services);

        final ConditionsAndClausesBuilder conditionsAndClausesBuilder = new ConditionsAndClausesBuilder(
                contract, heaps, vars[0], instantiation.self, services);
        final Map<LocationVariable, Term> modifiesClauses = conditionsAndClausesBuilder
                .buildModifiesClauses();
        final Term[] assumptions = createPreconditions(instantiation.self,
                contract, heaps, localInVariables, conditionsAndClausesBuilder,
                services);
        final Term[] postconditions = createPostconditions(modifiesClauses,
                conditionsAndClausesBuilder);
        final Term[] usageAssumptions = createUsageAssumptions(postconditions,
                anonOutHeaps, localOutVariables, conditionsAndClausesBuilder);
        final Term decreasesCheck = conditionsAndClausesBuilder
                .buildDecreasesCheck();
        final Term[] postconditionsNext = createPostconditionsNext(
                instantiation.self, contract, heaps, vars[1], modifiesClauses,
                services);
        final UpdatesBuilder updatesBuilder = new UpdatesBuilder(vars[0],
                services);
        final Term[] updates = createUpdates(instantiation, heaps, anonOutHeaps,
                modifiesClauses, updatesBuilder);
        final Term nextRemembranceUpdate = new UpdatesBuilder(vars[1], services)
                .buildRemembranceUpdate(heaps);
        final Term context = createContext(heaps, updatesBuilder, services);
        final GoalsConfigurator configurator = new GoalsConfigurator(
                application, new TermLabelState(), instantiation,
                contract.getLabels(), vars[0], application.posInOccurrence(),
                services, this);
