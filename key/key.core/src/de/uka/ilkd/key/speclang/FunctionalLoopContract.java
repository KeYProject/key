...
/**
 * This class is only used to generate a proof obligation for a block that
 * starts with a loop (see {@link FunctionalLoopContractPO}.
 *
 * If a block is encountered during a proof, {@link LoopContract} is used
 * instead.
 * 
 * @author lanzinger
 */
public class FunctionalLoopContract implements Contract {
    ...
    FunctionalLoopContract(LoopContract contract, int id) {
        ...
        name = generateName(contract.getBaseName(),
                str -> ContractFactory.generateContractName(str, getKJT(),
                        getTarget(), getKJT(), id));
        displayName = generateName(contract.getBaseName(),
                str -> ContractFactory.generateDisplayName(str, getKJT(),
                        getTarget(), getKJT(), id));
        typeName = generateName(contract.getBaseName(),
                str -> ContractFactory.generateContractTypeName(str, getKJT(),
                        getTarget(), getKJT()));
    }
    ...
    private String generateName(String baseName,
            UnaryOperator<String> generator) {
        return Arrays
                .stream(baseName.split(
                        SpecificationRepository.CONTRACT_COMBINATION_MARKER))
                .map(generator)
                .reduce((acc, curr) -> acc
                        + SpecificationRepository.CONTRACT_COMBINATION_MARKER
                        + curr)
    ...
    public Term getPre(List<LocationVariable> heapContext,
        ...
        for (LocationVariable heap : heapContext) {
            final Term p = getPre(heap, selfVar, paramVars, atPreVars,
                    services);
    ...
    public Term getPre(LocationVariable heap, Term heapTerm, Term selfTerm,
            ImmutableList<Term> paramTerms, Map<LocationVariable, Term> atPres,
            Services services) {
        return contract.getPrecondition(heap, heapTerm, selfTerm, atPres,
                services);
    }
    ...
    public Term getPre(List<LocationVariable> heapContext,
        ...
        for (LocationVariable heap : heapContext) {
            final Term p = getPre(heap, heapTerms.get(heap), selfTerm,
    ...
    public ProofOblInput createProofObl(InitConfig initConfig,
        ...
        return new FunctionalLoopContractPO(initConfig,
    ...
    public Contract setTarget(KeYJavaType newKJT, IObserverFunction newPM) {
        return new FunctionalLoopContract(contract.setTarget(newKJT, newPM),
                id);
    }
