...
public abstract class TacletBuilder {
   ...
   protected ImmutableList<NewVarcond> varsNew = ImmutableSLList
         .<NewVarcond> nil();
   protected ImmutableList<NotFreeIn> varsNotFreeIn = ImmutableSLList
         .<NotFreeIn> nil();
   ...
   /**
    * List of additional generic conditions on the instantiations of schema
    * variables.
    */
   protected ImmutableList<VariableCondition> variableConditions = ImmutableSLList
         .<VariableCondition> nil();
   ...
   static void checkContainsFreeVarSV(Sequent seq, Name tacletName, String str) {
      if (containsFreeVarSV(seq)) {
         throw new TacletBuilderException(tacletName, "Free Variable in " + str
   ...
   static void checkContainsFreeVarSV(Term t, Name tacletName, String str) {
      if (containsFreeVarSV(t)) {
         throw new TacletBuilderException(tacletName, "Free Variable found in "
   ...
   /**
    * adds a new <I>new</I> variable to the variable conditions of the Taclet: v
    * is new and has the same type as peerSV
    */
   ...
   public void addVarsNew(NewVarcond nv) {
      if (!(nv.getSchemaVariable() instanceof ProgramSV)) {
         throw new TacletBuilderException(this, "Tried to add condition:" + nv
               + "to new vars-list. That can" + "match more than program"
               + " variables.");
      }
   ...
   /**
    * adds a list of <I>new</I> variables to the variable conditions of the
    * Taclet: v is new for all v's in the given list
    */
   ...
   /**
    * adds a list of <I>NotFreeIn</I> variable pairs to the variable conditions
    * of the Taclet: v0 is not free in v1 for all entries (v0,v1) in the given
    * list.
    */
   ...
   /**
    * Add a "v0 depending on v1"-statement. "v0" may not occur within the if
    * sequent or the find formula/term, however, this is not checked
    */
   public void addVarsNewDependingOn(SchemaVariable v0, SchemaVariable v1) {
      varsNewDependingOn = varsNewDependingOn
            .prepend(new NewDependingOn(v0, v1));
   }

   /**
    * Add an additional generic condition on the instantiation of schema
    * variables.
    */
   ...
   /**
    * adds a new goal descriptions to the goal descriptions of the Taclet. The
    * TacletGoalTemplate must be of the appropriate kind (Rewrite/Ante/Succ),
   ...
   /**
    * builds and returns the Taclet that is specified by former set... / add...
    * methods. If no name is specified then an Taclet with an empty string name
    * is build. No specifications for variable conditions, goals or rule sets
    * imply that the corresponding parts of the Taclet are empty. No
    * specification for the if-sequence is represented as a sequent with two
    * empty semisequences. No specification for the interactive or recursive
    * flags imply that the flags are not set. No specified find part for Taclets
    * that require a find part causes an IllegalStateException.
    */
   ...
   static class TacletBuilderException extends IllegalArgumentException {
      ...
      public String getMessage() {
         String message = (tacletname == null ? "(unknown taclet name)"
               : tacletname.toString());
