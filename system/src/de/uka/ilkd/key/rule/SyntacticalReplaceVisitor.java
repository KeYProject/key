...
public final class SyntacticalReplaceVisitor extends DefaultVisitor {
    ...
    public void visit(Term visited) {
        ...
        if (visitedOp instanceof SchemaVariable
            ...
            pushNew(TermBuilder.DF.label(newTerm, instantiateLabels(visited, newTerm.op(), newTerm.subs(), newTerm.boundVars(), newTerm.javaBlock(), "visit")));
            ...
            if (visitedOp instanceof ElementaryUpdate
                ...
                ImmutableArray<TermLabel> labels = instantiateLabels(visited, newTerm.op(), newTerm.subs(), newTerm.boundVars(), newTerm.javaBlock(), "elementary");
                if (labels.size() != 0) {
               ...
               ImmutableArray<TermLabel> labels = instantiateLabels(visited, newOp, new ImmutableArray<Term>(neededsubs), boundVars, jb, "boundVars");
               Term newTerm = tf.createTerm(newOp, neededsubs, boundVars, jb, labels);
                pushNew(resolveSubst(newTerm));
            } else {
                Term t;
                ImmutableArray<TermLabel> labels = instantiateLabels(visited, visited.op(), visited.subs(), visited.boundVars(), visited.javaBlock(), "else");
    ...
    private ImmutableArray<TermLabel> instantiateLabels(Term tacletTerm, 
                                                        ...
                                                        Object hint) {
       return TermLabelManager.instantiateLabels(services, applicationPosInOccurrence, rule, null, hint, tacletTerm, newTermOp, newTermSubs, newTermBoundVars, newTermJavaBlock);
    }
...
// 	    t=substVisit.getTerm();
	...
	if (subtreeRoot.op() instanceof TermTransformer) {
	    ...
	    pushNew(TermBuilder.DF.label(newTerm, instantiateLabels(subtreeRoot, newTerm.op(), newTerm.subs(), newTerm.boundVars(), newTerm.javaBlock(), "subtreeLeft")));
	} 
   }
}
