...
//    }
    ...
    public Term singleton2(Services services, Term o) {
	return func(services.getTypeConverter().getSetLDT().getSingleton(), o);
    }
    
    
    public Term union2(Services services, Term s1, Term s2) {
    ...
    public Term union2(Services services, Term ... subTerms) {
        Term result = empty(services);
        for(Term sub : subTerms) {
	    result = union2(services, result, sub);
	}
        return result;
    }
    
    
    public Term union2(Services services, Iterable<Term> subTerms) {
        Term result = empty2(services);
        for(Term sub : subTerms) {
	    result = union2(services, result, sub);
	}
        return result;
    ...
    public Term intersect2(Services services, Term s1, Term s2) {
	final SetLDT ldt = services.getTypeConverter().getSetLDT();
	if(s1.op() == ldt.getEmpty() || s2.op() == ldt.getEmpty()) {
	    return empty2(services);
	} else {
	    return func(ldt.getIntersect(), s1, s2);
	}
    }
    ...
    public Term intersect2(Services services, Term ... subTerms) {
        Term result = empty2(services);
        for(Term sub : subTerms) {
	    result = intersect2(services, result, sub);
	}
        return result;
    }
    
    
    public Term intersect2(Services services, Iterable<Term> subTerms) {
        Term result = empty2(services);
        for(Term sub : subTerms) {
	    result = intersect2(services, result, sub);
	}
    ...
    public Term setMinus2(Services services, Term s1, Term s2) {
	final SetLDT ldt = services.getTypeConverter().getSetLDT();
	if(s1.op() == ldt.getEmpty() || s2.op() == ldt.getEmpty()) {
	    return s1;
	} else {
    ...
    public Term infiniteUnion2(Services services, 
	                      QuantifiableVariable[] qvs, 
	                      Term s) {
	final SetLDT ldt = services.getTypeConverter().getSetLDT();
	return tf.createTerm(ldt.getInfiniteUnion(), 
    ...
    public Term guardedInfiniteUnion2(Services services, 
	    			     QuantifiableVariable[] qvs, 
	    			     Term guard, 
	    			     Term s) {
	return infiniteUnion2(services, 
		             qvs, 
		             ife(guard, s, empty2(services)));
    }
    
    
    public Term setComprehension2(Services services, 
	                         QuantifiableVariable[] qvs, 
	                         Term o) {
	return infiniteUnion2(services, qvs, singleton2(services, o));
    }
    
    
    public Term guardedSetComprehension2(Services services, 
	                         	QuantifiableVariable[] qvs,
	                         	Term guard,
	                         	Term o) {
	return guardedInfiniteUnion2(services, 
		                    qvs, 
		                    guard, 
		                    singleton2(services, o));
    }
     
    
    
    public Term elementOf2(Services services, Term o, Term s) {
	final SetLDT ldt = services.getTypeConverter().getSetLDT();
	if(s.op() == ldt.getEmpty()) {
	    return ff();
	} else {
	    return func(ldt.getElementOf(), new Term[]{o, s});
	}
    }
    
    
    public Term subset2(Services services, Term s1, Term s2) {
	final SetLDT ldt = services.getTypeConverter().getSetLDT();
	if(s1.op() == ldt.getEmpty()) {
	    return tt();
	} else {
    ...
    public Term disjoint2(Services services, Term s1, Term s2) {
	final SetLDT ldt = services.getTypeConverter().getSetLDT();
	if(s1.op() == ldt.getEmpty() || s2.op() == ldt.getEmpty()) {
	    return tt();
	} else {
