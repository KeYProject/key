...
public class WatchpointUtil {
    ...
    private static boolean satisfiesWatchpoint(HashSet<Node> leafNodesInETNode,
        ...
        for (Node node : leafNodesInETNode) {
            ...
            } System.out.println("wps evaluated to true (after intersection) for current node:" + intersection.size());
        }
     ...
     * EvaluateWatchpoints.
     * 
     * Returns true, if the concatenation of all watchpoints by the junctor
     * can be evaluated to true, i.e. the proof can be closed
     * 
     * Example: 
     ...
     *                        
     * @param watchpoints - a list of all watchpoints that have to be taken into account
     ...
     * getAllLeafNodes
     * 
     * @param nodes - an array containing all (proof)nodes of an ETNode
     ...
     * 
     * @param nodes -
     *                a list of ETNodes of the current ET.
     *                The watchpoints will be evaluated for all these nodes.
     */
    public static void setActiveWatchpoint(List<ETNode> nodes,
        ...
        for (ETNode node : nodes) {
            node.setWatchpoint(satisfiesWatchpoint(getLeafNodesInETNode(node
                    .getProofTreeNodes().toArray()), watchpoints, node));
        }
    }
    /**
