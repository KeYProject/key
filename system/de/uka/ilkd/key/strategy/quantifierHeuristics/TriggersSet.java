...
class TriggersSet {
    ...
    private SetOfQuantifiableVariable getAllUQS(Term allterm) {
        ...
        if ( op == Op.EX ) return getAllUQS ( allterm.sub ( 0 ) );
	...
	private class ClauseTrigger {
		...
		public ClauseTrigger(Term clause){
			this.clause = clause;
			selfUQVS =TriggerUtils
			          .intersect(uniQuantifiedVariables,clause.freeVars());
		...
		public void createTriggers() {
            ...
            while ( it.hasNext () ) {
                ...
                final IteratorOfTerm it2 = expandIfThenElse ( oriTerm ).iterator ();
                while ( it2.hasNext () ) {
                    Term t = it2.next ();
                    if ( t.op () == Op.NOT ) t = t.sub ( 0 );
                    recAddTriggers ( t );
                }
            }
		...
		private boolean recAddTriggers(Term term) {
            if ( !mightContainTriggers ( term ) ) return false;
        ...
        private SetOfTerm expandIfThenElse(Term t) {
            final SetOfTerm[] possibleSubs = new SetOfTerm [t.arity ()];
            boolean changed = false;
            for ( int i = 0; i != t.arity (); ++i ) {
                final Term oriSub = t.sub ( i );
                possibleSubs[i] = expandIfThenElse ( oriSub );
                changed = changed
                          || possibleSubs[i].size () != 1
                          || possibleSubs[i].iterator ().next () != oriSub;
            }
            
            SetOfTerm res = SetAsListOfTerm.EMPTY_SET;
            if ( t.op () == Op.IF_THEN_ELSE )
                res = possibleSubs[1].union ( possibleSubs[2] );
            
            if ( !changed ) return res.add ( t );
            ...
            return res.union ( combineSubterms ( t, possibleSubs, chosenSubs,
                                                 boundVars, 0 ) );
        }
        
        private SetOfTerm combineSubterms(Term oriTerm,
                                          SetOfTerm[] possibleSubs,
                                          Term[] chosenSubs,
            ...
            if ( i >= possibleSubs.length ) {
                final Term res =
                    TermFactory.DEFAULT.createTerm ( oriTerm.op (),
                                                     ...
                                                     boundVars,
                                                     oriTerm.javaBlock () );
                return SetAsListOfTerm.EMPTY_SET.add ( res );
            }
            
            SetOfTerm res = SetAsListOfTerm.EMPTY_SET;
            final IteratorOfTerm it = possibleSubs[i].iterator ();
            while ( it.hasNext () ) {
                chosenSubs[i] = it.next ();
                res = res.union ( combineSubterms ( oriTerm, possibleSubs,
		...
		private boolean mightContainTriggers(Term term) {
            if ( term.freeVars ().size () == 0 ) return false;
            final Operator op = term.op ();
            if ( op instanceof Modality
                 || op instanceof IUpdateOperator
                 || op instanceof QuantifiableVariable ) return false;
            if ( !UniTrigger.passedLoopTest ( term, allTerm ) ) return false;
            return true;
        }
        ...
        private boolean isAcceptableTrigger(Term term) {
            ...
            if ( op == Op.EQUALS
                 || "leq".equals ( op.name ().toString () )
                 || "geq".equals ( op.name ().toString () ) )
                return false;
		...
		private void addUniTrigger(Term term) {
            if ( !isAcceptableTrigger ( term ) ) return;
        ...
        private boolean addMultiTrigger(Term term) {
            if ( !isAcceptableTrigger ( term ) ) return false;
            final boolean isUnify = !term.freeVars ().subset ( selfUQVS );System.out.println(term);
            final boolean isElement = !selfUQVS.subset ( term.freeVars () );
            final SetOfQuantifiableVariable uniVarsInTerm =
                TriggerUtils.intersect ( term.freeVars (), selfUQVS );
            Trigger t = createUniTrigger ( term, uniVarsInTerm, isUnify, isElement );
		...
		private Set setMultiTriggers(Trigger[] ts, int i) {
            ...
            while ( it.hasNext () ) {
                ...
                next = next.add ( ts[i] );
                if ( addMultiTrigger ( next ) ) continue;
                res.add ( next );
            }
            return res;
        }
