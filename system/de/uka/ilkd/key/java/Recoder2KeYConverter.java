...
import recoder.list.ExpressionList;
...
public class Recoder2KeYConverter {
	...
	protected ExtList collectChildren(recoder.java.NonTerminalProgramElement pe) {
		...
		CommentMutableList l = pe.getComments();
		if (l != null) {
	...
	public MethodBodyStatement convert(
		...
		if (args != null) {
			...
			for (int i = 0, sz = args.size(); i < sz; i++) {
				keyArgs[i] = (Expression) callConvert(args.getProgramElement(i));
			}
		} else {
		...
		if (pv == null) {
			...
			if (varSpec == null) {
				...
				final ProgramElementName pen = new ProgramElementName(
						recoderVarSpec.getName(), recContainingClassType
								.getFullName());
	...
	public Expression convert(recoder.java.reference.FieldReference fr) {
		...
		if (recoderVarSpec == null) {
			...
			recoder.java.declaration.FieldSpecification fs = new recoder.java.declaration.FieldSpecification(
					fr.getIdentifier());
			pv = new LocationVariable(new ProgramElementName(fs.getName(),
	...
	public ForUpdates convertUpdates(recoder.java.statement.LoopStatement ls) {
		...
		ExpressionList recLoopUpdates = ls.getUpdates();
		inLoopInit = true;
		if (recLoopUpdates != null) {
			for (int i = 0, sz = recLoopUpdates.size(); i < sz; i++) {
				updates.add(callConvert(recLoopUpdates.getExpression(i)));
			}
			inLoopInit = false;
	...
	public LoopInit convertLoopInitializers(
		...
		LoopInitializerMutableList initializers = ls.getInitializers();
	...
	public New convert(recoder.java.expression.operator.New n) {
		...
		ExpressionMutableList args = n.getArguments();
		...
		if (rp == null) {
			return new New(arguments, (TypeReference) callConvert(tr),
	...
	public StatementBlock convert(recoder.java.StatementBlock arg) {
	   return new StatementBlock(collectChildrenAndComments(arg));
	}
