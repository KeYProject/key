...
public class SMTTranslator {
	...
	for (Iterator i = usedLocalMv.iterator(); i.hasNext();) {
	    tmp.append(' ').append(getUniqueVariableName((Operator) i.next()));
	}
	tmp.append(")\n ");
	tmp.append(s);
	tmp.append("\n))");
    ...
    protected final StringBuffer translate(Sequent sequent,
	    boolean lightWeight, Services services) throws SimplifyException {
	//          computeModuloConstraints(sequent);             
	StringBuffer hb = new StringBuffer();
	StringBuffer temp;
	hb.append('(').append(IMPLYSTRING).append(' ');
	temp = translate(sequent.antecedent(), ANTECEDENT, lightWeight,
		services);
	hb.append(temp);
	hb.append("\n");
	temp = translate(sequent.succedent(), SUCCEDENT, lightWeight, services);
	hb.append(temp);
	hb.append(')');
    ...
    protected final StringBuffer translate(Semisequent ss, int skolemization,
	    Services services) throws SimplifyException {
     ...
     */
    public final StringBuffer translate(Term term, Vector quantifiedVars,
	    Services services) throws SimplifyException {
	Operator op = term.op();
	if (op == Op.NOT) {
	    ...
	    return (translateSimpleTerm(term, EQSTRING, quantifiedVars,
		    services));
	    ...
	    hb.append('(').append(op == Op.ALL ? ALLSTRING : EXISTSTRING)
		    .append(" (");
	    ...
	    hb
		    .append(" "
			    + getSortName(vars.getQuantifiableVariable(0)
				    .sort()));
	    ...
	    return (translateSimpleTerm(term, funcName, quantifiedVars,
		    services));
	} else if (op instanceof Function) {
	    String name = op.name().toString().intern();
	    //                        if (name.equals("add")) {
	    if (op == services.getTypeConverter().getIntegerLDT().getAdd()) {
		return (translateSimpleTerm(term, PLUSSTRING, quantifiedVars,
			services));
		//} else if (name.equals("sub")) {
	    } else if (op == services.getTypeConverter().getIntegerLDT()
		    .getSub()) {
		return (translateSimpleTerm(term, MINUSSTRING, quantifiedVars,
			services));
		//} else if (name.equals("neg")) {
	    } else if (op == services.getTypeConverter().getIntegerLDT()
		    .getNeg()) {
		//%%: This is not really hygienic
		return (translateUnaryMinus(term, MINUSSTRING, quantifiedVars,
			services));
		//} else if (name.equals("mul")) {
	    } else if (op == services.getTypeConverter().getIntegerLDT()
		    .getMul()) {
		return (translateSimpleTerm(term, MULTSTRING, quantifiedVars,
			services));
		...
		//} else if (name.equals("lt")) {
		    ...
		    .getLessThan()) {
		return (translateSimpleTerm(term, LTSTRING, quantifiedVars,
			services));
		//} else if (name.equals("gt")) {
	    } else if (op == services.getTypeConverter().getIntegerLDT()
		    .getGreaterThan()) {
		return (translateSimpleTerm(term, GTSTRING, quantifiedVars,
			services));
		//} else if (name.equals("leq")) {
	    } else if (op == services.getTypeConverter().getIntegerLDT()
		    .getLessOrEquals()) {
		return (translateSimpleTerm(term, LEQSTRING, quantifiedVars,
			services));
		//} else if (name.equals("geq")) {
	    } else if (op == services.getTypeConverter().getIntegerLDT()
		    .getGreaterOrEquals()) {
		return (translateSimpleTerm(term, GEQSTRING, quantifiedVars,
			services));
     ...
     *           terms, but must be looped through until we get there.
     */
    protected final StringBuffer translateSimpleTerm(Term term, String name,
	    Vector quantifiedVars, Services services) throws SimplifyException {
	StringBuffer hb = new StringBuffer();
     ...
     *           terms, but must be looped through until we get there.
     */
    protected final StringBuffer translateEquiv(Term term,
	    Vector quantifiedVars, Services services) throws SimplifyException {
     ...
     *           terms, but must be looped through until we get there.
     */
    protected final StringBuffer translateAttributeOpTerm(Term term,
	    Vector quantifiedVars, Services services) throws SimplifyException {
	StringBuffer hb = new StringBuffer();
     ...
     *           terms, but must be looped through until we get there.
     */
    protected final StringBuffer translateUnaryMinus(Term term, String name,
	    Vector quantifiedVars, Services services) throws SimplifyException {
	StringBuffer hb = new StringBuffer();
