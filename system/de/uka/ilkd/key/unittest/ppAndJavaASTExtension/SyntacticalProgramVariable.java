 ...
 * {@link de.uka.ilkd.key.logic.op.ProgramVariable} but it does not support
...
public class SyntacticalProgramVariable extends TermSymbol implements
    ...
    /**
     * In contrast to ProgramVariable this field is not of type KeYJavaType but
     * only a type Type
     */
    ...
    /**
     * represents the type where this program variable is declared if and only
     * if the program variable denotes a field. In contrast to ProgramVariable
     * this field is not of type KeYJavaType but only a type Type.
     */
    private final Type containingType;

    protected SyntacticalProgramVariable(final ProgramElementName name,
	    final Sort s, final Type t, final Type containingType,
	    final boolean isStatic, final boolean isModel,
	    final boolean isGhost, final boolean isFinal) {
	super(name, s);
	this.type = t;
    ...
    protected SyntacticalProgramVariable(final ProgramElementName name,
	    final Sort s, final Type t, final Type containingType,
	    final boolean isStatic, final boolean isModel, final boolean isGhost) {
	this(name, s, t, containingType, isStatic, isModel, isGhost, false);
    }

    // chris von locationvariable
    public SyntacticalProgramVariable(final ProgramElementName name,
	    final Type t) {
     ...
     *            the Visitor
     */
    public void visit(final de.uka.ilkd.key.java.visitor.Visitor v) {
     ...
     */
    public void prettyPrint(final PrettyPrinter w) throws IOException {
	if (!(w instanceof CompilableJavaCardPP)) {
	    final CompilableJavaCardPP cjpp = new CompilableJavaCardPP(
		    new StringWriter(), false);
	    prettyPrint(cjpp);
	    cjpp.emergencyPrint(w);
	} else {
	    final CompilableJavaCardPP cjpp = (CompilableJavaCardPP) w;
     ...
     * should never be called because SyntacticalProgramVariable is supposed to
     * be independent from KeYJavaTypes. This method is declared to overwritte
     * the respective method from the parent class.
     */
    public KeYJavaType getKeYJavaType() {
	throw new RuntimeException(
	        "SyntactialProgramVariable.getKeYJavaType should never be called.\n"
	                + " If you *effectively* want to call getKeYJavaType().getJavaType() then\n"
	                + " just use the type attribute of this class instead.");
	// return null;
    }

    public KeYJavaType getKeYJavaType(final Services javaServ) {
    ...
    public ReferencePrefix setReferencePrefix(final ReferencePrefix r) {
	return this;
    }
     ...
     */
    public boolean equalsModRenaming(final SourceElement se,
	    final NameAbstractionTable nat) {
	return nat.sameAbstractName(this, se);
    }

    @SuppressWarnings("unchecked")
    public Expression convertToProgram(final Term t, final ExtList l) {
	if (isStatic()) {
	    final ExtList l2 = new ExtList();
	    l2.add(this);
     ...
     * .op.Location)
     */
    public boolean mayBeAliasedBy(final Location loc) {
	return loc instanceof SortedSchemaVariable || loc == this;
    }

    public MatchConditions match(final SourceData source,
