...
public final class NumberTranslation {
    ...
    /**
     * This methods translates a term with sort "numbers" into a BigInteger
	...
	if (!term.sort().name().toString().trim().equals("numbers")) {
	    throw new IllegalArgumentException(
		    "Only terms with sort \"numbers\" may be translated.\n"
			    + "Term " + term + " is  of sort "
			    + term.sort().name().toString().trim());
	}
	Operator op = term.op();
	...
	if (name.length() == 1) {
		...
		return translate(term.sub(0)).multiply(smallInts[10]).add(
			smallInts[ch - '0']);
	    } else if (ch == '#') {
	    ...
	    Debug.assertTrue(term.arity() == 1);
	    /*
	     * Hint: translate operator "neg" at all places correctly, e.g.
	     * neg(1(neg(1(#)))).
	     */
