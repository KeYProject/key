...
/**
 * Extracts the code, for which we want to create a unittest, from a formula
 * of the form: &lt updates&gt \\&lt code\\&gt post.
 */
...
//    private String packageName = null;
	...
	for(int i=0; i<succ.size(); i++){
	    Term current = succ.get(i).formula();
	...
	while(it.hasNext()){
		...
		(LocalVariableDeclaration) it.next();
	    ArrayOfVariableSpecification vars = lvd.getVariables();
	    for(int i=0; i<vars.size(); i++){
		IProgramVariable pv = 
		    vars.getVariableSpecification(i).getProgramVariable();
		if(pvn.lookup(pv.name())==pv){
    ...
    private HashSet getStatements(StatementBlock sbl){
	...
	for(int i = 0; i<sb.getStatementCount(); i++){
	    if(fileName == null){
		fileName = 
		    sb.getStatementAt(i).getPositionInfo().getFileName();
	    }
	    if(sb.getStatementAt(i) instanceof StatementBlock){
		result = result.append(
		    flatten((StatementBlock) sb.getStatementAt(i)));
	    }else if(sb.getStatementAt(i) instanceof MethodBodyStatement){
		result = result.append(
		    convertMBSToMethodCall(
			(MethodBodyStatement)sb.getStatementAt(i)));
	    }else{
    ...
    /**
     * Returns the formula from which an oracle for the extracted code can
     * be created.
     */
	...
	for(int i = 0; i<uop.locationCount(); i++){
	    Expression l,r;
	    l = (Expression) convertToProgramElement(uop.location(t, i));
	    r = (Expression) convertToProgramElement(uop.value(t, i));
	    result = result.append(TestGenerator.assignmentOrSet(l, r, serv));
	}
	return result;
    }
    ...
    /**
     * Returns the program variables newly introduced as substitution for
	...
	methodName = 
	    mbs.getProgramMethod(serv).getMethodDeclaration().getName();
	String sortName = 
	    mbs.getProgramMethod(serv).getContainerType().
	    getSort().name().toString();
	fileName = sortName.substring(sortName.lastIndexOf(".")+1, 
				      sortName.length());
...
//	}
	MethodReference mr = new MethodReference(
	    ...
	    mbs.getProgramMethod(serv).isStatic() ? 
	    new TypeRef(mbs.getProgramMethod(serv).
			getContainerType()) : 
