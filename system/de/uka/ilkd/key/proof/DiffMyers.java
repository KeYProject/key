...
public class DiffMyers
{
   ...
   */
  public DiffMyers(PosInTerm a, PosInTerm b)
  {
    Hashtable h = new Hashtable(a.depth() + b.depth());
    filevec[0] = new file_data(a,h);
    filevec[1] = new file_data(b,h);
  }


  public DiffMyers(Vector a, Vector b)
  {
  ...
  public static class change {
    ...
    public String toString() {
       String s = "";
       if (deleted >0) s+="deleted "+ deleted+" at "+line0+"\n";
       if (inserted>0) s+="inserted "+inserted+" at "+line1+"\n";
       if (link!=null) s += link;
       return s;
    }
    
    public int diminishingPenalty() {
       int p=0;
       for (int i=0; i<deleted; i++) p+= 100/(line0+i+1);
       for (int i=0; i<inserted; i++) p+= 75/(line1+i+1);
       p = -p;
       if (link==null) return p; else return p+link.diminishingPenalty();
    }


    public int uniformPenalty() {
       int p=10*deleted+5*inserted;
       p = -p;
       if (link==null) return p; else return p+link.uniformPenalty();
    }
...
<p>
    ...
    private byte[] discardable(final int[] counts) {
      ...
      final int[] equivals = this.equivs;
      int many = 5;
      int tem = end / 64;
    ...
    file_data(Vector data,Hashtable h) {
    ...
    file_data(PosInTerm data,Hashtable h) {
      buffered_lines = data.depth();

      equivs = new int[buffered_lines]; 
      undiscarded = new int[buffered_lines];
      realindexes = new int[buffered_lines];
      
      IntIterator it = data.reverseIterator();
      
      for (int i = buffered_lines-1; i >= 0; i--) {
          equivs[i] = it.next();
          if (equivs[i]>equiv_max) equiv_max=equivs[i];
      }
      equiv_max++; // will be used as array length
    }
  ...
  public static void main(String argv[]) {
      PosInTerm p1 = PosInTerm.TOP_LEVEL.down(1).down(2).down(3).down(4);
      PosInTerm p2 = PosInTerm.TOP_LEVEL.down(0).down(1).down(3).down(5);
      System.out.println(new DiffMyers(p1, p2).diff_2());
  }
