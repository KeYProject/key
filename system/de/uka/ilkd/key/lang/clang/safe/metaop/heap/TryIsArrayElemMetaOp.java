...
/**
 * Meta operation that tries to evaluate expression <code>isArrayElem(o)</code>, 
 * where <code>o</code> is of object type. Evaluation is based on removing accessors
 * at the top-level of term <code>o</code>.
 * 
 * @author oleg.myrk@gmail.com
 */
public class TryIsArrayElemMetaOp extends SafeBaseMetaOp {

        public TryIsArrayElemMetaOp() {
                super(new Name("#ClangTryIsArrayElem"), 1);
        }
        ...
        protected Term calculate(Term term, IClangSafeEnvironment context) {
                ...
                if (!(term.sub(0).op() instanceof Function && term.sub(0).op().name().toString().equals("isArrayElem")))
                        throw new IllegalArgumentException("#TryIsArrayElem first argument must be 'isArrayElem' predicate");

                Term isArrayElemTerm = term.sub(0);
                Term objTerm = isArrayElemTerm.sub(0);
