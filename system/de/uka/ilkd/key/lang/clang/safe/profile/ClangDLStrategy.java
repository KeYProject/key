...
import de.uka.ilkd.key.strategy.feature.DirectlyBelowOpClassFeature;
...
import de.uka.ilkd.key.strategy.feature.UCIncompatibleFeature;
...
import de.uka.ilkd.key.strategy.termProjection.DivideMonomialsProjection;
...
/**
 * Safe C Dynamic Logic (CDL) calculus execution strategy.
 */
public class ClangDLStrategy extends AbstractFeatureStrategy {
    ...
    protected ClangDLStrategy(Proof p_proof,
...
//              splitF,
    ...
    private RuleSetDispatchFeature setupCostComputationF(Proof p_proof) {
		...
		// C specific: begin
        bindRuleSet(d, "simplify_arithm", -100);
        bindRuleSet(d, "simplify_heap", -100);
        bindRuleSet(d, "simplify_updates", -100);

        bindRuleSet(d, "infer", -500);
        bindRuleSet(d, "infer_heap", -500);
        
        bindRuleSet ( d, "simplify_heap_try",
                        add ( EqNonDuplicateAppFeature.INSTANCE,
                              longConst ( -500 ) ) );                                
        
        bindRuleSet ( d, "simplify_arithm_bounds",
                        add ( EqNonDuplicateAppFeature.INSTANCE,
                              longConst ( -500 ) ) );                                
		// C specific: end
...
//                            rec ( any (), longTermConst ( 5 ) ) )
    ...
    public static class Factory extends StrategyFactory {
        ...
        public Strategy create ( Proof p_proof, 
                StrategyProperties strategyProperties) {     
            return new ClangDLStrategy ( p_proof, strategyProperties );
        }
