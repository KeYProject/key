...
public abstract class AbstractInReachableStatePOBuilder extends TermBuilder {

    protected final UpdateFactory uf;

    protected final Services services;
    protected final Sort intSort;
    protected final ProgramVariable created;
    protected final Term TRUE;
    protected final Term FALSE;
    protected final ProgramVariable arraylength;

    public AbstractInReachableStatePOBuilder(Services services) {
	super();
	...
	Term result =
	        update(update, imp(equals(
	                ...
	                createdOrNull(dot(tPre,
	                        (AttributeOp) loc))));
	return all(vPre, imp(preAx, result));
    }
	...
	Term result = staticFieldLiveRef(update, pair);
    ...
    protected Term validateNextToCreateUpdate(boolean[] ax, final Update update,
     ...
     */
    protected Term staticFieldLiveRef(Update update, AssignmentPair pair) {
     ...
     */
    protected Term nextToCreateUpdatedSafely(final Update update, final ProgramVariable pv) {
        Term result;
     ...
     */
    protected Term addNextToCreateEnumPO(ProgramVariable pv, Update update) {
     ...
     */
    protected Term enumConstantPO(ProgramVariable pv, Update update) {
     ...
     * 
     */
    private Term classInitializedUpdateIRSConform(Update update, ProgramVariable[] implicitFields) {
    ...
    protected Term arrayStoreValid(Term arrayRef, Term arrayValue) {
	final Function f = (Function) services.getNamespaces().functions()
	        .lookup(new Name("arrayStoreValid"));
    ...
    protected Term update(Update update, Term target) {          
        return uf.prepend(update, target);
    }

    protected Term createdOrNull(final Term t_o_a) {
    ...
    protected Term preAx(Term[] t1, Term[] t2) {
        Term result = tt();
    ...
    protected LogicVariable[] atPre(AssignmentPair pair) {
