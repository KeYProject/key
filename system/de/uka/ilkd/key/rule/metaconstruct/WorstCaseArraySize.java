...
public class WorstCaseArraySize extends AbstractMetaOperator {
    ...
    public Term calculate(Term term, SVInstantiations svInst, Services services) {
        ...
        lit = new IntLiteral("18");
        Term eightteen =  services.getTypeConverter().convertToLogicElement(lit);
        lit = new IntLiteral("19");
        Term nineteen =  services.getTypeConverter().convertToLogicElement(lit);
        
        if(convertToDecimalString(term.sub(0), services).equals("8") ||
           convertToDecimalString(term.sub(0), services).equals("4")){
            return tf.createFunctionTerm(add, sixteen, tf.createFunctionTerm(
                    mul, term.sub(0), term.sub(1)));
        }else if(convertToDecimalString(term.sub(0), services).equals("2")){
            return tf.createFunctionTerm(add, eightteen, tf.createFunctionTerm(
                    mul, term.sub(0), term.sub(1)));            
        }else{
            return tf.createFunctionTerm(add, nineteen, tf.createFunctionTerm(
                    mul, term.sub(0), term.sub(1)));            
        }
    }
}
