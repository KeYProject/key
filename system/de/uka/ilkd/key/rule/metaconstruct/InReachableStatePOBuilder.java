...
/**
 * generates the proof obligation establishing that a given state describes
 * a legal pointer structure, i.e. that serveral system invariants are satisfied, like
 * no created object references a non-created one. 
 */
public class InReachableStatePOBuilder extends TermBuilder {
    ...
    public InReachableStatePOBuilder(Services services) {
        ...
        this.created = services.getJavaInfo().getAttribute(
     ...
     * described by update <tt>U</tt> in <tt>updateInReachableState:=U(<tt>inReachableState</tt>)</tt> 
     * leads to a legal pointer structure reachable from the current state.    
     */
    public Term generatePO(Term updateInReachableState) {
        ...
        final IUpdateOperator updateOp = (IUpdateOperator)updateInReachableState.op();        
        ...
        for (int i = 0; i<pairs.size(); i++) {
            ...
            if (loc instanceof ProgramVariable && loc.sort() != Sort.NULL) {
                ...
                if (pv.isStatic()) {                    
                    ...
                    } else { // all implicit field are currently no reference fields
                        final ObjectSort containerType = (ObjectSort) pv.getContainerType().getSort();
                        ProgramVariable[] implicitFields = new ProgramVariable[]{
                                cErroneous(containerType), cInitialized(containerType),
                                cInInit(containerType), cPrepared(containerType), ntc(containerType)      
                        };
                        
                        if (pv == implicitFields[4]) {                       
                            result = nextToCreateUpdatedSafely(update, pv);
                        } else {
                            // if one of these fields is updated we need an additional axiom
                            boolean additionalAxiom = true;
                            if (pv == implicitFields[0]) {                                
                                result = classErroneousUpdateIRSConform(update, implicitFields);                                                         
                            } else if (pv == implicitFields[1]) {
                                result = classInitializedUpdateIRSConform(update, implicitFields);                         
                            } else if (pv == implicitFields[2]) {
                                result = classInitInProgressUpdateIRSConform(update, implicitFields);                         
                            } else {
                                additionalAxiom = false;
                            }
                            if (additionalAxiom) {
                                // in <tt>inReachableState</tt>, if a class is neither initialized, nor inInitialization, nor
                                // erroneous (yes, objects of erroneous classes can exist) than no objects
                                // are created
                                final Term notErroneous   = equals(var(implicitFields[0]), FALSE);
                                final Term notInitialized = equals(var(implicitFields[1]), FALSE);
                                final Term notInInit      = equals(var(implicitFields[2]), FALSE);
                                final Term ntcIsZero      = equals(var(implicitFields[4]), zero(services));
                                result = and(update(update, 
                                                imp(and(and(notErroneous, notInitialized), notInInit),
                                                    ntcIsZero)), result);
                            }                            
                        }
                    }                                                           
                ...
                final ProgramVariable pv = (ProgramVariable) ((AttributeOp)loc).attribute();
                final Term refPrefix     = ((AttributeOp)loc).referencePrefix(pair.locationAsTerm());
                ...
                if (refPrefix.sort() != Sort.NULL) {                    
                    if (loc.sort() instanceof ObjectSort) {                    
                        ...
                        result = update ( update, imp ( equals(dot (tPre[0], created), TRUE), 
                                                        createdOrNull(dot(tPre, (AttributeOp)loc))));                        
                        result = all ( vPre, imp(preAx, result));                    
                    } else if (pv == created) {
                        if (refPrefix.op() instanceof SortDependingFunction && 
                                ((SortDependingFunction)refPrefix.op()).getKind().
                                equals(AbstractSort.OBJECT_REPOSITORY_NAME)) {                                                                                    
                            result = createdInvariantForReposInstance(update, refPrefix);
                        } else {
                            return updateInReachableState;
                        }
                ...
                final Sort elementSort = ((ArraySort)((ArrayOp)loc).arraySort()).elementSort();
                if (elementSort instanceof ObjectSort) {
                    ...
                    result = update ( update, and( imp ( equals ( dot ( tPre[0], created ), TRUE),
    ...
    private Term arrayStoreValid(Term arrayRef, Term arrayValue) {
        final Function f = (Function) services.getNamespaces().functions().lookup(new Name("arrayStoreValid"));
    ...
    /**
     * in general an update of the <tt><created></tt> attribute would imply to show a large 
     * formula in order to ensure that the <tt>inReachableState</tt> property is preserved. In case of an update 
     * pair 
     *    <tt> T::<get>(idx).<created>=TRUE </tt>
     * the formula can be drastically reduced to  
     * <code>
    ...
    private Term createdInvariantForReposInstance(Update update, Term t_get) {
        ...
        result = equiv( equals(dot(getPreIdx, created), TRUE), 
                        ex(lv, and( interval(zero(services), var(lv), var(ntc(os))), 
    ...
    /**
     * for each assignment pair of an update a formula is created with the relevant parts 
     * of the system invariants necessary to be shown to be preserved. This method computes
     * the closure of this formula wrt. quantified updates.  
     * 
     * @param pair the assignment pair used to determine the relevant parts of the
     * system invariant
     * @param relevantInvariants  the relevant part of the system invariants to be checked
     * for preserveness
     * @return the closure of the relvant invariant formula
     */
    private Term quanUpdateClosure(final AssignmentPair pair, Term relevantInvariants) {
        Term closure = relevantInvariants;
        if (pair.nontrivialGuard()) {
            closure = imp(pair.guard(), closure);
        }
        if (pair.boundVars().size() > 0) {
            closure = tf.createQuantifierTerm(Op.ALL, pair.boundVars(), closure);                                        
        }
        return closure;
    }
    ...
    /**
     * conjunction of terms that state global invariants, i.e. which have to be added to
     * the generated proof obligation in each case and where no optimisation is possible by
     * disregarding pieces by looking on the assignment pairs
     * @param update the Update to be checked to preserve <tt>inReachableState</tt>
    ...
    /**
     * Generates a formula ensuring that an update (U) does not delete a former created
     * object. This created axiom is as follows:
     *     <code>
     *          \forall Object o; (o.<created> = TRUE -> U o.<created> = TRUE)
     *     </code> 
     * @param update the Update U which is checked for object deletion
     * @return a formula that evaluates to true if the update does not delete objects
     */
    private Term noObjectDeletion(Update update) {
        final LogicVariable o = new LogicVariable(new Name("o"), 
    ...
    /**
     * Generates a formula checking that the given static field <tt>T.sv</tt> references <tt>null</tt>
     * or a created object.    
     * Let 
     *    <tt>pair=(T.sv := t)</tt>
     * the method assumes that loc is a static field of reference type ref (ref != Sort.NULL)
     * and generates the following formula
     * <code>
     ...
     * @param the update U to be checked
     * 
     * @return a formula that evaluates to true iff the update U does not update the
     * static field loc to an "illegal" value
     */
    private Term staticFieldLiveRef(Update update, AssignmentPair pair) {
        final ProgramVariable classInit = services.getJavaInfo().
    ...
    /**
     * generates a formula that ensures that all ensuring that the updated 
     * field <tt>T.<nextToCreate></tt> still satisfies its invariants
     * namely that its value 
     *   <ul>
     *   <li>remains greater than or equals zero</li>
     *   <li>that all created objects have an index between 0 and T.<nextToCreate> </li>
     *   </ul>
     * <code>
     *   U ( T.<nextToCreate> >= 0 )  
     *     &
     ...
     * </code>
     * @param update the update to be checked
     * @param pv     the ProgramVariable representing <tt>T.<nextToCreate></tt>
     * @return a formula that evaluates to true if the update U does not destroy the 
     * system invariants of <tt>T.<nextToCreate></tt>
     */
    private Term nextToCreateUpdatedSafely(final Update update, final ProgramVariable pv) {
        Term result;
        ...
        result = and(result, all(iv, imp(interval, 
                                         update(update, 
                                            equals(dot(func(rep(os), var(iv)), 
                                                       created),  
                                                   TRUE(services))))));         
    ...
    /**
     * A class that is marked as erroneous is neither initialized nor is its initialization
     * in progress. 
     * 
     * If a class is marked erroneous its subclasses are not allowed to be initialized. It is
     * sufficient to test this property for its direct subclasses. As if 
     *    ...TODO
     * 
     * Additionally: from an <tt>inReachableState</tt> state with a class C marked erroneous, no state in 
     * <tt>inReachableState</tt> is reachable where the same class is not erroneous 
     * 
     * <code>
     ...
     * </code>
     * @param update the Update describing the new state to be checked for the <tt>inReachableState</tt> property
     ...
     *    (in this order)
     * @return a formula that evaluates to true if erroneous has been updated by U in an <tt>inReachableState</tt>
     * comforming way
     */
    private Term classErroneousUpdateIRSConform(Update update, ProgramVariable[] implicitFields) {
        
        final Term classErroneous = equals(var(implicitFields[0]), TRUE);       
        
        Term result = classFieldUpdateConform(update, implicitFields[0], implicitFields[1], 
                implicitFields[2], null);        
        ...
        if (!directSubTypes.isEmpty()) {
            ...
            while (it.hasNext()) {                
                ...
                subsNotInit = and(subsNotInit, 
                        equals(var(subsCInitPV), FALSE));
            }           
            result = and(result, update(update, subsNotInit));
        }        
        
        result =  and(result, imp(classErroneous, update(update, classErroneous)));
     ...
     * comforming way
     */
    private Term classInitInProgressUpdateIRSConform(Update update, ProgramVariable[] implicitFields) {
  
        Term result = classFieldUpdateConform(update, implicitFields[2], 
    ...
    /**
     * A class that is marked as initialized is neither initialized nor is its initialization
     * in progress. 
     * 
     * If a class is initialized all its supertypes must be initialized, too. It is sufficient
     * to look at the direct supertypes as if they are initialized all their supertypes are also
     * initialized in the prestate (as it has the <tt>inReachableState</tt> property) or their fields have been
     * updated and the formula ensuring this property for their supertypes will be 
     * (or have been) created when looking at the other assignment pairs of the update)
     * 
     * Additionally: from an <tt>inReachableState</tt> state with an initialized class C, no state in 
     * <tt>inReachableState</tt> is reachable where the same class is not initialized 
     * 
     * <code>
    ...
    private Term classInitializedUpdateIRSConform(Update update, 
        ...
        Term result = classFieldUpdateConform(update, implicitFields[1], 
                implicitFields[0], implicitFields[2], implicitFields[3]);
        // direct supertypes
        
        final ListOfKeYJavaType directSuperTypes = 
            services.getJavaInfo().getDirectSuperTypes(implicitFields[0].
                    getContainerType());
        ...
        if (!directSuperTypes.isEmpty()) {
            ...
            while (it.hasNext()) {                
                final ProgramVariable superCInitPV = 
                    cInitialized((ObjectSort) it.next().getSort());
                superTypesInit = and(superTypesInit, equals(var(superCInitPV), TRUE));
            }
            result = and(result, update(update, imp(classInitialized, superTypesInit)));
        }        
        
        // reachable <tt>inReachableState</tt> state
        result =  and(result, imp(classInitialized, update(update, classInitialized)));
    ...
    private Term arrayLengthIsIRSConform(Term arrayReference, Update u) {
        ...
        return all(preRef, 
                imp(and(equals(var(preRef), arrayReference),
                        not(equals(var(preRef), NULL(services)))),
                        update(u, 
                                imp(equals(dot(var(preRef), created), 
                                        TRUE(services)),
                                    geq(dot(var(preRef), arraylength), 
                                    zero(services), services)))));
    }
    ...
    private Term classFieldUpdateConform(Update update, ProgramVariable fieldA, 
            ProgramVariable fieldB, ProgramVariable fieldC, ProgramVariable fieldD) {
    ...
    private Term createdOrNull(final Term t_o_a) {
        return or(equals(dot(t_o_a, created), TRUE), 
                  equals(t_o_a, NULL(services)));        
    }
        
    private ProgramVariable ntc(ObjectSort os) {
        return services.getJavaInfo().
         getAttribute(ImplicitFieldAdder.IMPLICIT_NEXT_TO_CREATE, os);
    }
    
    private ProgramVariable cInInit(ObjectSort os) {
        return services.getJavaInfo().
        getAttribute(ImplicitFieldAdder.IMPLICIT_CLASS_INIT_IN_PROGRESS, os);   
    }


    private ProgramVariable cInitialized(ObjectSort os) {
        return services.getJavaInfo().
        getAttribute(ImplicitFieldAdder.IMPLICIT_CLASS_INITIALIZED, os);  
    }

    private ProgramVariable cPrepared(ObjectSort os) {
        return services.getJavaInfo().
        getAttribute(ImplicitFieldAdder.IMPLICIT_CLASS_PREPARED, os);  
    }


    private ProgramVariable cErroneous(ObjectSort os) {
        return services.getJavaInfo().
        getAttribute(ImplicitFieldAdder.IMPLICIT_CLASS_ERRONEOUS, os);
    }
    ...
    private Function rep(ObjectSort os) {
        return (Function) ((SortDefiningSymbols)os).
