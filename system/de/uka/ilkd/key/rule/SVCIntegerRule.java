...
import de.uka.ilkd.key.proof.decproc.DecisionProcedureSVC;
 ...
 * decicion procedure SVC
 * 
 * @author akuwertz
 * @version 1.0,  05/29/2006
 */

public class SVCIntegerRule extends AbstractIntegerRule {
     ...
     */
    public SVCIntegerRule( boolean resultWindow, 
                               DecisionProcedureTranslationFactory dptf ) {
        super( new Name( "Decision procedure SVC" ), resultWindow, dptf );
    }
    
    
    /** A mere constructor for convenience. Creates an <tt>SVCIntegerRule</tt> with 
     * <tt>resultWindow</tt> set to <tt>true</tt>
     * 
     * @param dptf the <tt>DecisionProcedureTranslationFactory</tt> to be used
     */
    public SVCIntegerRule( DecisionProcedureTranslationFactory dptf ) {
        this( true, dptf );
    }
    ...
    protected AbstractDecisionProcedure getDecisionProcedure( Goal goal, Constraint userConstraint,
                                                              Services services ) {
       return new DecisionProcedureSVC( goal, dptf, services );
    }

    public AbstractIntegerRule clone(DecisionProcedureTranslationFactory dptf) {
        return new SVCIntegerRule(showResults, dptf);
    }
