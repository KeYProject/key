...
/**
 * Class representing the intersection of a set of constraints (i.e. of
 * arbitrary objects implementing the "Constraint"-interface). Intersection
...
public class IntersectionConstraint implements Constraint {
    ...
    /**
     * Elements of the set this constraint is the intersection of; objects
     * having "subConstraints" with less than two elements are not well-formed,
     * and the list must not contain two different elements from which one is
    ...
    /**
     * Intersects p_c0 with p_c1, returning a constraint weaker or as strong as
     * both p_c0 and p_c1.
     * 
     * @param p_diff
     *            a constraint which is as strong (-> simple) as possible
     *            satisfying intersect ( p_c0, p_diff.val () ) == intersect (
     *            p_c0, p_c1 )
     */
    public static Constraint intersect(Constraint p_c0, Constraint p_c1,
     ...
     * 
     * @param p_diff
     *            a constraint which is as strong as possible satisfying
     *            this.intersectHelp ( p_diff.val () ) == this.intersectHelp (
     *            p_co )
     * @return A constraint representing the intersection of this and p_co
     */
    ...
    /**
     * Removes all elements from "this.subConstraints" that are stronger than
     * "p_co". Iff "p_co" is stronger than any element of "this.subConstraints",
     * "p_stronger" is set to true (and "this" is returned). The return value
     * may in fact be a not well-formed object of this class ("subConstraints"
     * may be empty or contain only one element), that can be made correct by
     ...
     *            term to be unified
     * @param services
     *            a Namespace where new sorts that may be created when unifying
     *            are added. If the value is null unifying will fail if new
     *            sorts are required for a succesfull unification
     ...
     *            sorts are required for a succesfull unification
     * @param unchanged
     *            true iff the new constraint is subsumed by this one
     * @return TOP if not possible, else a new constraint with after unification
     *         of t1 and t2
     */
    // TODO
     ...
     *        unifying are added. If the value is null unifying and therewith
     *        joining will fail if new sorts are required
	...
	while (it.hasNext())
	    res = IntersectionConstraint.intersect(res, co.join(it.next(),
		    services));
     ...
     *            sorts are required for a succesfull unification
     * @param unchanged
     *            the BooleanContainers value set true, if this constraint is
     *            stronger than co (false may stand for "don't know")
     * @return the joined constraint
     */
    // TODO
    public Constraint join(Constraint co, Services services,
	...
	if (co instanceof IntersectionConstraint) {
	    IteratorOfConstraint it = ((IntersectionConstraint) co).subConstraints
		    .iterator();
	    while (it.hasNext()) {
