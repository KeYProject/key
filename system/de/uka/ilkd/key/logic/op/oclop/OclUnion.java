...
/**
 * Represents the OCL operation: union()
 */
public class OclUnion extends TermSymbol {
   
    public OclUnion() {
	super(new Name("$union"), OclSort.COLLECTION_OF_OCLANY);
    }
	...
	if ((collSort0.getCollectionKind() == CollectionSort.SEQUENCE
	    ...
	    || (collSort0.getCollectionKind() != CollectionSort.SEQUENCE
		&& collSort1.getCollectionKind() == CollectionSort.SEQUENCE)) {
	    return false;
	}
	...
	if (collKind0 == CollectionSort.SET
	    && collKind1 == CollectionSort.SET) {
	    resultCollKind = CollectionSort.SET;
	} else if (collKind0 == CollectionSort.BAG
		   || collKind1 == CollectionSort.BAG) {
	    resultCollKind = CollectionSort.BAG;
	} else if (collKind0 == CollectionSort.SEQUENCE
		   && collKind1 == CollectionSort.SEQUENCE) {
	    resultCollKind = CollectionSort.SEQUENCE;
	} else {
	    throw new IllegalArgumentException("Cannot determine sort of invalid term\n"
					       + "(Operator not defined on these Collection sorts).");
	}
	...
	return CollectionSort
	    .getCollectionSort(resultCollKind, resultElemSort);
    }
