...
public class BagSort extends de.uka.ilkd.key.logic.sort.AbstractCollectionSort {
    ...
    protected BagSort(de.uka.ilkd.key.logic.sort.Sort elemSort) {
        super("Bag"+"Of"+elemSort.name(), elemSort);
    }
    ...
    public Sort cloneFor ( Sort p ) {
        if ( p instanceof AbstractNonCollectionSort )
            return ((AbstractNonCollectionSort)p).getBagSort ();
        else
            return null;
    }
	...
	if(!symbolsCreated) {
	    ...
	    final Sort intSort = (Sort)sort_ns.lookup(new Name("int"));	
            ...
            res = createSymbol ( res, "notEmpty",            Sort.FORMULA,
                                 new Sort[] { this } );
            res = createSymbol ( res, "emptyBag",            this,
                                 new Sort [0] );
    ...
    ( ListOfSortDependingSymbol p_list,
      ...
      Sort[]                    p_argSorts ) {
        ...
        Name kind     = new Name ( "Bag::" + p_name );
        Name fullname = new Name ( "Bag" + "Of" + elementSort().name() +
                                   "::" + p_name );
