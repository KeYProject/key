...
public final class WhileInvariantRule implements BuiltInRule {
    ...
    public Term getInnermostSelfTerm(Term term, Services services) {
        ...
        while(term.op() instanceof UpdateApplication) {
            term = term.sub(UpdateApplication.targetPos());
        }
...
//	    Goal goal) {
	...
	// allowedLocalVars =
	// allowedLocalVars.add((ProgramVariable)locTerm.op());
	// }
	// }
	// }
	// {
	...
	// Update heapAnonUpdate = TB.elementaryUpdate(heapTerm,
	// changeToAnonHeapTerm);
	// anonUpdate = TB.parallel(anonUpdate, heapAnonUpdate);
	// }
	//    
	// //prepare heap frame condition, heap frame taclet, heap atPre
	// definitions
	// final Sort objectSort =
	// services.getJavaInfo().getJavaLangObjectAsSort();
	// final Sort fieldSort =
	// services.getTypeConverter().getHeapLDT().getFieldSort();
	// final LogicVariable objVar = new LogicVariable(new Name("o"),
	// objectSort);
	// final LogicVariable fieldVar = new LogicVariable(new Name("f"),
	// fieldSort);
	...
	//        
	// //put together
	// final Term frameCondition = TB.and(heapFrameCondition,
	// localFrameCondition);
...
//	return result;
	...
	public AnonymisationInfo(Term anonUpdate, Term frameCondition,
		Term localFrameCondition, Term atPreDef) {
