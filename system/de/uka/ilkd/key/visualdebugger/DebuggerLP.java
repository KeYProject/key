...
public class DebuggerLP extends LogicPrinter {
    ...
    public DebuggerLP(ProgramPrinter prgPrinter, NotationInfo notationInfo,  Services services, HashMap inputValues) {
    ...
    public DebuggerLP(ProgramPrinter prgPrinter, NotationInfo notationInfo,  Services services, HashMap inputValues,LinkedList objects, SymbolicObject thisObject) {
        super(prgPrinter, notationInfo,  services);
        info = notationInfo;
...
//        maybeParens(t.sub(0), ass);
    ...
    if(getNotationInfo().getAbbrevMap().isEnabled(t)){
        ...
        if (inputValues.containsKey(t)){
            ...
            if ((!vd.isStaticMethod()&&!vd.getSelfTerm().equals(it))||vd.isStaticMethod())
                layouter.print(it.toString()+"_i");
            //else if (vd.isStaticMethod());
            else layouter.print(it.toString());
        }else{
            if (t.op() instanceof SortDependingFunction &&
                    ((SortDependingFunction)t.op()).name().toString().endsWith("<get>")){
                Term sub = t.sub(0);
                if (sub.arity()>1)
                layouter.print("new"+t.sort()+sub.sub(0).toString());
                else layouter.print("new"+t.sort()+"0");
            } else
            
        getNotationInfo().getNotation(t.op(), services).print(t,this);
        }
    }
}
    ...
    public void printFunctionTerm(String name,
       ...
       // System.out.println("PF "+ t);
        if (s!=null){
            startTerm(0);
            layouter.print(s);
        }else super.printFunctionTerm(name, t)
            ;
