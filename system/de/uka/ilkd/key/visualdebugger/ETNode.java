...
public class ETNode {
    ...
    private KeYMediator mediator = VisualDebugger.getVisualDebugger().getMediator();
    ...
    protected void setMethodInvocation(){
        if (this instanceof ETMethodInvocationNode){
            ...
            if (parent.getLastMethodInvocation().getParent()!=null)
                this.lastMethodInvocation=parent.getLastMethodInvocation().getParent().getLastMethodInvocation();
    ...
    public void appendBC(ListOfTerm l){
        if(bc!=null) //TODO ??????? remove this!
            this.bc= bc.append(l);
        else bc = SLListOfTerm.EMPTY_LIST.append(l);
    }
    ...
    public String print(){
        ...
        if (getITNodesArray().length>0)
            result = result+" ETNode "+
            getITNodesArray()[0].getId()+"  "+ bc;
        else result = result+" ETNode "+
        "  "+ bc;
    ...
    public void removeRedundandITNodes(){
        ...
        for(int i=0;i<nodes.length;i++){
            for(int j=0;j<nodes.length;j++){
                if (i!=j && !a[j] && !a[i]){
                    //   System.out.println("Redundant? "+nodes[i].getPc()+"  ->   "+nodes[j].getPc());
