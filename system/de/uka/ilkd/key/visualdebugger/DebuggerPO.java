...
public class DebuggerPO implements ProofOblInput {
     ...
     * @param name a String with a short describing name 
     * what is computed 
     */
    ...
    /**
     * the formula with the program whose specification
     *  has to be computed 
     * @param specFormula the Term to be loaded
     */
    ...
    public void setSpecFormula(ListOfTerm specFormula) {
        ...
        for(IteratorOfTerm it = specFormula.iterator();it.hasNext();){
            ...
            if (result == null)
                ...
                result = TermFactory.DEFAULT.createJunctorTerm(Op.AND, result, t);            
        }       
    ...
    public void setPCImpl(ListOfTerm l1, ListOfTerm l2){
        ...
        specFormula = TermFactory.DEFAULT.createJunctorTerm(Op.IMP,new Term[]{t1,t2});
    }
    ...
    public void setSpecFormula(Sequent s) {
        ...
        Term a = TermFactory.DEFAULT.createJunctorTerm(Op.AND, getTerms(ant.toList()).toArray());
        Term b = TermFactory.DEFAULT.createJunctorTerm(Op.OR, getTerms(succ.toList()).toArray());
    ...
    public void setUp(Sequent precondition, ITNode n){
        ...
        for(IteratorOfTerm it = n.getPc(true).iterator();it.hasNext();){
            Term t = it.next();
            if (t.op() == Op.NOT)
                result = result.addFormula(new ConstrainedFormula(t.sub(0),Constraint.BOTTOM), false, true).sequent();
            else result = result.addFormula(new ConstrainedFormula(t,Constraint.BOTTOM), true, true).sequent();
    ...
    public void setUp(Sequent precondition, ITNode n,SetOfTerm indexConf){
        ...
        for(IteratorOfTerm it = indexConf.iterator();it.hasNext();){
            Term t = it.next();
            if (t.op() == Op.NOT)
                this.sequent = sequent.addFormula(new ConstrainedFormula(t.sub(0),Constraint.BOTTOM), false, true).sequent();
            else sequent = sequent.addFormula(new ConstrainedFormula(t,Constraint.BOTTOM), true, true).sequent();
        }
    }

    public void setUp(Sequent precondition, ITNode n,SetOfTerm indexConf,Term post){
        this.setUp(precondition, n, indexConf);
        sequent = sequent.addFormula(new ConstrainedFormula(post,Constraint.BOTTOM), false,true).sequent();
    ...
    public void setTerms(ListOfTerm terms){
        ...
        specFormula = TermFactory.DEFAULT.createJunctorTerm(Op.NOT,specFormula);
    }

    /**
     * the proof settings to be used
     * @param settings the ProofSettings to be used for e.g. the
     *  update simplifier to be taken
     */
    ...
    /**
     * the initial config containing for example the services which 
     * provide access to the Java model 
     */
    ...
    /**
     * the indices with the rules to be used for specification 
     * computation
     * @param taclets the TacletIndex with the taclet rule base to be used
     * @param builtInRules the BuiltInRuleIndex with all available built-in 
     * rules    
     */
    public void setIndices(TacletIndex taclets, 
    ...
    public ProofAggregate getPO() {          
        if (po == null) {
            ...
            Proof proof= null;
            if (specFormula!=null)
                proof= new Proof(name, specFormula, "", 
                        taclets, builtInRules, 
                        initConfig.getServices(), 
                        settings);
            else if (sequent!=null ){
                proof = new Proof(name, sequent, "", 
                        taclets, builtInRules, 
                        initConfig.getServices(), 
                        settings);
            }
            proof.setSimplifier(settings.getSimultaneousUpdateSimplifierSettings().getSimplifier());
