...
public class StateVisualization  {
    ...
    public StateVisualization(ITNode itn, KeYMediator mediator){
       ...
       this.programPio =            
            VisualDebugger.getVisualDebugger().getProgramPIO(itNode.getNode().sequent());
        if (programPio==null){
            programPio = VisualDebugger.getVisualDebugger().getExecutionTerminatedNormal(itNode.getNode());
        }
        if (programPio==null)
            throw new RuntimeException("Program Pio not found in Sequent "+itNode.getNode().sequent());
...
//        System.out.println("AttributeOps  "+this.changedAttributes);
        ...
        VisualDebugger.print("Formual"+programPio.constrainedFormula().formula());
        
        
        programPio2 = addRememberPrestateUpdates(programPio.constrainedFormula().formula());
        ...
        VisualDebugger.print("----------- Open Goals after ref case distinctions -------");
        ...
        for(int i=0;i<instanceConfigurations.length;i++){
            ...
            VisualDebugger.print("----------- Open Goals after array case dist ---------");
        ...
        for(int i=0;i<instanceConfigurations.length;i++){
            ...
            for(int j=0;j<indexConfigurations[i].length;j++){
                this.setUpProofForPostValues(instanceConfigurations[i].union(indexConfigurations[i][j]));
                ...
                postValues[i][j]=
                    getPostState(ps.getProof().openGoals().iterator().next().node().sequent());
       ...
       //for(IteratorOfGoal it = ps.getProof().openGoals().iterator();it.hasNext();)
        //   applyCutsForPost(it.next()); 
      // getPostStates(ps.getProof().openGoals());
       VisualDebugger.getVisualDebugger().fireDebuggerEvent(new DebuggerEvent(DebuggerEvent.VIS_STATE,this));
    }
    ...
    private void simplifyUpdate() {
        ...
        if (programPio==null){
            programPio = VisualDebugger.getVisualDebugger().getExecutionTerminatedNormal(ps.getProof().openGoals().iterator().next().node());
        } 
    }
    ...
    private void computeInstanceConfigurations() {
        ...
        for(IteratorOfGoal it = ps.getProof().openGoals().iterator();it.hasNext();){
            indexConf.add(getAppliedCutsSet(it.next().node(),ps.getProof().root()));
        }
    ...
    private SetOfTerm getAppliedCutsSet(Node n, Node root){
        //  System.out.println("getAppliedCuts "+n.serialNr()+" "+root.serialNr());
              ...
              if (n.getAppliedRuleApp() instanceof NoPosTacletApp){
                  ...
                  if (npta.taclet().name().toString().toUpperCase().equals("CUT")){
                      ...
                      if (n.child(1)==oldN)// TODO or 0
                          inst =TermFactory.DEFAULT.createJunctorTerm(Op.NOT,inst);                    
                ...
                if (!t1[i1].equals(t2[i2])){
                    //terms = terms.append(this.createEqu(t1[i1], t2[i2]));                    
                     if (t1[i1].sort().extendsTrans(t2[i2].sort())||
                         ...
                         //System.out.println("Cut and run to all open goals "+createEqu(t1[i1], t2[i2]));
                     applyCutAndRun((ps.getProof().openGoals()), createEqu(t1[i1], t2[i2]));
                     }
                }
            }
        }
    ...
    private void setUpProof(){
      ...
      po.setIndices(mediator.getProof().env().getInitConfig().createTacletIndex(), 
    ...
    //    Sort sorts[]=new Sort[locs.size()];
        ...
        stateOp = new RigidFunction(new Name("STATE"), Sort.FORMULA, s.toArray());

        result = TermFactory.DEFAULT.createFunctionTerm(stateOp, locs.toArray());
    ...
    private Term addRememberPrestateUpdates(Term target) {
        //QuanUpdateOperator op = QuanUpdateOperator.createUpdateOp(this.refsInPreState.toArray(), new boolean[locations.size()]);        
       ...
       // for(IteratorOfTerm it= this.refsInPreState.iterator();it.hasNext();){
        for(int i=0;i<locs.length;i++){
                ...
                final LocationVariable pv  =
                    new LocationVariable(new ProgramElementName ("pre"+i), locs[i].sub(0).sort());
                final Term t = TermFactory.DEFAULT.createFunctionTerm(pv);
                postAttributes[i]= TermFactory.DEFAULT.createAttributeTerm(
                        AttributeOp.getAttributeOp(((AttributeOp)locs[i].op()).attribute() ), t);
                
                newAP.add(new AssignmentPairImpl(pv, new Term[0], locs[i].sub(0)));
               // System.out.println("APS "+aps[i]);
                ...
                else if(locs[i].op() instanceof ArrayOp)
                {
                    final LocationVariable pv_array_ref  =
                        new LocationVariable(new ProgramElementName ("pre_array_"+i), locs[i].sub(0).sort());
                       final Term t = TermFactory.DEFAULT.createFunctionTerm(pv_array_ref);
                       
                       newAP.add(new AssignmentPairImpl(pv_array_ref, new Term[0], locs[i].sub(0)));
                       
                       
                       final LocationVariable pv_index  =
                           new LocationVariable(new ProgramElementName ("pre_array_index_"+i), locs[i].sub(1).sort());

                       final Term indexT = TermFactory.DEFAULT.createFunctionTerm(pv_index);
                       newAP.add(new AssignmentPairImpl(pv_index, new Term[0], locs[i].sub(1)));

                       
                       
                       postAttributes[i]= TermFactory.DEFAULT.createArrayTerm((ArrayOp)locs[i].op(), t, indexT);
                    
//                    aps[i]= new AssignmentPairImpl(pv_array_ref, new Term[0], locs[i].sub(0));
        ...
        statePred = createPredicate(SLListOfTerm.EMPTY_LIST.append(postAttributes));

        
        
        return UpdateSimplifierTermFactory.DEFAULT.createUpdateTerm(aps, this.statePred);
    ...
    private ListOfTerm getPostState(Sequent s){
        ...
        for(IteratorOfConstrainedFormula it =s.succedent().iterator();it.hasNext();){
    ...
    public SymbolicObjectDiagram getSymbolicState(int i, SetOfTerm indexTerms, boolean pre) {
        for (int j = 0; j < indexConfigurations[i].length; j++) {
            if (indexTerms.subset(indexConfigurations[i][j])) {
                VisualDebugger.print("Found " + j);
                final ListOfTerm pt = postValues[i][j];
                SymbolicObjectDiagram s = new SymbolicObjectDiagram(itNode, mediator
                        .getServices(), itNode.getPc(), refInPC, locations, pt,
                        pre, this.arrayLocations, indexConfigurations[i],
                        indexConfigurations[i][j], instanceConfigurations[i]);
