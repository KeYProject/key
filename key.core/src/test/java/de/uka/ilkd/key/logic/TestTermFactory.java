...
public class TestTermFactory {
    ...
    final JavaDLFunction p = new JavaDLFunction(new Name("p"), JavaDLTheory.FORMULA, sort1);
    // p(:S1):BOOL
    final LogicVariable x = new LogicVariable(new Name("x"), sort1); // x:S1
    final JavaDLFunction q =
        new JavaDLFunction(new Name("q"), JavaDLTheory.FORMULA, new SortImpl(new Name("Whatever")));
    // q(:Whatever):BOOL
    final LogicVariable z = new LogicVariable(new Name("z"), sort1); // z:S1
    final JavaDLFunction r = new JavaDLFunction(new Name("r"), JavaDLTheory.FORMULA, sort1, sort2);
    // r(:S1, :S2):BOOL
    ...
    final JavaDLFunction f = new JavaDLFunction(new Name("f"), sort1, sort3);
    // f(:S3):S1
    ...
    final JavaDLFunction g = new JavaDLFunction(new Name("g"), osort3, osort2, osort1);
    ...
    public void testSubSortsSubst() {
        ...
        JavaDLFunction c = new JavaDLFunction(new Name("c"), osort2, new Sort[0]);
        Term st = TB.subst(WarySubstOp.SUBST, v2, tf.createTerm(c), t);
        c = new JavaDLFunction(new Name("c"), osort4, new Sort[0]);
        st = TB.subst(WarySubstOp.SUBST, v2, tf.createTerm(c), t);
        c = new JavaDLFunction(new Name("c"), osort3, new Sort[0]);
        st = TB.subst(WarySubstOp.SUBST, v1, tf.createTerm(c), t);
        Exception exc = new Exception();
        try {
            c = new JavaDLFunction(new Name("c"), osort1, new Sort[0]);
            st = TB.subst(WarySubstOp.SUBST, v2, tf.createTerm(c), t);
        } catch (TermCreationException e) {
            exc = e;
        }
