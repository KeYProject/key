...
/**
 * Measures whether memory accumulates <em>across</em> repeated load/prove/dispose cycles of the same
 * obligation in one JVM. The 128-core stress run #1 was GC-bound with a steadily growing ~17 GB live
 * set, which suggested per-proof state was retained after {@link KeYEnvironment#dispose()} rather
 * than something intrinsic to the parallel prover; this probe isolates that on a single thread.
 *
 * <p>
 * It proves the obligation {@code reps} times, and after each cycle forces a full GC and records the
 * live heap. A flat trend means {@code dispose()} releases everything; a rising trend quantifies the
 * leak (MB per proof). With {@code -Dkey.mt.retention.histo=true} it additionally prints, at the end,
