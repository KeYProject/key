...
public class TestApplyTaclet {
    ...
    public void testSuccTacletWithoutIf() {
        ...
        assertEquals(seq.antecedent().getFirst().formula(), fma.sub(0), "Wrong antecedent after imp-right");
        assertEquals(seq.succedent().getFirst().formula(), fma.sub(1), "Wrong succedent after imp-right");
    }
    ...
    public void testAddingRule() {
        ...
        Sequent seq = goals.head().sequent();
        assertEquals(seq.antecedent().getFirst().formula(), fma.sub(0), "Wrong antecedent after imp_right_add");
        assertEquals(seq.succedent().getFirst().formula(), fma.sub(1), "Wrong succedent after imp_right_add");
        ...
        assertEquals(nfapp.head().instantiations().getInstantiation
                (TacletForTests.getSchemaVariables().lookup(new Name("b"))), aimpb, "Inserted cut rule's b should be instantiated to A -> B.");
        assertTrue(rApp.complete(), "Rule App should be complete");
        ...
        assertTrue(seq1.succedent().getFirst().formula().equals(aimpb) ||
                ...
                (seq2.succedent().get(1) != null
                        && seq2.succedent().get(1).formula().equals(aimpb)), "A->B should be in the succedent of one of the new goals now, "
    ...
    public void testSuccTacletAllRight() {
        ...
        Sequent seq = goals.head().sequent();
        assertEquals(seq.antecedent(), Semisequent.EMPTY_SEMISEQUENT, "Wrong antecedent after all-right");
        assertEquals(seq.succedent().getFirst().formula().op(), TacletForTests.getFunctions().lookup(new Name("p")), "Wrong succedent after all-right (op mismatch)");
    }
	...
	*/
    ...
    public void testAntecTacletWithoutIf() {
        ...
        if (!seq.succedent().isEmpty()) {
            assertEquals(seq.succedent().getFirst().formula(), fma.sub(0), "Wrong succedent after imp-left");
            goals = goals.tail();
            seq = goals.head().node().sequent();
            assertEquals(seq.antecedent().getFirst().formula(), fma.sub(1), "Wrong antecedent after imp-left");
        } else {
            assertEquals(seq.antecedent().getFirst().formula(), fma.sub(1), "Wrong antecedent after imp-left");
            goals = goals.tail();
            seq = goals.head().node().sequent();
    ...
    public void testNoFindTacletWithoutIf() {
        ...
        if (!seq1.antecedent().isEmpty()
            ...
            assertTrue(seq2.succedent().getFirst().formula().equals(t_c)
                    || seq2.succedent().get(1).formula().equals(t_c), "D is in antecedent of 1st goal but not in succedent of 2nd");
        } else {
            assertTrue(seq1.succedent().getFirst().formula().equals(t_c)
                    || seq1.succedent().get(1).formula().equals(t_c), "D is not in antecedent and not in succedent "
                    + "of first new goal");
            assertEquals(seq2.antecedent().getFirst().formula(), t_c, "D is in succedent of first new goal, but not in antecedent "
                    + "of second new goal");
        }
    }
...
*/
    ...
    public void testIncompleteNoFindTacletApp() {
        ...
        SchemaVariable b =
                TacletForTests.getSchemaVariables().lookup(new Name("b"));
        assertTrue(cut.uninstantiatedVars().contains(b), "b should be in the set of not instantiated SVs");
    }

    @Test
    public void testIncompleteSuccTacletApp() {
        TacletApp orright = TacletForTests.getRules().lookup("or_right");
        assertFalse(orright.complete(), "TacletApp should not be complete, as SVs are not instantiated");
        ...
        assertEquals(orright.uninstantiatedVars(), DefaultImmutableSet.<SchemaVariable>nil().add(b).add(c), "b and c should be in the set of not instantiated SVs");
        orright = orright.addInstantiation(b, TacletForTests.parseTerm("A"), false, services);
        assertFalse(orright.complete(), "TacletApp should not be complete, as B is not instantiated");
    ...
    public void testBugBrokenApply() {
        ...
        assertTrue(goals.head().sequent().antecedent().size() == 1 &&
                ...
                goals.head().sequent().succedent().iterator().
                        next().formula().op() == Quantifier.ALL, "First goal should be 'b==>b', but is " +
                goals.head().sequent());
        goals = goals.tail();
        assertTrue(goals.head().sequent().antecedent().size() == 0 &&
                goals.head().sequent().succedent().size() == 1 &&
                goals.head().sequent().succedent().iterator().
                        next().formula().op() == Quantifier.ALL, "Second goal should be '==>b', but is " +
                goals.head().sequent());
    ...
    public void testBugID177() {
        ...
        assertTrue(goals.head().sequent().antecedent().size() == 2 &&
                ...
                it.next().formula().equals(TacletForTests.parseTerm("B")), "Expected 'A, B ==>', but is " +
                goals.head().sequent());
    }
    ...
    private void doTestCatchList(int p_proof) {
        ...
        assertTrue(resultFormula.equalsModRenaming(correctFormula), "Wrong result. Expected:" + ProofSaver.printAnything(correctFormula, TacletForTests.services()) +
                " But was:" +
    ...
    public void testContextAdding() {
        ...
        assertTrue(expected.equalsModRenaming(is, new NameAbstractionTable()), "Expected:" + expected + "\n but was:" + is);
    }
    ...
    public void testAddExistingFormulaSucc() {
        ...
        assertEquals("A", goals.head().sequent().succedent().get(0).toString(), "Goal should be: ==> A, false");
        assertEquals("false", goals.head().sequent().succedent().get(1).toString(), "Goal should be: ==> A, false");


        assertEquals(1, goals.tail().head().sequent().antecedent().size(), "Goal should be: A ==> true");
        assertEquals(1, goals.tail().head().sequent().succedent().size(), "Goal should be: A ==> true");
        assertEquals("A", goals.tail().head().sequent().antecedent().getFirst().toString(), "Goal should be: A ==> true");
        assertEquals("true", goals.tail().head().sequent().succedent().getFirst().toString(), "Goal should be: A ==> true");
    ...
    public void testAddExistingFormulaAntec() {
        ...
        assertEquals(2, goals.tail().head().sequent().antecedent().size(), "Goal should be: true, A ==> ");
        assertEquals(0, goals.tail().head().sequent().succedent().size(), "Goal should be: true, A ==> ");
        assertEquals("A", goals.tail().head().sequent().antecedent().get(0).toString(), "Goal should be: true, A ==> ");
        assertEquals("true", goals.tail().head().sequent().antecedent().get(1).toString(), "Goal should be: true, A ==> ");
        ...
        assertEquals("false", goals.head().sequent().antecedent().get(0).toString(), "Goal should be: false ==> A");
        assertEquals("A", goals.head().sequent().succedent().get(0).toString(), "Goal should be: false ==> A");
    }
    /*
    "","A",
    "A","",
    ...
    */
    ...
    public void testAddExistingFormulaTwoInSucc() {
        ...
        assertEquals(0, goals.head().sequent().antecedent().size(), "Goal should be: ==> B, false, A");
        assertEquals(3, goals.head().sequent().succedent().size(), "Goal should be: ==> B, false, A");
        assertEquals("B", goals.head().sequent().succedent().get(0).toString(), "Goal should be:  ==> B, false, A");
        assertEquals("false", goals.head().sequent().succedent().get(1).toString(), "Goal should be:  ==> B, false, A");
        assertEquals("A", goals.head().sequent().succedent().get(2).toString(), "Goal should be:  ==> B, false, A");


        assertEquals(1, goals.tail().head().sequent().antecedent().size(), "Goal should be: B ==> true, A");
        assertEquals(2, goals.tail().head().sequent().succedent().size(), "Goal should be: B ==> true, A");
        assertEquals("B", goals.tail().head().sequent().antecedent().get(0).toString(), "Goal should be:B  ==> true, A");
        assertEquals("true", goals.tail().head().sequent().succedent().get(0).toString(), "Goal should be:B  ==> true, A");
        assertEquals("A", goals.tail().head().sequent().succedent().get(1).toString(), "Goal should be:B  ==> true, A");
    ...
    public void testAddExistingFormulaTwoInSucc2() {
        ...
        assertEquals(2, goals.size(), "Expected two goals.");
        assertEquals(0, goals.head().sequent().antecedent().size(), "Goal should be: ==> B, A, false");
        assertEquals(3, goals.head().sequent().succedent().size(), "Goal should be: ==> B, A, false");
        assertEquals("B", goals.head().sequent().succedent().get(0).toString(), "Goal should be:  ==> B, A, false");
        assertEquals("A", goals.head().sequent().succedent().get(1).toString(), "Goal should be:  ==> B, A, false");
        assertEquals("false", goals.head().sequent().succedent().get(2).toString(), "Goal should be:  ==> B, A, false");


        assertEquals(1, goals.tail().head().sequent().antecedent().size(), "Goal should be: A ==> B, true");
        assertEquals(2, goals.tail().head().sequent().succedent().size(), "Goal should be: A ==> B, true");
        assertEquals("A", goals.tail().head().sequent().antecedent().get(0).toString(), "Goal should be:A ==> B, true");
        assertEquals("B", goals.tail().head().sequent().succedent().get(0).toString(), "Goal should be:A ==> B, true");
        assertEquals("true", goals.tail().head().sequent().succedent().get(1).toString(), "Goal should be:A ==> B, true");
    ...
    public void testAddExistingFormulaTwoInAntec() {
        ...
        assertEquals(2, goals.head().sequent().antecedent().size(), "Goal should be: false, A ==> B ");
        assertEquals(1, goals.head().sequent().succedent().size(), "Goal should be: false, A ==> B");
        assertEquals("false", goals.head().sequent().antecedent().get(0).toString(), "Goal should be:  false, A ==> B");
        assertEquals("A", goals.head().sequent().antecedent().get(1).toString(), "Goal should be:  false, A ==> B");
        assertEquals("B", goals.head().sequent().succedent().get(0).toString(), "Goal should be:  false, A ==> B");


        assertEquals(3, goals.tail().head().sequent().antecedent().size(), "Goal should be: B, true, A ==>");
        assertEquals(0, goals.tail().head().sequent().succedent().size(), "Goal should be:  B, true, A ==>");
        assertEquals("B", goals.tail().head().sequent().antecedent().get(0).toString(), "Goal should be: B, true, A ==>");
        assertEquals("true", goals.tail().head().sequent().antecedent().get(1).toString(), "Goal should be: B, true, A ==>");
        assertEquals("A", goals.tail().head().sequent().antecedent().get(2).toString(), "Goal should be: B, true, A ==>");
    ...
    public void testAddExistingFormulaTwoInAntec2() {
        ...
        assertEquals(2, goals.head().sequent().antecedent().size(), "Goal should be: B, false ==> A ");
        assertEquals(1, goals.head().sequent().succedent().size(), "Goal should be: B, false  ==> A");
        assertEquals("B", goals.head().sequent().antecedent().get(0).toString(), "Goal should be:  B, false  ==> A");
        assertEquals("false", goals.head().sequent().antecedent().get(1).toString(), "Goal should be:  B, false  ==> A");
        assertEquals("A", goals.head().sequent().succedent().get(0).toString(), "Goal should be:  B, false  ==> A");


        assertEquals(3, goals.tail().head().sequent().antecedent().size(), "Goal should be: B, A, true  ==> ");
        assertEquals(0, goals.tail().head().sequent().succedent().size(), "Goal should be:   B, A, true  ==> ");
        assertEquals("B", goals.tail().head().sequent().antecedent().get(0).toString(), "Goal should be:  B, A, true  ==> ");
        assertEquals("A", goals.tail().head().sequent().antecedent().get(1).toString(), "Goal should be:  B, A, true  ==> ");
        assertEquals("true", goals.tail().head().sequent().antecedent().get(2).toString(), "Goal should be:  B, A, true  ==> ");
