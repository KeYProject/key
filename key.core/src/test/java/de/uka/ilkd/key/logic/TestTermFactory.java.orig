...
public class TestTermFactory {
   ...
   private TermFactory tf;
    ...
    @BeforeEach
	public void setUp() {
	Term et_x=new TermImpl(x, new ImmutableArray<>(), null, null, TermOrigin.EMPTY);
	Term et_px=new TermImpl(p, new ImmutableArray<>(et_x), null, null, TermOrigin.EMPTY);
	et1=et_px;       
	...
	Term invalidBuilt=new TermImpl(p, new ImmutableArray<>(new TermImpl(y, new ImmutableArray<>(), null, null, TermOrigin.EMPTY)), null, null, TermOrigin.EMPTY);
	try {
	...
	assertEquals(t_true, new TermImpl(Junctor.TRUE, new ImmutableArray<>(), null, null, TermOrigin.EMPTY));
    }

    @Test
	public void testQuantifierTerm() {
	Term t_forallx_px=TB.all(ImmutableSLList.<QuantifiableVariable>nil().append(x),t1());
	assertEquals(t_forallx_px, new TermImpl(Quantifier.ALL, new ImmutableArray<>(t1()), new ImmutableArray<>(x), null, TermOrigin.EMPTY));
    }

    @Test
	public void testJunctorTerm() {
	Term  t_px_imp_ryw= tf.createTerm(Junctor.IMP, t1(), t2());
	assertEquals(t_px_imp_ryw, new TermImpl(Junctor.IMP, new ImmutableArray<>(t1(), t2()), null, null, TermOrigin.EMPTY));
    }

    @Test
	public void testNegationTerm() {
	Term t_not_ryw=tf.createTerm(Junctor.NOT, t2());
	assertEquals(t_not_ryw, new TermImpl(Junctor.NOT, new ImmutableArray<>(t2()), null, null, TermOrigin.EMPTY));
    }
	...
	assertEquals(t_dia_ryw, new TermImpl(Modality.DIA, new ImmutableArray<>(t2()), null, jb, TermOrigin.EMPTY));
    }
	...
	assertEquals(t_dia_ryw, new TermImpl(Modality.BOX, new ImmutableArray<>(t2()), null, jb, TermOrigin.EMPTY));
    }
	...
	assertEquals(new TermImpl(WarySubstOp.SUBST, new ImmutableArray<>(t3(), t1()),
			new ImmutableArray<>(x), null, TermOrigin.EMPTY), t_x_subst_fy_in_px);
    }
	...
	assertTrue(exc instanceof TermCreationException, "Expected " + exc + " to be of type TermCreation but was: " +
