...
class TestParametricSorts {
    ...
    private ParametricSortDeclaration addParametricSort(String name, ParametricSortDeclaration.Variance... variance) {
        ImmutableList<ParametricSortDeclaration.SortParameter> params = ImmutableList.of();
        for (int i = 0; i < variance.length; i++) {
            ParametricSortDeclaration.Variance varia = variance[i];
            GenericSort genSort = (GenericSort) nss.sorts().lookup("G" + (i + 1));
            params = params.prepend(new ParametricSortDeclaration.SortParameter(genSort, varia));
        }
        ParametricSortDeclaration psd = new ParametricSortDeclaration(new Name(name), ImmutableSet.empty(), false,
                params, "", "");
    ...
    public void testParametricSortIdentical() {
        ParametricSortDeclaration psd = addParametricSort("List", ParametricSortDeclaration.Variance.COVARIANT);
    ...
    public void testParametricSortDependentFunctionInstantiation() {
        ...
        var listOfInt = ParametricSortInstance.get(psd, ImmutableList.of(intSort));
        var listOfG1 = ParametricSortInstance.get(psd, ImmutableList.of(g1));
        ...
        assertEquals("int", term.sub(0).op().sort(null).toString());
        assertSame(term.sub(0).sort(), term.sub(1).sort());
    }
