...
public class DocumentationGenerator {
    ...
    public static void main(String[] args) throws FileNotFoundException {
        ...
        Set<Map.Entry<String, ProofScriptCommand>> commands =
            ProofScriptEngine.loadCommands().entrySet();
        Map<String, List<Map.Entry<String, ProofScriptCommand>>> commandsByCategory =
            new TreeMap<>();
        ...
        List<Map.Entry<String, ProofScriptCommand>> uncategorized =
            commandsByCategory.get("Uncategorized");
    ...
    private static void listCategory(String category,
            List<Map.Entry<String, ProofScriptCommand>> proofScriptCommands) {
        ...
        for (Map.Entry<String, ProofScriptCommand> entry : proofScriptCommands) {
            ...
            if (entry.getKey().equals(entry.getValue().getName())) {
                ...
                System.out
                        .println("### <span style=\"color: var(--md-primary-fg-color);\"> Command `"
                            + command.getName() + "`</span>\n\n");
                System.out.printf(
                    "<span style=\"float:right;\">[Source](https://github.com/KeYProject/key/blob/%s/key.core/src/main/java/%s.java)</span>\n\n",
                    link,
                ...
                System.out
                        .println("### <span style=\"color: var(--md-primary-fg-color);\"> Command `"
                            + entry.getKey() + "`</span>\n");
                System.out.println("Alias for command [\u2192 `" + entry.getValue().getName()
                    + "`](#command-" + entry.getValue().getName() + ")\n");
            }
        }
    }
}
