...
public class TestPredicateConstruction {
    ...
    TestPredicateConstruction() {
        URL urlToTestFile = KeYResourceManager.getManager().getResourceFile(this, "test.key");// here
                                                                                              // we
                                                                                              // can
                                                                                              // define
                                                                                              // array
    ...
    private LoopInvariantGenerationResult generateResultNested(Sequent seq, Term succFormula,
            boolean relaxed) {
        if (!relaxed) {
            final LIGNestedMDarr loopInvGenerator = new LIGNestedMDarr(seq, services);
            return loopInvGenerator.generate();
        }
        final NestedLIGNewRelaxed loopInvGenerator =
            new NestedLIGNewRelaxed(seq, succFormula, services);
        return loopInvGenerator.generate();
    ...
    public LoopInvariantGenerationResult shiftArrayToLeft(boolean relaxed) {
        ...
        try {
            succFormula = parse(
                "{i:=0}\\<{" + "		while (i<=a.length-2) {a[i] = a[i+1];" + "			i++;}"
                    + "		}\\>true");
        } catch (Exception e) {
        ...
        Sequent seq = Sequent.EMPTY_SEQUENT.addFormula(new SequentFormula(succFormula), false, true)
                .sequent();

        String[] arrLeft;
        if (!relaxed) {
            arrLeft = new String[] { "noW(arrayRange(a,0,a.length-1))",
                "noR(arrayRange(a,0,a.length-1))", "a.length > 10" };
        } else {
            arrLeft = new String[] { "relaxedNoW(arrayRange(a,0,a.length-1))",
                "relaxedNoR(arrayRange(a,0,a.length-1))", "a.length > 10" };
        }
    ...
    public LoopInvariantGenerationResult withFunc(boolean relaxed) {
        ...
        try {
            succFormula = parse(
                "{i:=0}\\<{" + "			while (i<=a.length-1) {a[i] = Object.arrayFct(a[i]);"
                    + "			i++;}"
                    + "		}\\>true");
        } catch (Exception e) {
    ...
    public LoopInvariantGenerationResult indexToPowerOf2(boolean relaxed) {
        ...
        try {
            succFormula =
                parse("{i:=0}\\<{" + "			while (i<=a.length-1) {a[i] = a[i^2];"
                    + "			i++;}"
                    + "		}\\>true");
        } catch (Exception e) {
            System.out.println(e.getMessage());
    ...
    public LoopInvariantGenerationResult withoutFunc(boolean relaxed) {
        ...
        try {
            succFormula =
                parse("{i:=0}\\<{" + "			while (i<=a.length-1) {a[i] = a[i]+1;"
                    + "			i++;}"
                    + "		}\\>true");
        } catch (Exception e) {
            System.out.println(e.getMessage());
    ...
    public LoopInvariantGenerationResult onlyRead(boolean relaxed) {
        ...
        try {
            succFormula =
                parse("{i:=0}\\<{"
                    + "			while (i<=a.length-1) { if(cond) {f(a[i])}; else{} ;"
                    + "			i++;}"
                    + "		}\\>true");
        } catch (Exception e) {
            System.out.println(e.getMessage());
    ...
    public LoopInvariantGenerationResult intaDepOnly(boolean relaxed) {
        ...
        if (!relaxed) {
            ...
            arrLeft = new String[] { "relaxedNoW(arrayRange(a,0,a.length-1))",
                "relaxedNoR(arrayRange(a,0,a.length-1))", "a.length > 10", "sum = 0" };
        }
        String[] arrRight = { "a=null" };
        try {
            for (String fml : arrLeft) {
    ...
    public LoopInvariantGenerationResult stencil(boolean relaxed) {
        ...
        try {
            succFormula = parse("{i:=1}\\<{"
                + "			while (i<a.length-1) {a[i] = a[i-1] + a[i+1];" + "			i++;}"
                + "		}\\>true");
        } catch (Exception e) {
            // TODO Auto-generated catch block
            System.out.println(e.getMessage());
    ...
    // return null;
    // }
    // Sequent seq = Sequent.EMPTY_SEQUENT.addFormula(new SequentFormula(succFormula), false,
    // true).sequent();
    //
    // String[] arrLeft = { "noW(arrayRange(a,0,a.length))","noR(arrayRange(a,0,a.length))",
    // "a.length > 10" };
    ...
    public LoopInvariantGenerationResult condition(boolean relaxed) {
        ...
        MergePointInline inlineMergePoints =
            new MergePointInline(formula.sub(1).javaBlock().program(), false, services);
        ...
        // recreate formula
        formula = tb.apply(formula.sub(0),
            tb.dia(JavaBlock.createJavaBlock((StatementBlock) s), tb.tt()));
        // System.out.println("Formula with merge point: "+ProofSaver.printAnything(formula,
        // services));

        Sequent seq =
            Sequent.EMPTY_SEQUENT.addFormula(new SequentFormula(formula), false, true).sequent();

        String[] arrLeft;
        if (!relaxed) {
            arrLeft = new String[] { "noR(arrayRange(a,0,a.length))",
                "noW(arrayRange(a,0,a.length))", "a.length>10" };
        } else {
            arrLeft = new String[] { "relaxedNoR(arrayRange(a,0,a.length))",
                "relaxedNoW(arrayRange(a,0,a.length))", "a.length>10" };
        }
        String[] arrRight = { "a=null" };
        try {
            for (String fml : arrLeft) {
    ...
    public LoopInvariantGenerationResult conditionDifferentNumberOfEvents(boolean relaxed) {
        ...
        String[] arrLeft;
        if (!relaxed) {
            arrLeft = new String[] { "noW(arrayRange(a,0,a.length))",
                "noR(arrayRange(a,0,a.length))", "a.length>10" };
        } else {
            arrLeft = new String[] { "relaxedNoW(arrayRange(a,0,a.length))",
                "relaxedNoR(arrayRange(a,0,a.length))", "a.length>10" };
        }
        String[] arrRight = { "a=null" };
        try {
            for (String fml : arrLeft) {
    ...
    // return;
    // }
    // Sequent seq = Sequent.EMPTY_SEQUENT.addFormula(new SequentFormula(formula), false,
    // true).sequent();
    //
    ...
    public LoopInvariantGenerationResult basicEx0() {// Change length of arrays in
                                                     // AbstractLoopInvariantGenerator to 1
        ...
        String[] arrLeft = { "noW(arrayRange(a,0,a.length-1))", "noR(arrayRange(a,0,a.length-1))",
            "a.length > 10" };
        String[] arrRight = { "a=null" };
        try {
            for (String fml : arrLeft) {
    ...
    public LoopInvariantGenerationResult basicMltpArrDiffIndex() {
        ...
        String[] arrLeft = { "a.length=b.length", "noW(arrayRange(a,0,a.length-1))",
            "noR(arrayRange(a,0,a.length-1))", "a.length > 10", "noW(arrayRange(b,0,b.length-1))",
            "noR(arrayRange(b,0,b.length-1))", "b.length > 10" };
    ...
    public LoopInvariantGenerationResult basicMDArray() {
        ...
        String[] arrLeft = {
            ...
            "a.length = b.length" };
        String[] arrRight =
            { "a=null", "b=null", "a=b", "\\forall int r;(\\forall int s;(a[r]=b[s]))" };
        try {
            for (String fml : arrLeft) {
                seq = seq.addFormula(new SequentFormula(parse(fml)), true, true).sequent();
            }
    ...
    // ==================================================Correlation=========================================================
    public LoopInvariantGenerationResult correlation_init_array(boolean relaxed) {// Change length
                                                                                  // of arrays in
                                                                                  // AbstractLoopInvariantGenerator
                                                                                  // to 1
        ...
        String[] arrLeft;
        if (!relaxed) {
            arrLeft = new String[] { "wellFormed(heap)", "a.<created>=TRUE",
                "wellFormedMatrix(a, heap)", "noW(arrayRange(a,0,a.length-1))",
                "noW(matrixRange(heap,a,0,N-1,0,M-1))", "noR(matrixRange(heap,a,0,N-1,0,M-1))",
                "a.length > N", "a[0].length > M", "N >10", "M >10" };
        } else {
            arrLeft = new String[] { "wellFormed(heap)", "a.<created>=TRUE",
                "wellFormedMatrix(a, heap)", "relaxedNoW(arrayRange(a,0,a.length-1))",
                "relaxedNoW(matrixRange(heap,a,0,N-1,0,M-1))",
                "relaxedNoR(matrixRange(heap,a,0,N-1,0,M-1))",
                "a.length > N", "a[0].length > M", "N >10", "M >10" };
        }
    ...
    public LoopInvariantGenerationResult three_nested_loops(boolean relaxed) {// Change length of
                                                                              // arrays in
                                                                              // AbstractLoopInvariantGenerator
                                                                              // to 1
    ...
    public LoopInvariantGenerationResult correlation_print_array(boolean relaxed) {// Change length
                                                                                   // of arrays in
                                                                                   // AbstractLoopInvariantGenerator
                                                                                   // to 1
    ...
    public LoopInvariantGenerationResult gem_ver_scope_1(boolean relaxed) {// Change length of
                                                                           // arrays in
                                                                           // AbstractLoopInvariantGenerator
                                                                           // to 1
    ...
    public static void main(String[] args) {
        ...
        // result = tpc.interAndIntra(false);//New. Normal misses noR(a[0]). Relaxed works. But
        // noR(a[0]) is missing.
        ...
        // result = tpc.conditionWithDifferentEvents(false); //Change the s0 in LIGNew.Normal misses
        // noW(a[0]). Precise Result except that it doesn't have the noWaR(a[1]). Because we don't
        // allow breaking the array more than once. Relaxed works.
        // result = tpc.shiftArrayToLeftWithBreak(false);//Normal works. Relaxed works.
        // result = tpc.stencil(false); //Change the s0 in LIGNew. Normal works. Precise Result
        // except that it doesn't have the noWaR(a[1]). Because we don't allow breaking the array
        // more than once. Relaxed works.
