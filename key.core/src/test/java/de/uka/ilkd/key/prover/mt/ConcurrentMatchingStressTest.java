...
public class ConcurrentMatchingStressTest {
    ...
    /**
     * Loading a .key file applies its embedded settings to the global
    ...
    private static ProofFingerprint prove(String file, boolean parallel) throws Exception {
        // Pin the property in BOTH directions: with an inherited -Dkey.prover.parallel=true the
        // single-threaded baseline would otherwise silently run multi-core, making the MT-vs-SC
        // comparison vacuous.
        System.setProperty(ParallelProver.PARALLEL_PROPERTY, Boolean.toString(parallel));
        if (parallel) {
