...
class Instantiation {
    ...
    private Instantiation(Term allterm, Sequent seq, Services services,
        ...
        // written-array-index candidates are part of the theory-aware selection, dropped in classic
        if (!treatment.isClassic()) {
            addWrittenArrayIndexInstances((JTerm) matrix, services);
        }
    }

    /**
     * Instance candidates taken from written array indices. Where the matrix reads an array
     * through a store chain, {@code select(... store(h, o, arr(c), v) ..., o, arr(j))}, the
     * written index {@code c} is a candidate for the quantified {@code j}: instantiating with it
     * collapses the select by the select-over-store rules. No trigger produces it, since {@code c}
     * holds no quantified variable. The candidates are costed like matched ones, so useless ones
     * are ranked down as usual.
     */
    private void addWrittenArrayIndexInstances(JTerm term, Services services) {
        ...
        for (int i = 0; i < term.arity(); i++) {
            addWrittenArrayIndexInstances(term.sub(i), services);
        }
    }

    /** Adds every ground array index written on {@code obj}'s array fields in {@code heap}. */
