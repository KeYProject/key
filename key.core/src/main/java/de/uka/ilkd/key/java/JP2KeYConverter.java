...
class JP2KeYVisitor extends GenericVisitorAdapter<Object, Void> {
    ...
    public TypeReference visit(PrimitiveType n, Void arg) {
        ...
        switch (n.getType()) {
            case KEY_BIGINT:
                return new TypeRef(ji.getPrimitiveKeYJavaType(de.uka.ilkd.key.java.abstraction.PrimitiveType.JAVA_BIGINT));
            case KEY_LOCSET:
                return new TypeRef(ji.getPrimitiveKeYJavaType(de.uka.ilkd.key.java.abstraction.PrimitiveType.JAVA_LOCSET));
            case KEY_REAL:
                return new TypeRef(ji.getPrimitiveKeYJavaType(de.uka.ilkd.key.java.abstraction.PrimitiveType.JAVA_REAL));
            case KEY_MAP:
                return new TypeRef(ji.getPrimitiveKeYJavaType(de.uka.ilkd.key.java.abstraction.PrimitiveType.JAVA_MAP));
            case KEY_SEQ:
                return new TypeRef(ji.getPrimitiveKeYJavaType(de.uka.ilkd.key.java.abstraction.PrimitiveType.JAVA_SEQ));
        }
    ...
    private ImmutableArray<Expression> map(Optional<NodeList<com.github.javaparser.ast.expr.Expression>> arguments) {
        return arguments.<ImmutableArray<Expression>>map(this::map).orElseGet(ImmutableArray::new);
    }
    ...
    public Object visit(KeyMetaConstruct n, Void arg) {
        ...
        switch (mcName) {
            ...
            case "#resolve-multiple-var-decl":
                return new MultipleVarDecl((SchemaVariable) n.getChild().accept(this, arg)); //TODO weigl inspect this special case
