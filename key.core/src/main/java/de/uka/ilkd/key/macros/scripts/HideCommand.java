...
public class HideCommand extends AbstractCommand<HideCommand.Parameters> {
    ...
    public void execute(Parameters args) throws ScriptException, InterruptedException {
        ...
        for (SequentFormula s : args.sequent.antecedent()) {
            ...
            app = app.addCheckedInstantiation(sv, s2.formula(), Objects.requireNonNull(service), true);
        ...
        Taclet hideRight =
                Objects.requireNonNull(state).getProof().getEnv().getInitConfigForEnvironment().lookupActiveTaclet(HIDE_RIGHT);
