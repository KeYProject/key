...
public final class ParametricSortInstance extends AbstractSort {
    /**
     * Interns parametric sort instances so that equal instances are the same object. Thread-safe
     * (the previous {@code WeakHashMap} + check-then-put could hand two distinct-but-equal
     * instances
     * to concurrent workers, breaking that identity). The miss action -- registering a genuinely
     * new
     * sort in the namespace -- runs inside {@link WeakValueInterner#intern}, so it happens exactly
     * once per new sort and is serialized with the interning.
     */
