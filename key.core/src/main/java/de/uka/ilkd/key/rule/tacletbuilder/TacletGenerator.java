...
public class TacletGenerator {
    ...
    public Taclet generateRelationalRepresentsTaclet(Name tacletName, Term originalAxiom,
        ...
        for (SchemaVariable heapSV : heapSVs) { vars = vars.append(tb.var(heapSV)); }
        if (!target.isStatic()) { vars = vars.append(tb.var(selfSV)); }
        for (SchemaVariable sv : paramSVs) { vars = vars.append(tb.var(sv)); }
    ...
    public ImmutableSet<Taclet> generateContractAxiomTaclets(Name name, Term originalPre,
        ...
        int i = 0;
        for (SchemaVariable heapSV : heapSVs) { subs[i++] = TB.var(heapSV); }
    ...
    public ImmutableSet<Taclet> generatePartialInvTaclet(Name name, List<SchemaVariable> heapSVs,
        ...
        i = 0;
        for (SchemaVariable heapSV : heapSVs) { hs[i++] = TB.var(heapSV); }
        // create taclet
