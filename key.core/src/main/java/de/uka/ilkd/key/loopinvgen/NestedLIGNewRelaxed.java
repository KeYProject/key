...
public class NestedLIGNewRelaxed extends AbstractLoopInvariantGenerator {
    ...
    public NestedLIGNewRelaxed(Sequent sequent, Term formula, Services services) {
        ...
        this.outerLoop = (LoopStatement) JavaTools
                .getActiveStatement(TermBuilder.goBelowUpdates(formula).javaBlock());
        initialization();
    }

    public NestedLIGNewRelaxed(Sequent sequent, LoopStatement loopStatement, Services services,
            Term[] arrays, Term outI, Term outL, Term outH, Term inI, Term inL, Term inH) {
        super(sequent, services);
    ...
    public LoopInvariantGenerationResult generate() {
        ...
        allDepPreds.add(tb.relaxedNoR(
            tb.matrixRange(tb.getBaseHeap(), arrays[0], lowOuter, highOuter, lowInner, highInner)));
        allDepPreds.add(tb.relaxedNoW(
            tb.matrixRange(tb.getBaseHeap(), arrays[0], lowOuter, highOuter, lowInner, highInner)));
        allDepPreds.add(tb.relaxedNoW(
            tb.arrayRange(arrays[0], tb.zero(), tb.sub(tb.dotLength(arrays[0]), tb.one()))));


        int itrNumber = -1;
        PredicateRefiner pr0 = new LoopIndexAndDependencyPredicateRefiner(currentGoal.sequent(),
            allDepPreds, allCompPreds, indexOuter,
        ...
        do {
            ...
            ImmutableList<Goal> goalsAfterUnwind =
                ruleApp.applyLoopScopeUnwindRule(goalsAfterShift);
            ...
            if (!(ls.equalsModProperty(outerLoop,
                RenamingSourceElementProperty.RENAMING_SOURCE_ELEMENT_PROPERTY,
                new NameAbstractionTable()))) {
                System.out.println("Inner Loop detected");
                Sequent innerSeq = modifySeq(currentGoal, ls);

                NestedLIGNewRelaxed innerLoopGen = new NestedLIGNewRelaxed(innerSeq, ls, services,
                    arrays, indexOuter, lowOuter, highOuter, indexInner, lowInner, highInner);
                ...
                LoopSpecification loopSpec =
                    new LoopSpecificationImpl(ls, tb.and(result.getConjuncts()));

                services.getSpecificationRepository().addLoopInvariant(loopSpec);

                // System.out.println("============Goals After Shift============ " +
                // goalsAfterShift);
                ImmutableList<Goal> goalsAfterInnerLoop =
                    ruleApp.applyNestedLoopUsecaseRule(goalsAfterShift);
                // System.out.println("============Goals After Inner Loop============ " +
                // goalsAfterInnerLoop);

                currentGoal = ruleApp.findLoopUnwindTacletGoal(
                    ruleApp.applyShiftUpdateRule(goalsAfterInnerLoop));
            }
    ...
    private Set<Term> filterConjuncts(LoopInvariantGenerationResult result) {
        ...
        for (Term conjunct : result.getConjuncts()) {
            ...
            // collector.ops()
            if (!collector
                    .contains(services.getTypeConverter().getDependenciesLDT().getTimestamp())) {
    ...
    private Sequent modifySeq(Goal g, LoopStatement innerLoop) {
        ...
        initialFormulas.add(tb.relaxedNoR(
            tb.matrixRange(tb.getBaseHeap(), arrays[0], lowOuter, highOuter, lowInner, highInner)));
        initialFormulas.add(tb.relaxedNoW(
            tb.matrixRange(tb.getBaseHeap(), arrays[0], lowOuter, highOuter, lowInner, highInner)));
        initialFormulas.add(tb.relaxedNoW(
            tb.arrayRange(arrays[0], tb.zero(), tb.sub(tb.dotLength(arrays[0]), tb.one()))));
        ...
        Term post = tb.tt();
        Term programFormula =
            tb.dia(JavaBlock.createJavaBlock(new StatementBlock(innerLoop)), post);
    ...
    private Semisequent filterSemisequent(Semisequent semisequent) {
        ...
        filter: for (SequentFormula sf : semisequent) {
