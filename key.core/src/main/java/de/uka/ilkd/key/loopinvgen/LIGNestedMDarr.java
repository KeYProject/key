...
public class LIGNestedMDarr extends AbstractLoopInvariantGenerator {
    ...
    public LIGNestedMDarr(Sequent sequent, Services services, List<LocationVariable> indexes,
            int nrArrays) {
    ...
    public LoopInvariantGenerationResult generate() {
        ...
        outerDepPreds.add(tb.noR(
            tb.matrixRange(tb.getBaseHeap(), arrays[0], lowOuter, highOuter, lowInner, highInner)));
        outerDepPreds.add(tb.noW(
            tb.matrixRange(tb.getBaseHeap(), arrays[0], lowOuter, highOuter, lowInner, highInner)));
        outerDepPreds.add(
            tb.noW(tb.arrayRange(arrays[0], tb.zero(), tb.sub(tb.dotLength(arrays[0]), tb.one()))));
        ...
        innerDepPreds.add(tb.noR(
            tb.matrixRange(tb.getBaseHeap(), arrays[0], lowOuter, highOuter, lowInner, highInner)));
        innerDepPreds.add(tb.noW(
            tb.matrixRange(tb.getBaseHeap(), arrays[0], lowOuter, highOuter, lowInner, highInner)));
        innerDepPreds.add(
            tb.noW(tb.arrayRange(arrays[0], tb.zero(), tb.sub(tb.dotLength(arrays[0]), tb.one()))));
        ...
        // if(arrays.length==1)
        // prInner = new
        // NestedLoopIndexAndDependencyPredicateRefiner(goalsAfterShift.head().sequent(),
        ...
        do {
            ...
            // System.out.println("Goal after Shifting Outer Loop's Updates: " +
            // goalsAfterShiftUpdate.head());
                    ...
                    if (formula.op() instanceof Modality mod
                        ...
                        if (activePE instanceof While) {
                            ...
                            if (firstApproach && !once) {
                                ...
                                innerLoop = (LoopStatement) activePE;
                                SideProof innerLoopProof =
                                    new SideProof(services, modifySeqFor2ndApproach(g));
                                innerLI =
                                    innerLIComputation(innerLoopProof.retGoal().head(), activePE);// For
                                                                                                  // now
                                                                                                  // only
    ...
    private LoopInvariantGenerationResult innerLIComputation(Goal g, Statement innerLoop) {
        ...
        LIGNewInner innerLIG = new LIGNewInner(newSeq, services, innerDepPreds, innerCompPreds,
            indexOuter, indexInner, arrays.length);
