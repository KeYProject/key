...
public class LIGNestedMltpArr extends AbstractLoopInvariantGenerator {
    ...
    public LoopInvariantGenerationResult generate() {
        ...
        boolean once = false; // In the first approach the inner loop's invariant should only be
                              // calculated once
    ...
    private LoopInvariantGenerationResult innerLIComputation(Goal g, int itrNumber,
        ...
        Term newDiamond = tb.dia(jb, tb.tt()); // "tb.tt()" should be the post condition we want to
                                               // prove
        ...
        LIGNewInnerMltpArr innerLIG = new LIGNewInnerMltpArr(newSeq, services, allDepPreds,
            innerCompPreds, arrays[0], arrays[1], indexOuter, indexInner);
