 ...
 * <code>XXX</code> being Long, Int, etc.) is in the range of Long, Integer etc.
 */
public class IsInRangeProvable implements Feature {
     ...
     */
    private ImmutableSet<Term> extractAssumptionsFrom(final Semisequent semisequent,
            boolean negated, ImmutableSet<Term> assumptions, final HashSet<Operator> ops,
            final SequentFormula formulaToIgnore, Services services) {
    ...
    /**
     * helper methods to filter out the sequent formulas of interest, here those formulas with an
     * equality between ints or any of the operators in ops as top level operator
     *
     ...
     */
    private boolean filterSequent(final HashSet<Operator> ops, final IntegerLDT integerLDT,
            final Term formula) {
