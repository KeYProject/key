 ...
 * bookkeeping (variable-length prefix descent, inner execution context, prefix/suffix positions) is
 * still performed by {@link ContextStatementBlock#match}; only the matching of the active
 * statements
 * is delegated to the supplied VM sub-program, replacing the monolithic
 * {@code MatchProgramInstruction}
 * for that subtree. The current element is the modality's {@link JavaBlock} (as for
 * {@code MatchProgramInstruction}).
 *
