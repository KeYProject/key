...
public class MergeRule implements BuiltInRule {
    ...
    protected Triple<SymbolicExecutionState, LinkedHashSet<Name>, LinkedHashSet<Term>> mergeStates(
        ...
        while (!eventUpdatesState1.isEmpty()) {
            ...
            final Term locset =
                MergeByIfThenElse.createIfThenElseTerm(state1, state2, evUpd1.sub(1),
                    locset2, switched ? tb.not(distinguishingFormula) : distinguishingFormula,
                    services);
            final Term timestamp =
                MergeByIfThenElse.createIfThenElseTerm(state1, state2, evUpd1.sub(2),
                    timestamp2, switched ? tb.not(distinguishingFormula) : distinguishingFormula,
                    services);

            newElementaryUpdates =
                newElementaryUpdates.append(tb.eventUpdate(kind, locset, timestamp));
