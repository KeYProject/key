...
class ExprEvaluator {
    ...
    public void addConvertersToValueInjector(ValueInjector v) {
        ...
        v.addConverter(ScriptBlock.class, ProofScriptExpressionContext.class,
            this::convertToScriptBlock);

    }

    private ScriptBlock convertToScriptBlock(ProofScriptExpressionContext ctx)
            throws ConversionException {
        if (ctx.proofScriptCodeBlock() == null) {
            throw new ConversionException(
                "Need a script block here, not: " + ANTLRUtil.reconstructOriginal(ctx));
        }
    ...
    private JTerm convertToTerm(ProofScriptExpressionContext ctx) throws ConversionException {
        try {
            if (ctx.string_literal() != null) {
                ...
                return (JTerm) evaluateExpression(
                    (ParserRuleContext) ctx.getChild(ParserRuleContext.class, 0));
            }
        } catch (Exception e) {
            throw new ConversionException(
                "Cannot convert expression to term: " + ANTLRUtil.reconstructOriginal(ctx), e);
        }
    }
        ...
        try {
            if (ctx.string_literal() != null) {
                ...
                return JavaDLSequentKit
                        .createSuccSequent(ImmutableList.of(new SequentFormula(term)));
            }
        } catch (Exception e) {
            throw new ConversionException(
                "Cannot convert expression to sequent: " + ANTLRUtil.reconstructOriginal(ctx));
        }
    }
    ...
    private Boolean convertToBoolean(ProofScriptExpressionContext ctx) throws ConversionException {
        if (ctx.boolean_literal() != null) {
            ...
            throw new ConversionException(
                "Cannot convert expression to boolean: " + ANTLRUtil.reconstructOriginal(ctx));
        }
    }

    private Integer convertToInteger(ProofScriptExpressionContext proofScriptExpressionContext)
            throws ConversionException {
        if (proofScriptExpressionContext.integer() != null) {
            ...
            String text =
                StringUtil.trim(proofScriptExpressionContext.string_literal().getText(), '"');
            return Integer.parseInt(text);
        } else {
            throw new ConversionException("Cannot convert expression to integer: "
                + ANTLRUtil.reconstructOriginal(proofScriptExpressionContext));
        }
    }
