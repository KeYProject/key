...
public class NestedLoopIndexAndDependencyPredicateRefiner2 extends PredicateRefiner {
    ...
    public NestedLoopIndexAndDependencyPredicateRefiner2(Sequent sequent, Set<Term> depPredList,
            Set<Term> compPredList, Term arrOuter, Term arrInner,
            Term indexOuter, Term indexInner, int iteration, boolean innerLoop, Term m,
            Services services) {
        ...
        if (!sProof.proofEquality(outLow, outHigh)) {
            ...
            if (sProof.proofLT(tb.zero(), newOutHigh)) {
                if (sProof.proofLT(newOutLow, newOutHigh)) {
                    subLoc = tb.infiniteUnion(new QuantifiableVariable[] { l },
                        tb.and(tb.geq(tb.var(l), newOutLow), tb.leq(tb.var(l), newOutHigh)),
        ...
        if (locSet != null) {
            ...
            if (!sProof.proofEquality(outLow, indexInner)) {
                ...
                if (!sProof.proofEquality(indexInner, outHigh)) {
                    innerToHigh = tb.infiniteUnion(new QuantifiableVariable[] { l },
                        tb.and(tb.geq(tb.var(l), indexInner), tb.leq(tb.var(l), outHigh)),
                        tb.arrayRange(tb.dotArr(array, tb.var(l)), inLow, inHigh));
                } else {
            ...
            if (!sProof.proofEquality(outLow, indexOuter)) {
                lowToOuter = tb.infiniteUnion(new QuantifiableVariable[] { l },
                    tb.and(tb.geq(tb.var(l), outLow), tb.leq(tb.var(l), indexOuter)),
                    tb.arrayRange(tb.dotArr(array, tb.var(l)), inLow, inHigh));
                if (!sProof.proofEquality(indexOuter, outHigh)) {
                    outerToHigh = tb.infiniteUnion(new QuantifiableVariable[] { l },
                        tb.and(tb.geq(tb.var(l), indexOuter), tb.leq(tb.var(l), outHigh)),
                        tb.arrayRange(tb.dotArr(array, tb.var(l)), inLow, inHigh));
                } else {
