...
public class FinalFieldsPOExtension implements POExtension {
    ...
    @Override
    public Term modifyPostTerm(InitConfig proofConfig, Services services, ProgramVariable selfVar,
            Term postTerm) {

        Choice choice = proofConfig.choiceNS().lookup("finalFields:immutable");
        if (!proofConfig.getActivatedChoices().contains(choice)) {
            return postTerm;
        }
