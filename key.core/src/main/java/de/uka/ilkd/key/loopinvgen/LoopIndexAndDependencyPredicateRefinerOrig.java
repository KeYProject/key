...
public class LoopIndexAndDependencyPredicateRefinerOrig extends PredicateRefiner {
    ...
    public LoopIndexAndDependencyPredicateRefinerOrig(Sequent sequent, Set<Term> depPredList,
    ...
    private Set<Term> weakenBySubSetForMatrixRangeOrig(Term unProven) {
        ...
        if (!sProof.proofEquality(inLow, inHigh)) {
            // final Term lowArr = tb.matrixRange(heap, arr , outLow, outHigh, inLow,
            // inLow);//=empty
            // final Term highArr = tb.matrixRange(heap, arr , outLow, outHigh, inHigh,
            // inHigh);//=empty
    ...
    //// System.out.println("because "+ seqLabel2 + " is less then " + " there is a " +
    // sequentFormula1 + " after "+ sequentFormula2);
    // }
    // }
    // }
    ...
    private Term findArrayRange(Term locSet) {
        ...
        // else if(locSet.op()==locsetLDT.getUnion() || locSet.op()==locsetLDT.getIntersect() ||
        // locSet.op()==locsetLDT.getSetMinus()) {
            ...
            if (array != null && low != null && high != null && index != null) {
                ...
                if (lowToI != null && iToHigh != null) {
                    if (depLDT.isDependencePredicate(pred.op())) {
                        ...
                        if (dependencyOp == depLDT.getRelaxedNoRaW()
                                || dependencyOp == depLDT.getRelaxedNoWaR()) {
                            result.add(
                                tb.func(dependencyOp, lowToI, tb.empty(), tb.empty(), tb.empty()));
                            result.add(
                                tb.func(dependencyOp, iToHigh, tb.empty(), tb.empty(), tb.empty()));
    ...
    private Set<Term> weakenByIndexesANDPredicateForMatrixRange(Term pred) {
        ...
        if (!outLow.equalsModProperty(indexOuter, RenamingTermProperty.RENAMING_TERM_PROPERTY)) {
            ...
            if (!indexOuter.equalsModProperty(outHigh,
                RenamingTermProperty.RENAMING_TERM_PROPERTY)) {
                outerToHigh = tb.matrixRange(heap, arr, indexOuter, outHigh, inLow, inHigh);
            } else {
                outerToHigh = tb.matrixRange(heap, arr, indexOuter, indexOuter, inLow, inHigh);// matrixRange(heap,
                                                                                               // arr,
                                                                                               // indexOuter,
                                                                                               // indexOuter,
                                                                                               // inLow,
            ...
            lowToOuter = tb.matrixRange(heap, arr, indexOuter, indexOuter, inLow, inHigh); // tb.arrayRange(arr,
                                                                                           // inLow,
                                                                                           // inHigh);//
            if (!indexOuter.equalsModProperty(outHigh,
                RenamingTermProperty.RENAMING_TERM_PROPERTY)) {
        ...
        if (lowToOuter != null && outerToHigh != null) {
            if (depLDT.isDependencePredicate(pred.op())) {
                ...
                if (!lowToOuter.sub(2).equalsModProperty(lowToOuter.sub(3),
                    RenamingTermProperty.RENAMING_TERM_PROPERTY)) {
                ...
                if (!outerToHigh.sub(2).equalsModProperty(outerToHigh.sub(3),
                    RenamingTermProperty.RENAMING_TERM_PROPERTY)) {
                    result.add(tb.func(dependencyOp,
                        tb.matrixRange(outerToHigh.sub(0), outerToHigh.sub(1),
        ...
        if (locSet != null) {
            ...
            if (lowToOuter != null && outerToHigh != null) {
                if (depLDT.isDependencePredicate(pred.op())) {
                    ...
                    // result.add(tb.func(dependencyOp, tb.matrixRange(outerToHigh.sub(0),
                    // outerToHigh.sub(1),
                    // tb.add(outerToHigh.sub(2), tb.one()),
                    // outerToHigh.sub(3),
