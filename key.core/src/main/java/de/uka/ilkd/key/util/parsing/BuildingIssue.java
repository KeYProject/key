...
public record BuildingIssue(String message,@Nullable Throwable cause,boolean isWarning,Position position,@Nullable String sourceName){
...
private static BuildingIssue fromToken(String message,boolean isWarning,@Nullable Token token,@Nullable Throwable cause){if(token!=null){var position=Position.fromToken(token);return new BuildingIssue(message,cause,isWarning,position,token.getTokenSource().getSourceName());}return new BuildingIssue(message,cause,isWarning,Position.UNDEFINED,null);}
...
public PositionedString asPositionedString(){try{return new PositionedString(message,new Location(new URI(sourceName),position));}catch(URISyntaxException e){return null;}}}
