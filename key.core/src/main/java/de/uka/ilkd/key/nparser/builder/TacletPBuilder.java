...
public class TacletPBuilder extends ExpressionBuilder {
    ...
    public Object visitGoalspec(KeYParser.GoalspecContext ctx) {
        ...
        if (ctx.addrules() != null)
            addedRules = accept(ctx.addrules()); // modifies goalChoice
        if (ctx.addprogvar() != null)
            addedProgramVariables = accept(ctx.addprogvar());
    ...
    private void addGoalTemplate(
            ...
            ImmutableSet<SchemaVariable> pvs, @Nullable ChoiceExpr soc, ParserRuleContext ctx) {
        TacletBuilder<?> b = peekTBuilder();
