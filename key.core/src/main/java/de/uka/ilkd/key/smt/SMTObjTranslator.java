...
public class SMTObjTranslator implements SMTTranslator {
    ...
    private void generateWellFormedAssertions() throws IllegalFormulaException {
        ...
        for (Sort s : thierarchy.getArraySortList()) {
            ...
            SMTTerm typeReq;
            switch (single) {
                case "int", "char", "byte" -> typeReq = SMTTerm.call(getIsFunction(sorts.get(BINT_SORT)), selectArr);
                case "java.lang.Object" -> typeReq = SMTTerm.call(getIsFunction(sorts.get(OBJECT_SORT)), selectArr);
