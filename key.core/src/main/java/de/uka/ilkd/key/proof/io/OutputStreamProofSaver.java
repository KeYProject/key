    ...
    public void save(OutputStream out) throws IOException {
        ...
        try (var ps = new PrintWriter(out, true, StandardCharsets.UTF_8)) {
            ...
            for (var fs : proof.getServices().getNamespaces().functions().allElements()) {
                if (
                /*
                 * !proof.getEnv().getInitConfigForEnvironment().namespaces().functions().contains(
                 * fs) &&
                 */ fs.arity() == 0) {
                    functions.append(
                        fs.sort() + " " + fs.name() + "; //" + fs.isSkolemConstant() + "\n");
                }
            }
            functions.append("}\n");
            ...
            for (var pv : proof.getServices().getNamespaces().programVariables().allElements()) {
                if (true || proof.getEnv().getInitConfigForEnvironment().namespaces()
                        .programVariables().contains(pv)) {
