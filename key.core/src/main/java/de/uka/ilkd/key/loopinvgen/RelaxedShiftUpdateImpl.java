...
public class RelaxedShiftUpdateImpl {
    ...
    private Term generateRenameUpdate(Term loopFormula) {
        ...
        for (UpdateableOperator lhs : updatedLocations) {
            ...
            final Name freshConsSymb =
                new Name(tb.newName("f_" + lhs.name().toString(), services.getNamespaces()));
        ...
        final Term updateRenameAndInverseEvents =
     ...
     * @param counter the {@link Term} is the iteration number for all because we don't want to
     *        capture the dependences inside an iteration
     */
