...
public class ShiftUpdateImplNew {
    ...
    public void shiftUpdate(Goal g, PosInOccurrence pos) {
        ...
        g.addFormula(new SequentFormula(UpdateApplication.getTarget(loopFormula)), pos.isInAntec(),
            true);
     ...
     *        loop
     */
    private void doShift(final Term renameUpdate, Goal g, PosInOccurrence pos,
            final Term loopFormula) {
        ImmutableList<Term> updateList =
            ImmutableSLList.<Term>nil().prepend(UpdateApplication.getUpdate(loopFormula));
    ...
    private Term generateRenameUpdate(Term loopFormula) {
        ...
        for (UpdateableOperator lhs : updatedLocations) {
            ...
            final ProgramElementName freshConsSymb = new ProgramElementName(
                tb.newName("f_" + lhs.name().toString(), services.getNamespaces()));
        ...
        Term renamesAndInverseEvents =
            parallelInversesEvents.op() == UpdateJunctor.SKIP ? keepParallelUpdateRenames
                    : tb.sequential(keepParallelUpdateRenames, parallelInversesEvents);
        allParallelUpdateRenames =
            parallelInvAnonEvents.op() == UpdateJunctor.SKIP ? renamesAndInverseEvents
     ...
     *        of {@link #goal}
     */
    private void renameFormulasOnSemisequent(final Term renameUpdate, Semisequent semi,
            boolean antec) {
    ...
    private void shiftEventUpdate(Term eventUpdate, Term counter) {
        ...
        Term writeMarker =
            tb.func(services.getTypeConverter().getDependenciesLDT().getWriteMarker());
        ...
        goal.addFormula(
            new SequentFormula(tb.apply(keepParallelUpdateRenames, linkTerm4EventUpdate)), true,
            true);

        final Term relaxedLinkTerm =
            tb.ife(cond3, tb.and(tb.relaxedRPred(tb.empty(), tb.timestamp()),
                tb.relaxedWPred(tb.empty(), tb.timestamp())), tb.tt());
        goal.addFormula(new SequentFormula(tb.apply(allParallelUpdateRenames, relaxedLinkTerm)),
            true, true);
    }
    ...
    private void shiftAnonEventUpdate(Term anonEventUpdate, Term counter) {
        ...
        //
        // goal.addFormula(new SequentFormula(tb.apply(keepParallelUpdateRenames,
        // linkTerm4InvEventUpdate)), true, true);
    }
}
