...
public class SWTBotProfilePropertiesTest {
   ...
   public void testBasics() throws CoreException, InterruptedException {
      ...
      assertTrue("Can select profiles without project specific settings",
      ...
      assertTrue("List of profiles does not contain all profiles",
            items.length == ALL_PROFILES.size());
      for (int i = 0; i < ALL_PROFILES.size(); i++) {
         assertTrue("List does not contain profiles in sorted order",
               items[i].equals(ALL_PROFILES.get(i).getName()));
      }
      ...
      assertTrue("Project profile not changed properly",
            projectProfile == ALL_PROFILES.get(newIndex));
