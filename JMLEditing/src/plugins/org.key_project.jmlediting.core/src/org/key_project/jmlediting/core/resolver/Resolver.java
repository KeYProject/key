...
public final class Resolver {
    ...
    public static ResolveResult resolve(final ICompilationUnit cu, IASTNode toResolve) throws JavaModelException, ParserException {
        ...
        if(toResolve == null || cu == null || toResolve.getType() != NodeTypes.STRING) {
            return null;
        }
        ...
        if(!isSingleIdentifier(name)) {
            // TODO: do something to find method identifiers, because they may be overloaded.
        }
        ...
        ASTNode jdtNode = findVariableIdentifier(context, name, null);
        ...
        while(jdtNode == null) {
            ...
            jdtNode = findVariableIdentifier(context.getParent(), name, context);
    ...
    private static boolean isSingleIdentifier(String identifier) {
    ...
    private static ASTNode findVariableIdentifier(final ASTNode context, String name, final ASTNode except) {
        ...
        if(context instanceof TypeDeclaration) {
            if(((TypeDeclaration) context).getName().getIdentifier().equals(name)) {
                return findVariableIdentifier(context, name, except);
            }
            
            for(FieldDeclaration field : ((TypeDeclaration) context).getFields()) {
                ASTNode result = findVariableIdentifier(field, name, except);
                if(result != null) {
                    return result;
                }
            }
            ...
            for(MethodDeclaration method : ((TypeDeclaration) context).getMethods()) {
                ASTNode result = findVariableIdentifier(method, name, except);
                if(result != null) {
                    return result;
                }
            }
            ...
            if(((MethodDeclaration) context).getName().getIdentifier().equals(name)) {
                ...
                if(except == null && !((MethodDeclaration) context).isConstructor()) {
                    for(Object parameter : ((MethodDeclaration) context).parameters()) {
                        ASTNode result = findVariableIdentifier(((SingleVariableDeclaration) parameter), name, except);
                        if(result != null) {
                            return result;
                        }
                    }
        ...
        // FIELD DECLERATION
            ...
            for(Object fragment : ((FieldDeclaration) context).fragments()) {
                ASTNode result = findVariableIdentifier(((VariableDeclarationFragment) fragment), name, except);
                if(result != null) {
                    return result;
                }
            }
