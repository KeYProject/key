   ...
   public final Object[] modify(Object parent, Object[] currentChildren) {
      ...
      if (comments != null) {
         ...
         if (javaParent.getElementType() == IJavaElement.TYPE && javaParent.getParent().equals(root)){
            ...
            Object[] childrenToShowInOutline = new Object[currentChildren.length+comments.getClassInvariants().size()];
            
           //add old elements
            for (int i =  comments.getClassInvariants().size(); i < currentChildren.length+comments.getClassInvariants().size(); i++){
               childrenToShowInOutline[i] = currentChildren[i-comments.getClassInvariants().size()];
            }
            int i = 0;
            //add JML elements
