...
/**
 * Marks a {@link Feature} whose cost for a given rule application does not change as the proof goes
 * on: asked to score the same application now or much later, it always returns the same cost. Such a
 * feature derives its cost only from the rule application it is scoring, the taclet, how its schema
 * variables and {@code \assumes} matched, and the term the application is applied to, and from
 * nothing else about the goal.
 *
 * <p>
 * In particular, the cost of a feature you mark here must NOT depend on any of the following, all of
 ...
 *
 * <p>
 * <b>Why this annotation exists.</b> To pick the next rule, the automatic strategy asks the features
 ...
 * scoring an application return an unchanging cost, the prover computes that cost once and then keeps
 ...
 * sequent, or counts how the branch has split so far: its answer, and hence the cost, can differ from
 * one moment to the next. Such a feature must be marked {@link VolatileCost}, not this one. When you
 * are not sure, leave the feature unmarked.
 *
 * <p>
 ...
 * that is <em>not</em> in fact fixed, on the other hand, lets the strategy work with a stale cost and
 ...
 * {@code -Dkey.strategy.costReuse.verify=true}: it recomputes every reused cost and prints a warning
 * whenever the reused value disagrees with a freshly computed one, pointing at the offending taclet.
 *
 * <p>
