 ...
 * A {@code Trigger} consists of a schema variable ({@code triggerVar}), a trigger term {@code trigger}
 ...
 * <p>
 *  A trigger is activated if the trigger term {@code trigger} can be matched against a formula/term occurring in the
 *  sequent and the such found instantiation for the trigger variable {@code triggerVar} does not satisfy the
 *  avoid conditions {@code avoidConditions}.
 * </p>
 *
 *
 * The {@code Trigger} is used to determine whether the application of a taclet is promising in a given proof situation.
 * A {@code Trigger} has <strong>no</strong> influence on correctness of the rule, and in particular, does not influence
 * the rule's applicability. The rule itself must be correct in any situation where it is applicable.
 *
 */
public record Trigger(OperatorSV triggerVar, Term trigger, ImmutableList<Term> avoidConditions) {
    ...
    /**
     * Checks whether there are any conditions to indicate to strategies that an application of the taclet is not
