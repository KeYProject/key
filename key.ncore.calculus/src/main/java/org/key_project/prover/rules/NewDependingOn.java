 ...
 */
public NewDependingOn{if(!((first.isSkolemTerm())&&(second.isFormula()||second.isTerm()))){throw new RuntimeException("NewDependingOn: First SchemaVariable has to be a SkolemTermSV or FormulaSV, "+"the second one has to be a FormulaSV or a TermSV");}}

/**
 * returns the first SchemaVariable of the pair. This SchemaVariable has to be matched to a
 * QuantifiableVariable
 */
@Override public SchemaVariable first(){return first;}

/**
 * returns the second SchemaVariable of the pair.
 */
@Override public SchemaVariable second(){return second;}

public String toString(){return"\\newDependingOn("+first()+", "+second()+")";}

public boolean equals(Object o){if(!(o instanceof NewDependingOn(SchemaVariable first1,SchemaVariable second1))){return false;}return(first1==first()&&second1==second());}

public int hashCode(){int result=17;result=37*result+first().hashCode();result=37*result+second().hashCode();return result;}}
