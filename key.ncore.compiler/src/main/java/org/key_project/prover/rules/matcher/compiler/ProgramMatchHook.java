 ...
 * a
 * symbolic-execution taclet). This is the second of the two axes on which the Java/Rust/Solidity
 ...
 * the
 * enclosing modality head can advance to the post-condition subterm afterwards.
 *
 * <p>
 * The framework owns the surrounding modality skeleton (check the modality, match its kind, then
 * the
 * program, then recurse the subterms); a language plugs in only the divergent program matching
 * here.
 * Java additionally has the rich {@code ContextStatementBlock} prefix/suffix machinery, which is
 * entirely encapsulated behind this hook; Rust and Solidity supply their own simpler block
 * matchers.
 */
public interface ProgramMatchHook {
     ...
     * the
     * source program block; it consumes that block.
     *
