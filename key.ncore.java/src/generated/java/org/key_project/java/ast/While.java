...
public final class While extends JavaSourceElement implements LoopStatement {
    ...
    public While withAttachedJml(ImmutableList<TextualJMLConstruct> attachedJml) {
        return new While(attachedJml, body(), guard(), inits(), positionInfo(), updates());
    }

    public While withBody(Statement body) {
        return new While(attachedJml(), body, guard(), inits(), positionInfo(), updates());
    }

    public While withGuard(IGuard guard) {
        return new While(attachedJml(), body(), guard, inits(), positionInfo(), updates());
    }

    public While withInits(ILoopInit inits) {
        return new While(attachedJml(), body(), guard(), inits, positionInfo(), updates());
    }

    public While withPositionInfo(PositionInfo positionInfo) {
        return new While(attachedJml(), body(), guard(), inits(), positionInfo, updates());
    }

    public While withUpdates(IForUpdates updates) {
        return new While(attachedJml(), body(), guard(), inits(), positionInfo(), updates);
    }
    ...
    public String toString() {
        return "While[attachedJml=%s, body=%s, guard=%s, inits=%s, positionInfo=%s, updates=%s]".formatted(attachedJml, body, guard, inits, positionInfo, updates);
    }
