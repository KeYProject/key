...
@NullMarked()
public final class For extends JavaSourceElement implements LoopStatement {
    ...
    public For(@EqEx ImmutableList<TextualJMLConstruct> attachedJml, Statement body, IGuard guard, ILoopInit inits, @EqEx @Nullable PositionInfo positionInfo, IForUpdates updates) {
    ...
    public For(@EqEx ImmutableList<TextualJMLConstruct> attachedJml, Statement body, IGuard guard, ILoopInit inits, IForUpdates updates) {
        this.attachedJml = Objects.requireNonNull(attachedJml);
        this.body = Objects.requireNonNull(body);
        this.guard = Objects.requireNonNull(guard);
        this.inits = Objects.requireNonNull(inits);
    ...
    public For withAttachedJml(ImmutableList<TextualJMLConstruct> attachedJml) {
        return new For(attachedJml, body(), guard(), inits(), positionInfo(), updates());
    }

    public For withBody(Statement body) {
        return new For(attachedJml(), body, guard(), inits(), positionInfo(), updates());
    }

    public For withGuard(IGuard guard) {
        return new For(attachedJml(), body(), guard, inits(), positionInfo(), updates());
    }

    public For withInits(ILoopInit inits) {
        return new For(attachedJml(), body(), guard(), inits, positionInfo(), updates());
    }

    public For withPositionInfo(PositionInfo positionInfo) {
        return new For(attachedJml(), body(), guard(), inits(), positionInfo, updates());
    }

    public For withUpdates(IForUpdates updates) {
        return new For(attachedJml(), body(), guard(), inits(), positionInfo(), updates);
    }
    ...
    public String toString() {
        return "For[attachedJml=%s, body=%s, guard=%s, inits=%s, positionInfo=%s, updates=%s]".formatted(attachedJml, body, guard, inits, positionInfo, updates);
    }
