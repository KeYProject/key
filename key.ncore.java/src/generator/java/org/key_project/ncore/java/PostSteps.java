...
public class PostSteps {
    String topClass = "JavaSourceElement";
    String name = "org.key_project.java.ast.visitor";
    List<String> imports =
            List.of("org.key_project.java.ast.visitor.*", "org.key_project.java.ast.*",
                    "de.uka.ilkd.key.java.ast.PositionInfo", "org.key_project.util.collection.*");
    String listClass = "RoList";

    public class CreateVisitor implements PostStep {

        @Override
        public void applyOn(List<CompilationUnit> nodeUnits, SourceRoot sourceRoot) {
    ...
    class CreateVoidVisitor implements PostStep {
        @Override
        public void applyOn(List<CompilationUnit> nodeUnits, SourceRoot sourceRoot) {
            var cu = new CompilationUnit();
            ...
            for (
                    CompilationUnit clazz : nodeUnits) {
                try {
                    var t = clazz.getPrimaryType().get();
                    if (!(t instanceof ClassOrInterfaceDeclaration c))
                        continue;
    ...
    class CreateTraversalVisitor implements PostStep {
        @Override
        public void applyOn(List<CompilationUnit> nodeUnits, SourceRoot sourceRoot) {
            var cu = new CompilationUnit();
            var type = createTypeAndSetDefaults(cu, "CopyVisitor", PUBLIC);
            type.getImplementedTypes().
                    add((ClassOrInterfaceType) StaticJavaParser.parseType("Visitor<"+topClass+">"));
    ...
    class CreateTraversalCopyOnDemandVisitor implements PostStep {
        @Override
        public void applyOn(List<CompilationUnit> nodeUnits, SourceRoot sourceRoot) {
            var cu = new CompilationUnit();
            ...
            type.getImplementedTypes().add(
                    (ClassOrInterfaceType) StaticJavaParser.parseType("Visitor<"+topClass+">"));
            addAcceptMethods(type);
            ...
            for (CompilationUnit clazz : nodeUnits) {
                try {
                    ...
                    t.getFields()
                            ...
                            .filter(PostSteps.this::isAstNode)
                            .forEach(f -> body.addStatement(
                                    "b.%s = (%s) accept(n.%s());"
                                            .formatted(f.getVariable(0).getNameAsExpression(),
                                                    f.getVariable(0).getTypeAsString(),
    ...
    private boolean isAstNode(FieldDeclaration field) {
        return isAstNode(field.getVariables().getFirst());
    }

    private boolean isAstNode(VariableDeclarator v) {
        return isAstNode(v.getType());
    }

    private boolean isAstNode(Type type) {
        if (type.isClassOrInterfaceType()) {
            var c = type.asClassOrInterfaceType();
            if (c.getNameAsString().equals(listClass)) {
    ...
    /// }
    /// ```
    private void addAcceptMethods(ClassOrInterfaceDeclaration type) {
...

        {
            ...
            acceptList.addParameter(StaticJavaParser.parseType(listClass + "<T>"), "n");
            acceptList.setType(listClass + "<T>");

            var collector =
                    listClass.equals("RoList") ? ".collect(RoList.collector())"
                            : ".toList()";
            acceptList.getBody().get().addStatement(
                    "return n != null ? n.stream().map(it -> (T) it.accept(this)) " + collector + " : null;");
        }
    ...
    private ClassOrInterfaceDeclaration createTypeAndSetDefaults(
            CompilationUnit cu, String typeName, Modifier.DefaultKeyword... mods) {

        cu.setPackageDeclaration(name);
        imports.forEach(cu::addImport);
        cu.setStorage(ROOT.resolve("%s/%s.java".formatted(name.replace('.', '/'), typeName)));
        return cu.addClass(typeName, mods);
    }

    class CreateArgVisitor implements PostStep {
        @Override
        public void applyOn(List<CompilationUnit> nodeUnits, SourceRoot sourceRoot) {
            var cu = new CompilationUnit();
