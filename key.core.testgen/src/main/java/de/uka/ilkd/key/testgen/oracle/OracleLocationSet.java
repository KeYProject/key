...
public class OracleLocationSet {
    ...
    public static OracleLocationSet union(OracleLocationSet l1, OracleLocationSet l2) {
        ...
        if (l1 == ALL_LOCS || l2 == ALL_LOCS) { return ALL_LOCS; }
    ...
    public static OracleLocationSet intersect(OracleLocationSet l1, OracleLocationSet l2) {
        ...
        if (l1 == EMPTY || l2 == EMPTY) { return EMPTY; }

        if (l1 == ALL_LOCS) { return l2; }

        if (l2 == ALL_LOCS) { return l1; }

        OracleLocationSet result = new OracleLocationSet();
        for (OracleLocation l : l1.locs) { if (l2.contains(l)) { result.add(l); } }


        for (OracleLocation l : l2.locs) { if (l1.contains(l)) { result.add(l); } }
    ...
    public boolean contains(OracleLocation l) {
        for (OracleLocation loc : locs) {
            ...
            if (loc.equals(l)) { return true; }

            if (loc.isAllFields() && loc.getObject().equals(l.getObject())) { return true; }
    ...
    public String toString() {
        ...
        for (OracleLocation loc : locs) { result.append(loc).append(" "); }
