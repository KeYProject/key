...
public abstract class AbstractTestGenerator {
    ...
    public void generateTestCases(final StopRequest stopRequest, final TGReporter log)
        ...
        final List<SolverType> solvers = List.of(SolverTypes.Z3_CE_SOLVER);
        SolverTypes.Z3_CE_SOLVER.checkForSupport();
        if (stopRequest.shouldStop() || Thread.interrupted()) {
            return;
        }
            ...
            try {
                ...
                if (removeDuplicatePathConditions) {
                    ...
                    p = createProofForTestingNoDuplicate(oldGoalIter.next(),
                        Collections.emptyList(), removePostCondition);
                }
                res.add(Objects.requireNonNull(p));
            } catch (final Exception e) {
     ...
     */
    private @Nullable Proof createProofForTestingNoDuplicate(Node node, List<Proof> otherProofs,
    ...
    protected void handleLauncherStopped(SolverLauncher launcher,
            Collection<SMTSolver> problemSolvers, TGReporter log) {
        try {
