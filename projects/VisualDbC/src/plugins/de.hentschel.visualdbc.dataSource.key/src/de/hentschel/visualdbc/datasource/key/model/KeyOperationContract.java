...
/**
 * Special KeY implementation of {@link IDSOperationContract}.
 * @author Martin Hentschel
 */
public class KeyOperationContract extends MemoryOperationContract {
   /**
   ...
   public IDSProof openInteractiveProof(String obligation) throws DSException, DSCanceledException {
      if (connection.isInteractive()) {
         ...
         if (!isProofValid(oldDsProof)) {
            // Open new proof.
            OpenedProof proofResult = connection.openProof(type, pm, oc, obligation);
            if (proofResult != null) {
