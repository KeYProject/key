...
public class JMLPreferencesHelperTest {
   ...
   public void testSetDefaultProfile() {
      ...
      JMLPreferencesHelper.setDefaultJMLProfile(defaultProf);
      assertTrue("Default profile is not correct after changing", JMLPreferencesHelper.getDefaultJMLProfile() == defaultProf);
   }
   ...
   public void testSetProjectProfile() throws CoreException {
      IProject project = TestUtils.createJavaProject("Test");
      assertTrue("New profile has no project specific profile", !JMLPreferencesHelper.hasProjectJMLProfile(project));
      assertTrue("getProjectJMLProfile returns not null when no profile set", JMLPreferencesHelper.getProjectJMLProfile(project) == null);
      
      IJMLProfile profile = profiles.get(0);
      JMLPreferencesHelper.setProjectJMLProfile(project, profile);
      assertTrue("Project has no profile but it has been set", JMLPreferencesHelper.hasProjectJMLProfile(project));
      assertTrue("Wrong profile returned for project", JMLPreferencesHelper.getProjectJMLProfile(project) == profile);
      ...
      assertTrue("Project has profile but it has been cleared", !JMLPreferencesHelper.hasProjectJMLProfile(project));
      assertTrue("getProjectJMLProfile returns not null when no profile set", JMLPreferencesHelper.getProjectJMLProfile(project) == null);
   }
