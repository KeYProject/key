 ...
 */
public class PostconditionComposite implements IDisposable {
   /**
   ...
   protected void computeAndAddNewContent(final IKeYSEDDebugNode<?> node) {
      try {
         if (node != null) {
            ...
            final Node keyNode = executionNode.getProofNode();
            Term term = keyNode.getAppliedRuleApp().posInOccurrence().subTerm();
            if (term.op() instanceof Modality) {
               term = term.sub(0);
            }
            final Term termToShow = removeUninterpretedPredicate(keyNode, term);
            // Compute result
   ...
   protected Term removeUninterpretedPredicate(TermBuilder tb, 
      ...
      if (term.op() == Junctor.AND) {
         ...
         if (subsChanged) {
            Term newAnd = tb.and(newSubs);
            if (term.hasLabels()) {
               newAnd = tb.label(newAnd, term.getLabels());
            }
            return newAnd;
         }
         else {
