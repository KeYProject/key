 ...
 * </ul>
 * <p>
 * The Java reflection framework allows to load classes at runtime for instance via
 * {@link Class#forName(String)}. In a configuration-file configured classes can also be
 * instantiated via the {@link ServiceLoader}. But both approaches work only, if the {@link Class}
 * to work with is known by the current {@link ClassLoader}. This is no problem in a normal Java
 * Application since all classes are managed by the same {@link ClassLoader}. But in an OSGI setup
 * like Eclipse, each component (plug-in) has its own {@link ClassLoader}. The consequence is that
 * the reflection framework can only work with classes in the same component as they are only known
 * by the current {@link ClassLoader}.
 * <p>
 * To achieve the same behavior in all supported types provides this class utility methods which
 ...
 * instance is accessible via {@link #getClassLoader()} which is by default the
 * {@link JavaApplicationClassLoader}. In an OSGI (Eclipse) setup the instance is replaced via
 * {@link #setClassLoader(IClassLoader)} with an {@code OSGIClassLoader} instance.
 *
