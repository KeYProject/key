...
public final class Immutables {
    ...
    public static <T> ImmutableList<T> filter(ImmutableList<T> ts, Predicate<T> predicate) {
        ...
        while (ts.size() > 0) { T hd = ts.head(); if (predicate.test(hd)) { acc = acc.prepend(hd); } ts = ts.tail(); }
        return acc.reverse();
    }
    ...
    public static <T, R> ImmutableList<R> map(ImmutableList<T> ts, Function<T, R> function) {
        ...
        while (ts.size() > 0) { T hd = ts.head(); acc = acc.prepend(function.apply(hd)); ts = ts.tail(); }
        return acc.reverse();
    }
}
