...
public class TermActionUtil {
    ...
    public TermActionUtil(@NonNull NodeTextId nodeTextId, @NonNull KeYEnvironment<?> env, @NonNull PosInSequent pos, @NonNull Goal goal) {
        this.pos = pos;
        this.goal = goal;
        ...
        for (ProofMacro macro : ProofMacroFacade.instance().getMacros()) {
            var id = new KeyIdentifications.TermActionId(nodeTextId.nodeId(), pos.toString(), "macro:" + macro.getScriptCommandName());
            TermActionDesc ta = new TermActionDesc(id, macro.getName(), macro.getDescription(), macro.getCategory(), TermActionKind.Macro);
            add(ta);
        }
        ...
        for (TacletApp tacletApp : find) {
            var id = new KeyIdentifications.TermActionId(nodeTextId.nodeId(), pos.toString(), "find:" + tacletApp.rule());
        ...
        for (TacletApp tacletApp : nofind) {
            var id = new KeyIdentifications.TermActionId(nodeTextId.nodeId(), pos.toString(), "nofind:" + tacletApp.rule());
            TermActionDesc ta = new TermActionDesc(id, tacletApp.rule().displayName(),
                    tacletApp.rule().toString(), "", TermActionKind.Taclet);
            add(ta);
        }
