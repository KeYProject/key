...
public class HTMLReportEngine extends AbstractReportEngine {
   ...
   public String createReport(AbstractEvaluation evaluation) throws Exception {
      ...
      if (!formInputs.isEmpty()) {
         ...
         appendToolStatistics(evaluation, statistics, sb);
         appendChoiceStatistcs(evaluation, statistics, sb);
   ...
   /**
    * Appends the computed {@link ChoiceStatistics} to the HTML report.
    * @param evaluation The analyzed {@link AbstractEvaluation}.
    * @param statistics The computed {@link Statistics}s.
    * @param sb The {@link StringBuffer} to append to.
    */
   protected void appendChoiceStatistcs(AbstractEvaluation evaluation, Statistics statistics, StringBuffer sb) {
      ...
      for (AbstractForm form : evaluation.getForms()) {
         Map<AbstractPage, Integer> questionCount = countChoiceStatistics(statistics, form);
         int formSpan = 0;
               ...
               if (pageSpan > 0) {
                  ...
                  if (page instanceof QuestionPage) {
                     ...
                     for (AbstractQuestion question : questionPage.getQuestions()) {
                        if (appendChoicesQuestion(evaluation, statistics, question, !firstChoiceQuestion, sb)) {
    ...
    * @param sb The {@link StringBuffer} to append to.
    */
   protected boolean appendChoicesQuestion(AbstractEvaluation evaluation, Statistics statistics, AbstractQuestion question, boolean openTr, StringBuffer sb) {
            ...
            for (Choice choice : choiceQuestion.getChoices()) {
               if (choice.countChildQuestions() > 0) {
                  for (AbstractQuestion cildQuestion : choice.getChildQuestions()) {
                     if (appendChoicesQuestion(evaluation, statistics, cildQuestion, questionAppended || openTr, sb)) {
                        questionAppended = true;
                     }
                  }
               }
    ...
    * @return The results.
    */
   protected Map<AbstractPage, Integer> countChoiceStatistics(Statistics statistics, AbstractForm form) {
      ...
      for (AbstractPage page : form.getPages()) {
         ...
         if (!page.isReadonly()) {
            if (page instanceof QuestionPage) {
               QuestionPage questionPage = (QuestionPage) page;
               for (AbstractQuestion question : questionPage.getQuestions()) {
                  pageCount += countChoiceStatistics(statistics, question);
               }
            }
            else {
               throw new IllegalStateException("Unsupported page: " + page);
    ...
    */
   protected int countChoiceStatistics(Statistics statistics, AbstractQuestion question) {
      int questionCount = 0;
      ...
      if (question instanceof AbstractChoicesQuestion) {
         ...
         if (choiceQuestion.hasChildQuestions()) {
            for (Choice choice : choiceQuestion.getChoices()) {
               if (choice.countChildQuestions() > 0) {
                  for (AbstractQuestion cildQuestion : choice.getChildQuestions()) {
                     questionCount += countChoiceStatistics(statistics, cildQuestion);
                  }
               }
            }
         }
    ...
    * @param sb The {@link StringBuffer} to append to.
    */
   protected void appendToolStatistics(AbstractEvaluation evaluation, Statistics statistics, StringBuffer sb) {
