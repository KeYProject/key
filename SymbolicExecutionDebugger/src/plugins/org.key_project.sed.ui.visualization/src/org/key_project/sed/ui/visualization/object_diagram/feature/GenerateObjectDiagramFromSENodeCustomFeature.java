 ...
 * on the state provided via an {@link ISENode}.
 * The {@link ISENode} is specified via property {@link #PROPERTY_NODE} 
...
public class GenerateObjectDiagramFromSENodeCustomFeature extends AbstractGenerateObjectDiagramCustomFeature {
    ...
    */
   public GenerateObjectDiagramFromSENodeCustomFeature(IFeatureProvider fp) {
      super(fp);
   }
   ...
   public void execute(ICustomContext context) {
      ...
      if (contextNode instanceof ISENode) {
         node = (ISENode)contextNode;
      }
    ...
    * @param node The {@link ISENode} to visualize its state. 
    * @param monitor The {@link IProgressMonitor} to use.
    * @return The {@link PictogramElement} of the created state.
    * @throws DebugException Occurred Exception.
    */
   protected PictogramElement createState(ODModel model, ISENode node, IProgressMonitor monitor) throws DebugException {
   ...
   protected boolean isMultiValued(IVariable variable) throws DebugException {
      return variable.getValue() instanceof ISEValue &&
             ((ISEValue)variable.getValue()).isMultiValued();
   }
   ...
   protected boolean isObject(IVariable variable) throws DebugException {
      return variable.getValue() instanceof ISEValue &&
             ((ISEValue)variable.getValue()).isObject();
   }
}