...
public class TacletTranslationSelection {
    ...
    private TreeCellEditor getTreeCellEditor() {
        return new DefaultTreeCellEditor(getSelectionTree(), getTreeCellRenderer()) {
            public Component getTreeCellEditorComponent(JTree t, Object value, boolean selected,
                    boolean expanded, boolean leaf, int row) {

                TreeItem node = (TreeItem) value;

                TreeItem root = (TreeItem) t.getModel().getRoot();
                return leaf ? treePanels[LEAF_PANEL][EDIT].init(node, root, t, smtSettings)
                        : treePanels[INNER_PANEL][EDIT].init(node, root, t, smtSettings);
        ...
        if (cellRenderer == null) {
            cellRenderer = new DefaultTreeCellRenderer() {
                ...
                public Component getTreeCellRendererComponent(JTree tree, Object value,
                        boolean select, boolean expanded, boolean leaf, int row, boolean arg6) {
                    TreeItem node = (TreeItem) value;
                    TreeItem root = (TreeItem) tree.getModel().getRoot();

                    return leaf ? treePanels[LEAF_PANEL][PAINT].init(node, root, tree, smtSettings)
                            : treePanels[INNER_PANEL][PAINT].init(node, root, tree, smtSettings);
