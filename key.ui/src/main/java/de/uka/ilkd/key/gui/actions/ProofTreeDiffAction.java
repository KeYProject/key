...
public class ProofTreeDiffAction extends MainWindowAction {
    ...
    public void actionPerformed(ActionEvent e) {
        ...
        var currentLoaded =
            mainWindow.getProofList().getModel().getLoadedProofs().toArray(new Proof[0]);
    ...
    private Proof selectProof(String s, Proof[] proofs) {
        ...
        var selected =
            JOptionPane.showInputDialog(mainWindow, "Select a " + s + " proof for comparison",
    ...
    public void showFrame(Proof left, Proof right) {
        ...
        dockable.addAction(
            DockingHelper.translateAction(panel.getActionFindingDifferencesMatching()));
        dockable.addAction(
            DockingHelper.translateAction(panel.getActionFindingDifferencesString()));
...
class ProofTreeDiffView extends JPanel {
    ...
    public ProofTreeDiffView(Proof left, Proof right, VisibleTermLabels termLabels) {
        ...
        treeRight.getDelegateView().addTreeSelectionListener(e -> {
            final var nodeRight =
                ((GUIAbstractTreeNode) e.getPath().getLastPathComponent()).getNode();
...
class HasDifferenceBasedOnLogicPrinter implements BiFunction<Sequent, Sequent, Boolean> {
    ...
    HasDifferenceBasedOnLogicPrinter(Proof proofLeft, Proof proofRight,
    ...
    private static Collection<String> print(Function<Term, String> printer,
            ImmutableList<SequentFormula> list) {
        return list.stream().map(it -> printer.apply(it.formula())).sorted()
...
class HasDifferenceBasedOnNonMatching implements BiFunction<Sequent, Sequent, Boolean> {
    ...
    HasDifferenceBasedOnNonMatching(Proof proofLeft, Proof proofRight,
            VisibleTermLabels termLabels) {
    ...
    public Boolean apply(Sequent left, Sequent right) {
        var s = SequentDifference.create(proofLeft.getServices(), proofRight.getServices(), left,
            right, termLabels);
