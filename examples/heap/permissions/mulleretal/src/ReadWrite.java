public class ReadWrite {
    ...
    /*@ public normal_behavior
          ...
          assignable<heap> this.val, this.lockedRds;
          diverges true; @*/
    public /*@ helper @*/ void doWrite() {
        boolean done = false;
        /*@ loop_invariant !\dl_readPermission(\permission(this.rds));
            assignable<permissions> this.rds, this.lockedRds, this.val;
            assignable<heap> this.lockedRds, this.val; @*/
        while(!done) {
            lock();
            if(rds == 0) {
              write();
    ...
    /*@ public normal_behavior
          ...
          ensures rds >= 0;
          ensures rds == 0 ==> \dl_writePermissionObject(this, \old(\permission(this.val)));
          ensures rds > 0 ==>  \dl_readPermissionObject(this, \old(\permission(this.val)));
          ensures \permission(this.rds) == \dl_transferPermission(\dl_FALSE(), this, \dl_currentThread(), 0, \old(\permission(this.rds)));
          ensures \permission(this.lockedRds) == \dl_transferPermission(\dl_FALSE(), this, \dl_currentThread(), 0, \old(\permission(this.lockedRds)));
          ensures \permission(this.val) == \dl_transferPermission(\dl_FALSE(), this, \dl_currentThread(), 0, \old(\permission(this.val)));
    ...
    /*@ public normal_behavior
          ...
          requires rds >= 0;
          ensures \permission(this.rds) == \dl_returnPermission(\dl_currentThread(), this, \old(\permission(this.rds)));
          ensures \permission(this.lockedRds) == \dl_returnPermission(\dl_currentThread(), this, \old(\permission(this.lockedRds)));
          ensures \old(this.rds) <= \old(this.lockedRds) ==>
              \permission(this.val) == \dl_returnPermission(\dl_currentThread(), this, \old(\permission(this.val)));
          ensures \old(this.rds) > \old(this.lockedRds) ==>
              \permission(this.val) == \dl_transferPermission(\dl_TRUE(), this, \dl_currentThread(), 0, \dl_returnPermission(\dl_currentThread(), this, \old(\permission(this.val))));
          assignable<permissions> this.rds, this.lockedRds, this.val;
