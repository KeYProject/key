...
private static BuildingIssue fromToken(String message,boolean isWarning,@Nullable Token token,@Nullable Throwable cause){if(token!=null){var position=Position.make(token);return new BuildingIssue(message,cause,isWarning,position,token.getTokenSource().getSourceName());}return new BuildingIssue(message,cause,isWarning,null,null);}
