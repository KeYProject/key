...
public class DefaultNameInfo extends AbstractService implements NameInfo, PropertyChangeListener {
    ...
    public void propertyChange(PropertyChangeEvent evt) {
        ...
        if (changedProp.equals(PropertyNames.CLASS_SEARCH_MODE)) { updateSearchMode(); }
    }
     ...
     *
     * @param name
     *        the name of the primitive type to be registered
    ...
    public void register(ClassType ct) {
        ...
        if (al != null) {
            for (Object o : al) { ArrayType at = (ArrayType) o; at.makeNames(); name2type.put(at.getFullName(), at); }
        }
    }
    ...
    public ClassType getJavaLangObject() {
        if (javaLangObject == null) { javaLangObject = getClassType("java.lang.Object"); }
        return javaLangObject;
    }

    public ClassType getJavaLangString() {
        if (javaLangString == null) { javaLangString = getClassType("java.lang.String"); }
        return javaLangString;
    }

    public ClassType getJavaLangBoolean() {
        if (javaLangBoolean == null) { javaLangBoolean = getClassType("java.lang.Boolean"); }
        return javaLangBoolean;
    }

    public ClassType getJavaLangByte() {
        if (javaLangByte == null) { javaLangByte = getClassType("java.lang.Byte"); }
        return javaLangByte;
    }

    public ClassType getJavaLangCharacter() {
        if (javaLangCharacter == null) { javaLangCharacter = getClassType("java.lang.Character"); }
        return javaLangCharacter;
    }

    public ClassType getJavaLangShort() {
        if (javaLangShort == null) { javaLangShort = getClassType("java.lang.Short"); }
        return javaLangShort;
    }

    public ClassType getJavaLangInteger() {
        if (javaLangInteger == null) { javaLangInteger = getClassType("java.lang.Integer"); }
        return javaLangInteger;
    }

    public ClassType getJavaLangLong() {
        if (javaLangLong == null) { javaLangLong = getClassType("java.lang.Long"); }
        return javaLangLong;
    }

    public ClassType getJavaLangFloat() {
        if (javaLangFloat == null) { javaLangFloat = getClassType("java.lang.Float"); }
        return javaLangFloat;
    }

    public ClassType getJavaLangDouble() {
        if (javaLangDouble == null) { javaLangDouble = getClassType("java.lang.Double"); }
        return javaLangDouble;
    }

    public ClassType getJavaLangClass() {
        if (javaLangClass == null) { javaLangClass = getClassType("java.lang.Class"); }
        return javaLangClass;
    }

    public ClassType getJavaLangCloneable() {
        if (javaLangCloneable == null) { javaLangCloneable = getClassType("java.lang.Cloneable"); }
        return javaLangCloneable;
    }

    public ClassType getJavaLangRunnable() {
        if (javaLangRunnable == null) { javaLangRunnable = getClassType("java.lang.Runnable"); }
        return javaLangRunnable;
    }

    public ClassType getJavaIoSerializable() {
        if (javaIoSerializable == null) { javaIoSerializable = getClassType("java.io.Serializable"); }
    ...
    public ClassType getJavaLangEnum() {
        if (javaLangEnum == null) { javaLangEnum = getClassType("java.lang.Enum"); }
        return javaLangEnum;
    }
    ...
    public Package createPackage(String name) {
        ...
        if (result == null) {
            ...
            if (ldp > 0) { createPackage(name.substring(0, ldp)); }
        }
        return result;
    }
    ...
    public List<Package> getPackages() {
        ...
        for (Package p : name2package.values()) { result.add(p); }
        return result;
    }

    public ClassType getClassType(String name) {
        Type result = getType(name);
        if (result instanceof ClassType) { return (ClassType) result; }
        return null;
    }
    ...
    public ArrayType createArrayType(Type basetype, int dimensions) {
        ...
        while (dimensions-- > 0) { result = createArrayType(result); }
    ...
    public Type getType(String name) {
        Debug.assertNonnull(name);
        updateModel();
        if (DEBUG) { Debug.log("Search requested for type " + name); }
        ...
        if (result == unknownType) {
            if (DEBUG) { Debug.log(name + " is known to be unknown"); }
            return null; // report null
        } else if (result == null) {
            if (name.endsWith("]")) {
                result = getType(name.substring(0, name.length() - 2));
                if (result != null) { return createArrayType(result); }
            }
            ...
            if (DEBUG && result == null) { Debug.log(name + " is set to unknown"); }
            name2type.put(name, (result != null) ? result : unknownType);
        }
        if (DEBUG && result != null) { Debug.log(name + " has been found"); }
        return result;
    }
    ...
    public List<Type> getTypes() {
        ...
        for (Type t : name2type.values()) { if (t != unknownType) { result.add(t); } }
        return result;
    }
    ...
    public List<ClassType> getTypes(Package pkg) {
        ...
        int s = tl.size();
        for (Type t : tl) { if (t instanceof ClassType ct) { if (ct.getContainer() == pkg) { result.add(ct); } } }
        return result;
    }
    ...
    public List<ClassType> getClassTypes() {
        ...
        List<Type> tl = getTypes();
        int s = tl.size();
        for (Type t : tl) { if (t instanceof ClassType) { result.add((ClassType) t); } }
        return result;
    }
    ...
    public List<Field> getFields() {
        ...
        for (Field f : name2field.values()) { result.add(f); }
        return result;
    }
    ...
    private boolean loadClass(String classname) {
        ...
        for (int i = 0; !result && i < searchMode.length; i += 1) {
            switch (searchMode[i]) {
            case SEARCH_SOURCE:
                if (DEBUG) { Debug.log("Searching source code: " + classname); }
                result = loadClassFromSourceCode(classname);
                break;
            case SEARCH_CLASS:
                if (DEBUG) { Debug.log("Searching class file: " + classname); }
                result = loadClassFromPrecompiledCode(classname);
                break;
            case SEARCH_REFLECT:
                if (DEBUG) { Debug.log("Searching class: " + classname); }
    ...
    public String information() {
        int unknown = 0;
        for (Type t : name2type.values()) { if (t == unknownType) { unknown += 1; } }
    ...
    void unregisterClassType(String fullname, boolean recycleArrayEntries) {
        ...
        while ((array = name2type.remove(fullname)) != null) {
            if (recycleArrayEntries) { al.add((ArrayType) array); }
            fullname += "[]";
        }
        // Assumes that for any given dimension, all array types with
        // smaller dimensions already exist.
        if (recycleArrayEntries) { removedArrayCache.put(old, al); }
    }
    ...
    public void unregisterPackages() {
        ...
        for (int i = cf.size() - 1; i >= 0; i -= 1) {
            ...
            if (ctc instanceof Package) { n2p.put(ctc.getFullName(), (Package) ctc); }
        }
    ...
    void handleTypeRename(ClassType ct, String unregisterFrom, String registerTo) {
        ...
        for (Field currentField : fl) {
            ...
            if (unregister) { unregisterField(fieldremove); }
            if (register) { register(currentField); }
        }
    }
