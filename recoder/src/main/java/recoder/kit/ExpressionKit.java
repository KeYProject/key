...
public class ExpressionKit {
    ...
    public static boolean containsStatements(Expression expr) {
        if (expr instanceof Statement) { if (!(expr instanceof ParenthesizedExpression)) { return true; } }
        if (expr instanceof ExpressionContainer con) {
            for (int i = 0, s = con.getExpressionCount(); i < s; i += 1) {
                if (containsStatements(con.getExpressionAt(i))) { return true; }
            }
        }
        return false;
    }
    ...
    public static List<Expression> collectPreceedingExpressions(Expression x) {
        ...
        if ((x instanceof MethodReference) || (x instanceof ConstructorReference)) {
            ...
            for (int i = 0, s = ec.getExpressionCount(); i < s; i += 1) { dest.add(ec.getExpressionAt(i)); }
        } else if (x instanceof ReferenceSuffix) {
            ReferencePrefix rp = ((ReferenceSuffix) x).getReferencePrefix();
            if (rp instanceof Expression) { dest.add((Expression) rp); }
        }
        while (true) {
            ExpressionContainer parent = x.getExpressionContainer();
            if (parent == null) { return dest; }
            ...
            if (leftAssociative) {
                ...
                if (parent instanceof ReferenceSuffix) {
                    if (((ReferenceSuffix) parent).getReferencePrefix() instanceof Expression) { i = 1; }
                }
                while ((expr = parent.getExpressionAt(i)) != x) { dest.add(expr); i += 1; }
            } else {
                for (int i =
            ...
            if (!(parent instanceof Expression)) { return dest; }
            x = (Expression) parent;
        }
    }
    ...
    public static boolean shiftPreceedingStatementExpressions(SourceInfo si, Expression x,
        ...
        for (int i = exprs.size() - 1; i >= 0; i -= 1) { if (!containsStatements(exprs.get(i))) { exprs.remove(i); } }
        ...
        for (int i = 0; i < exSize; i += 1) { Expression ex = exprs.get(i); exTypes[i] = si.getType(ex); }
        ...
        for (int i = 0; i < exSize; i += 1) {
            ...
            if (ch != null) { ch.replaced(ex, vref); }
        }
        ...
        if (ch != null) { for (Statement tempVarDecl : tempVarDecls) { ch.attached(tempVarDecl); } }
        return true;
    }
    ...
    public static Literal createDefaultValue(ProgramFactory f, NameInfo ni, Type t) {
        ...
        if (t instanceof PrimitiveType) {
            if (t == ni.getIntType()) { return f.createIntLiteral(0); }
            if (t == ni.getBooleanType()) { return f.createBooleanLiteral(false); }
            if (t == ni.getCharType()) { return f.createCharLiteral('\0'); }
            if (t == ni.getShortType()) { return f.createIntLiteral(0); }
            if (t == ni.getByteType()) { return f.createIntLiteral(0); }
            if (t == ni.getLongType()) { return f.createLongLiteral(0L); }
            if (t == ni.getFloatType()) { return f.createFloatLiteral(0.0f); }
            if (t == ni.getDoubleType()) { return f.createDoubleLiteral(0.0); }
