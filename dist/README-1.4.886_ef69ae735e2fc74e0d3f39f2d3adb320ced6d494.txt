...
----------------------------
	1) Copy the KeY.tgz to a temporary directory; 
	   Here: tmp/
	...
	6) (This step is only necessary if you want to make use of the
	   ...
	   environment variable. If not, use (e.g., in the csh to
	   include Simplify which is assumed to be located in
	   ~/simplify):
...
-------------
	...
	Go to subdirectory bin and run 'startkey' (Windows: 'startkey.bat')

	If you want to start the standalone prover (JavaDL or JML
	front-end) without Together, run 'startProver' (Windows:
	'startProver.bat') instead.	
